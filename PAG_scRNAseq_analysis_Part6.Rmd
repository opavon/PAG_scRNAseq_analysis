...
start_time <- Sys.time() # Takes around 1.4 hours
PAG_DESeq_set_0 <- DESeq2::DESeq(PAG_DESeq_set_0, test = "LRT", sfType = "poscounts", minReplicatesForReplace = Inf, useT = TRUE, minmu = 1e-6, parallel = TRUE)
...
start_time <- Sys.time() # Takes around 1.6 hours
PAG_DESeq_set_1 <- DESeq2::DESeq(PAG_DESeq_set_1, test = "LRT", sfType = "poscounts", minReplicatesForReplace = Inf, useT = TRUE, minmu = 1e-6, parallel = TRUE)
...
start_time <- Sys.time() # Takes around 1.9 hours
PAG_DESeq_set_2 <- DESeq2::DESeq(PAG_DESeq_set_2, test = "LRT", sfType = "poscounts", minReplicatesForReplace = Inf, useT = TRUE, minmu = 1e-6, parallel = TRUE)
...
start_time <- Sys.time() # Takes around 2.5 hours
PAG_DESeq_set_3 <- DESeq2::DESeq(PAG_DESeq_set_3, test = "LRT", sfType = "poscounts", minReplicatesForReplace = Inf, useT = TRUE, minmu = 1e-6, parallel = TRUE)
...
start_time <- Sys.time() # Takes around 2.1 hours
PAG_DESeq_set_12 <- DESeq2::DESeq(PAG_DESeq_set_12, test = "LRT", sfType = "poscounts", minReplicatesForReplace = Inf, useT = TRUE, minmu = 1e-6, parallel = TRUE)
...
start_time <- Sys.time() # Takes around 2.4 hours
PAG_DESeq_set_12a <- DESeq2::DESeq(PAG_DESeq_set_12a, test = "LRT", sfType = "poscounts", minReplicatesForReplace = Inf, useT = TRUE, minmu = 1e-6, parallel = TRUE)
...
summary(keep_13)
PAG_DESeq_set_13 <- PAG_DESeq_set_13[keep_13,] # Remove lowly expressed genes.

start_time <- Sys.time() # Takes around 2.6 hours
PAG_DESeq_set_13 <- DESeq2::DESeq(PAG_DESeq_set_13, test = "LRT", sfType = "poscounts", minReplicatesForReplace = Inf, useT = TRUE, minmu = 1e-6, parallel = TRUE)
...
summary(keep_13b)
PAG_DESeq_set_13b <- PAG_DESeq_set_13b[keep_13b,] # Remove lowly expressed genes.

start_time <- Sys.time() # Takes around 3.1 hours
PAG_DESeq_set_13b <- DESeq2::DESeq(PAG_DESeq_set_13b, test = "LRT", sfType = "poscounts", minReplicatesForReplace = Inf, useT = TRUE, minmu = 1e-6, parallel = TRUE)
...
summary(keep_23)
PAG_DESeq_set_23 <- PAG_DESeq_set_23[keep_23,] # Remove lowly expressed genes.

start_time <- Sys.time() # Takes around 2.9 hours
PAG_DESeq_set_23 <- DESeq2::DESeq(PAG_DESeq_set_23, test = "LRT", sfType = "poscounts", minReplicatesForReplace = Inf, useT = TRUE, minmu = 1e-6, parallel = TRUE)
...
summary(keep_123)
PAG_DESeq_set_123 <- PAG_DESeq_set_123[keep_123,] # Remove lowly expressed genes.

start_time <- Sys.time() # Takes around 3.0 hours
PAG_DESeq_set_123 <- DESeq2::DESeq(PAG_DESeq_set_123,test = "LRT", sfType = "poscounts", minReplicatesForReplace = Inf, useT = TRUE, minmu = 1e-6, parallel = TRUE)
...
#### Design 1&2&3 with interaction ####
keep_123ab <- rowSums(counts(PAG_DESeq_set_123ab) >= 5) >= 5 # Identify genes with 5 copies in at least 5 cells.
summary(keep_123ab)
PAG_DESeq_set_123ab <- PAG_DESeq_set_123ab[keep_123ab,] # Remove lowly expressed genes.

start_time <- Sys.time() # Takes around 3.6 hours
PAG_DESeq_set_123ab <- DESeq2::DESeq(PAG_DESeq_set_123ab, test = "LRT", sfType = "poscounts", minReplicatesForReplace = Inf, useT = TRUE, minmu = 1e-6, parallel = TRUE)
DESeq2::plotDispEsts(PAG_DESeq_set_123ab)
end_time <- Sys.time()
end_time - start_time
saveRDS(PAG_DESeq_set_123ab, file = "PAG_DESeq_set_123ab.rds")
...
```{r}
PAG_DESeq_results_123ab <- DESeq2::results(PAG_DESeq_set_123ab, c("cell.type", "VGAT", "VGluT2"), alpha = 0.1)
head(PAG_DESeq_results_123ab)
#mcols(PAG_DESeq_set_123ab, use.names = TRUE)
resultsNames(PAG_DESeq_set_123ab)
```
...
```{r}
summary(PAG_DESeq_results_123ab)
hist(PAG_DESeq_results_123ab$pvalue)

## We can also add a couple of extra columns that will be useful for the interactive visualization later
PAG_DESeq_results_123ab$log10BaseMean <- log10(PAG_DESeq_results_123ab$baseMean)
PAG_DESeq_results_123ab$mlog10PValue <- -log10(PAG_DESeq_results_123ab$pvalue)
rowData(PAG_DESeq_set_123ab)$log10Dispersion <- log10(rowData(PAG_DESeq_set_123ab)$dispersion)
rowData(PAG_DESeq_set_123ab)$DESeq2_VGAT_vs_VGluT2 <- PAG_DESeq_results_123ab # Or any other comparison you called using results
```
...
```{r}
PAG_DESeq_results_123ab_05 <- DESeq2::results(PAG_DESeq_set_123ab, alpha = 0.05)
table(PAG_DESeq_results_123ab_05$padj < 0.05)
```
...
```{r}
PAG_DESeq_results_0_LFC1 <- results(PAG_DESeq_results_0, lfcThreshold = 1)
summary(PAG_DESeq_results_0_LFC1)
table(PAG_DESeq_results_0_LFC1$padj < 0.1)
```
...
```{r}
sum(PAG_DESeq_results_0_05$padj < 0.05, na.rm = TRUE)
```
...
```{r}
resSig <- subset(PAG_DESeq_results_0_05, padj < 0.1)
...
```{r}
plotCounts(PAG_DESeq_set_123ab, gene = "Asic4", intgroup = "cell.type", 
           normalized = TRUE, transform = FALSE)

topGene <- rownames(PAG_DESeq_results_123ab)[which.min(PAG_DESeq_results_123ab$padj)]
plotCounts(PAG_DESeq_set_123ab, gene = topGene, intgroup=c("cell.type"))
```

We can also make custom plots using the `ggplot` function from the `ggplot2` package.
```{r}
library("ggbeeswarm")
geneCounts <- plotCounts(PAG_DESeq_set_123ab, gene = topGene, intgroup = c("PAG.arearegistration","cell.type"),
                         returnData = TRUE)
...
```{r}
library("apeglm")
resultsNames(PAG_DESeq_set_123ab) # Check to see which coef to choose
```

```{r}
PAG_DESeq_results_0_LFC <- lfcShrink(PAG_DESeq_results_0, coef="vgat_vs_vglut2", type="apeglm")
PAG_DESeq_results_0_LFC
DESeq2::plotMA(PAG_DESeq_results_0, ylim = c(-5, 5))
```
...
```{r}
plotMA(PAG_DESeq_results_0, ylim = c(-5,5))
topGene <- rownames(PAG_DESeq_results_0)[which.min(PAG_DESeq_results_0$padj)]
with(PAG_DESeq_results_0[topGene, ], {
...
```{r}
hist(PAG_DESeq_results_0$pvalue[PAG_DESeq_results_0$baseMean > 1], breaks = 0:20/20,
...
```{r}
idx <- identify(PAG_DESeq_results_0$baseMean, PAG_DESeq_results_0$log2FoldChange)
rownames(PAG_DESeq_results_0)[idx]
```
...
```{r}
names(PAG_DGE_list)
PAG_DGE_design <- model.matrix(~ mouse.sex + batch.processing + batch.sequencing_round + cell.type + PAG.arearegistration + PAG.APaxis + cell.type:PAG.arearegistration + cell.type:PAG.APaxis
...
```{r}
DGE_genes_keep <- edgeR::filterByExpr(PAG_DGE_list, PAG_DGE_design, min.count = 10) # Choose a specific gene filter
