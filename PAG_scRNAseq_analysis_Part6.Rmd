...
After running the two different graph-based clustering approaches with different `k` (from 5 to 15), we observe that when using the HVGs from Var modelling and the top 12 PCs, neither the Rank-Walktrap nor the Jaccard-Louvain approach manage to correctly identify the 12 cells in the Cd33+ cluster at any value of `k`. This, together with the fact that both the tSNEs and UMAPs obtained with the HVG-Var yield worse visualisations than their CV2 counterparts, leads us to not proceed with this approach.

When using the HVGs obtained from CV2 modelling and the top 17 PCs, we find that in most cases both the Rank-Walktrap and Jaccard-Louvain approaches successfully capture the 12 cells from the Cd33+ cluster. Most results identify between 2-4 VGAT clusters and between 4-6 VGluT2 clusters. After overlaying the results to the UMAP visualisation, we decide to keep the results from different `k` to further analyse.
...
We will keep the different solutions and further examine their stability and modularity. One thing is clear though, there are 3 main clusters in our dataset: VGAT neurons, VGluT2 neurons, and the small Cd33+ clusters. Furthermore, it seems that VGluT2 neurons may have a bit more granularity, with different subpopulations that trail the different PAG subdivisions. VGAT neurons seem to be slightly more homogeneous, but we consistently find at least 2 clusters that differentiate vlPAG neurons from the rest.
```{r}
library(scran)
set.seed(42)
...
As we can see when looking at both the tree and the embedding of the k-means clusters on the UMAP, using PCs from mean-var modelling doesn't generate a tree that reflects the origin of the cells (i.e. excitatory or inhibitory cells), whereas we do see that in the results obtained with PCs from mean-cv2 modelling (it even separates the Cd33+ cluster in an entirely different branch). Overall we seem to obtain similar clusters to those we obtained from the graph-clustering approach, at least for the cv2 scenario.
...
* `k=5:8` start further subdividing excitatory cells and the Cd33+ cluster, although these clusters are not very well defined.

* `k=9`, which was the suggested `k` by SC3, does offer the best clustering of excitatory cells and captures the Cd33+ cluster, and finds 4 inhibitory clusters, although they are a bit too close to each other in the UMAP. 

* `k=10` misses the Cd33+ cluster, but splits excitatory and inhibitory cells into better defined clusters.

* Most of `k=11:15` again miss the Cd33+ cluster or seem to overcluster either excitatory or inhibitory neurons.
...
plotReducedDim(PAG_sceset_qc_norm_filt_corr, 
               dimred = "UMAP_cv2_corrected_15_neighbors_0.01_min_dist",
               colour_by = "Cd33", text_by = "SNN_clusters_cv2_jaccard_k8", shape_by = "cell.type"
...
We can use several markers to highlight that this cluster is enriched for immune response markers:

* "Cd33": Siglec-3 is a transmembrane receptor expressed on cells of myeloid lineage and plays a role in maintaining immune cells (monocytes) in a resting state.

* "Plaur": encodes CD87, which acts as a receptor for urokinase plasminogen activator and is part of the plasminogen activation system invovled in inflammation and wound healing.

* "Cxcl16": C-X-C Motif Chemokine Ligand 16 acts as a scavenger receptor on macrophages and induces a strong chemotactic response.

* "Cd68": encodes a glycoprotein that is highly expressed by human monocytes and tissue macrophage. Could play a role in phagocytic activities of tissue macrophages, allowing homing of macrophage subsets to particular sites
...
plotReducedDim(PAG_sceset_qc_norm_filt_corr, 
               dimred = "UMAP_cv2_corrected_15_neighbors_0.01_min_dist",
               colour_by = "Plaur", text_by = "SNN_clusters_cv2_jaccard_k8", shape_by = "cell.type"
               ) + labs(title = "UMAP (CV2) - Jaccard-Louvain (k = 8)", x = "UMAP 1", y = "UMAP 2")

plotReducedDim(PAG_sceset_qc_norm_filt_corr, 
               dimred = "UMAP_cv2_corrected_15_neighbors_0.01_min_dist",
               colour_by = "Cxcl16", text_by = "SNN_clusters_cv2_jaccard_k8", shape_by = "cell.type"
               ) + labs(title = "UMAP (CV2) - Jaccard-Louvain (k = 8)", x = "UMAP 1", y = "UMAP 2")
```

For each clustering approach, we can plot only the macrophage enriched cluster and see how many cells appear. 
```{r}
...
plotReducedDim(PAG_sceset_qc_norm_filt_corr[, PAG_sceset_qc_norm_filt_corr$SNN_clusters_cv2_rank_k7==8], 
               dimred = "UMAP_cv2_corrected_15_neighbors_0.01_min_dist",
               colour_by = "Cd33", shape_by = "cell.type"
...
paste0("Macrophage cluster has ", sum(PAG_sceset_qc_norm_filt_corr$cutreedynamic_clusters_cv2==0), " cells in hierarchical cutreedynamic_clusters_cv2 method") # does not find Cd33+ cluster
...
# SC3 clustering
UMAP_sc3_final_composed <- (UMAP_cv2_15_001_PAGsubdivisions + UMAP_cv2_sc3_k4) +
  plot_annotation(title = "SC3 clustering on UMAP (CV2)", tag_levels = "A", 
                  theme = theme(plot.title = element_text(size = 12, face = "bold"), plot.tag = element_text(size = 11))) + 
  plot_layout(ncol = 2, guides = "collect")
UMAP_sc3_final_composed

# Save them all
ggsave(filename = str_c(date, "_UMAP_final_SNN_rank_k7_clusters_composed.pdf"),
       plot = UMAP_SNN_RankWalktrap_k7_final_composed,
       device = "pdf", # or one of "eps", "ps", "tex" (pictex), "pdf", "jpeg", "tiff", "png", "bmp", "svg" or "wmf" (windows only).
       path = path_for_figures,
...
ggsave(filename = str_c(date, "_UMAP_final_SNN_rank_k12_clusters_composed.pdf"),
       plot = UMAP_SNN_RankWalktrap_k12_final_composed,
       device = "pdf", # or one of "eps", "ps", "tex" (pictex), "pdf", "jpeg", "tiff", "png", "bmp", "svg" or "wmf" (windows only).
       path = path_for_figures,
       width = 8, height = 4, units = "in", dpi = 300,
       family = "Arial", bg = "transparent"
...
ggsave(filename = str_c(date, "_UMAP_final_SNN_jaccard_k5_clusters_composed.pdf"),
       plot = UMAP_SNN_JaccardLouvain_k5_final_composed,
       device = "pdf", # or one of "eps", "ps", "tex" (pictex), "pdf", "jpeg", "tiff", "png", "bmp", "svg" or "wmf" (windows only).
       path = path_for_figures,
       width = 8, height = 4, units = "in", dpi = 300,
       family = "Arial", bg = "transparent"
...
ggsave(filename = str_c(date, "_UMAP_final_SNN_jaccard_k8_clusters_composed.pdf"),
       plot = UMAP_SNN_JaccardLouvain_k8_final_composed,
       device = "pdf", # or one of "eps", "ps", "tex" (pictex), "pdf", "jpeg", "tiff", "png", "bmp", "svg" or "wmf" (windows only).
       path = path_for_figures,
       width = 8, height = 4, units = "in", dpi = 300,
       family = "Arial", bg = "transparent"
...
ggsave(filename = str_c(date, "_UMAP_final_SNN_jaccard_k9_clusters_composed.pdf"),
       plot = UMAP_SNN_JaccardLouvain_k9_final_composed,
       device = "pdf", # or one of "eps", "ps", "tex" (pictex), "pdf", "jpeg", "tiff", "png", "bmp", "svg" or "wmf" (windows only).
       path = path_for_figures,
       width = 8, height = 4, units = "in", dpi = 300,
       family = "Arial", bg = "transparent"
...
ggsave(filename = str_c(date, "_UMAP_final_SNN_jaccard_k13_clusters_composed.pdf"),
       plot = UMAP_SNN_JaccardLouvain_k13_final_composed,
       device = "pdf", # or one of "eps", "ps", "tex" (pictex), "pdf", "jpeg", "tiff", "png", "bmp", "svg" or "wmf" (windows only).
       path = path_for_figures,
       width = 8, height = 4, units = "in", dpi = 300,
       family = "Arial", bg = "transparent"
...
ggsave(filename = str_c(date, "_UMAP_final_kmeans_clusters_composed.pdf"),
       plot = UMAP_kmeans_final_composed,
       device = "pdf", # or one of "eps", "ps", "tex" (pictex), "pdf", "jpeg", "tiff", "png", "bmp", "svg" or "wmf" (windows only).
       path = path_for_figures,
       width = 8, height = 4, units = "in", dpi = 300,
       family = "Arial", bg = "transparent"
...
ggsave(filename = str_c(date, "_UMAP_final_hierarchical_clusters_composed.pdf"),
       plot = UMAP_hierarchical_final_composed,
       device = "pdf", # or one of "eps", "ps", "tex" (pictex), "pdf", "jpeg", "tiff", "png", "bmp", "svg" or "wmf" (windows only).
       path = path_for_figures,
       width = 8, height = 4, units = "in", dpi = 300,
       family = "Arial", bg = "transparent"
...
ggsave(filename = str_c(date, "_UMAP_final_sc3_clusters_composed.pdf"),
       plot = UMAP_sc3_final_composed,
       device = "pdf", # or one of "eps", "ps", "tex" (pictex), "pdf", "jpeg", "tiff", "png", "bmp", "svg" or "wmf" (windows only).
       path = path_for_figures,
       width = 8, height = 4, units = "in", dpi = 300,
       family = "Arial", bg = "transparent"
