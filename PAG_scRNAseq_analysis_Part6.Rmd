...
# STEP 6 | Clustering
We have now normalized the data, identified highly variable genes, corrected confounding factors, and applied dimensionality reduction techniques to better visualize, denoise, and summarize our data. We can now proceed to carry out analyses that are relevant to the biological questions at hand.
...
We continue using the `PAG_sceset_qc_norm_filt_corr` after normalization, filtering, and batch correction. We should thus have a `corrected` slot in `assays`, and different PCA, tSNE, and UMAP solutions in the `reducedDim` slot:
```{r}
...
path_for_figures <- "D:/Dropbox (UCL - SWC)/Project_transcriptomics/figures_in_progress/R_figures_Part6_clustering/"
date <- Sys.Date()
date <- gsub("-", "_", date)
...
PAG_sceset_qc_norm_filt_corr <- readRDS("PAG_sceset_qc_norm_filt_corr.rds") # Contains filtered cells and genes, log-normalized, filtered, and corrected data
...
An alternative approach can be performed by changing the edge weighting scheme during graph construction. Setting `type = "number"` will weight edges based on the number of nearest neighbors that are shared between two cells (in other words, the weight between two nodes is simply the number of shared nearest neighbors between them). Similarly, `type = "jaccard"` will weight edges between nodes according to the Jaccard index of the two sets of neighbors. This weight can range from zero to 1, and is a monotonic transformation of the weight used by `type = "number"`. We can also disable weighting altogether by using `buildKNNGraph()`, which is occasionally useful for downstream graph operations that do not support weights. Pipelines involving `scran` default to rank-based weights followed by Walktrap clustering. In contrast, `Seurat` uses Jaccard-based weights followed by Louvain clustering. Both of these strategies work well.
...
After running two different graph-based clustering approaches with different `k` (from 5 to 15, not shown), we observe that the predominant numbers of obtained clusters is 7 or 9 for the *var_RankBased* approach, 7, 8 or 9 for the *var_JaccardLouvain* approach, 7 or 10 for *the cv2_RankBased*, and 10 or 11 for the *cv2_JaccardLouvain*. The more common results for all of them are obtained with `k = 7:10`. We therefore embedded the clustering results of each combination of PCA, HVG, and graph clustering method in both t-SNE or UMAP plots to assess the results. After inspecting all the results, we kept the best K for each approach as below: 
```{r}
library(scran)
## Using top 25 PCs and HVGs obtained from mean-var modeling
# Using HVG from Var with rank-based weights
graph_var_rank_k8 <- buildSNNGraph(PAG_sceset_qc_norm_filt_corr,
                                   ...
                                   use.dimred = "PCA_HVG_var_logcounts_25") # Use existing PCs and ignore any setting of d, subset.row and get.spikes
clusters_var_rank_k8 <- igraph::cluster_walktrap(graph_var_rank_k8)$membership # Use `cluster_louvain` if using `type = "jaccard"`, and `cluster_walktrap` if using `type = "rank"`
PAG_sceset_qc_norm_filt_corr$SNN_clusters_var_rank_k8 <- factor(clusters_var_rank_k8)
table(PAG_sceset_qc_norm_filt_corr$SNN_clusters_var_rank_k8)

# Using HVG from Var with jaccard-based weights
graph_var_jaccard_k10 <- buildSNNGraph(PAG_sceset_qc_norm_filt_corr,
                                       ...
                                       assay.type = "logcounts",
                                       use.dimred = "PCA_HVG_var_logcounts_25") # Use existing PCs and ignore any setting of d, subset.row and get.spikes
clusters_var_jaccard_k10 <- igraph::cluster_louvain(graph_var_jaccard_k10)$membership # Use `cluster_louvain` if using `type = "jaccard"`
PAG_sceset_qc_norm_filt_corr$SNN_clusters_var_jaccard_k10 <- factor(clusters_var_jaccard_k10)
table(PAG_sceset_qc_norm_filt_corr$SNN_clusters_var_jaccard_k10)

## Using top 25 PCs and HVGs obtained from mean-cv2 modeling
...
graph_cv2_rank_k7 <- buildSNNGraph(PAG_sceset_qc_norm_filt_corr,
                                   ...
                                   assay.type = "logcounts",
                                   use.dimred = "PCA_HVG_cv2_logcounts_25") # Use existing PCs and ignore any setting of d, subset.row and get.spikes
...
graph_cv2_jaccard_k10 <- buildSNNGraph(PAG_sceset_qc_norm_filt_corr,
                                       k = 10, # An integer scalar specifying the number of nearest neighbors to consider during graph construction
                                       #d = 50, # An integer scalar specifying the number of dimensions to use for the search
                                       type = "jaccard", # A string specifying the type of weighting scheme to use for shared neighbors
                                       subset.row = metadata(PAG_sceset_qc_norm_filt_corr)$hvg_cv2_out_no_spikes_filt,
                                       assay.type = "logcounts",
                                       use.dimred = "PCA_HVG_cv2_logcounts_25") # Use existing PCs and ignore any setting of d, subset.row and get.spikes
clusters_cv2_jaccard_k10 <- igraph::cluster_louvain(graph_cv2_jaccard_k10)$membership
PAG_sceset_qc_norm_filt_corr$SNN_clusters_cv2_jaccard_k10 <- factor(clusters_cv2_jaccard_k10)
table(PAG_sceset_qc_norm_filt_corr$SNN_clusters_cv2_jaccard_k10)
```
...
```{r}
## Using t-SNE (var)
tSNE_var_PAGsubdivisions <- plotReducedDim(PAG_sceset_qc_norm_filt_corr, dimred = "tSNE_var_corrected_perplexity_15",
                                           colour_by = "PAG.arearegistration", shape_by = "cell.type"
                                           ) + labs(title = "t-SNE (var) - PAG subdivisions", x = "t-SNE 1", y = "t-SNE 2")

# Using HVG from Var with rank-based weights
tSNE_var_RankWalktrap_k8 <- plotReducedDim(PAG_sceset_qc_norm_filt_corr, dimred = "tSNE_var_corrected_perplexity_15",
                                           colour_by = "SNN_clusters_var_rank_k8", shape_by = "cell.type", text_by = "SNN_clusters_var_rank_k8"
                                           ) + labs(title = "t-SNE (var) - Rank-Walktrap (k=8)", x = "t-SNE 1", y = "t-SNE 2")

# Using HVG from Var with jaccard-based weights
tSNE_var_JaccardLouvain_k10 <- plotReducedDim(PAG_sceset_qc_norm_filt_corr, dimred = "tSNE_var_corrected_perplexity_15",
                                              colour_by = "SNN_clusters_var_jaccard_k10", shape_by = "cell.type", text_by = "SNN_clusters_var_jaccard_k10"
                                              ) + labs(title = "t-SNE (var) - Jaccard-Louvain (k=10)", x = "t-SNE 1", y = "t-SNE 2")

## Using t-SNE (cv2)
tSNE_cv2_PAGsubdivisions <- plotReducedDim(PAG_sceset_qc_norm_filt_corr, dimred = "tSNE_cv2_corrected_perplexity_15",
                                           colour_by = "PAG.arearegistration", shape_by = "cell.type"
                                           ) + labs(title = "t-SNE (cv2) - PAG subdivisions", x = "t-SNE 1", y = "t-SNE 2")

# Using HVG from CV2 with rank-based weights
tSNE_cv2_RankWalktrap_k7 <- plotReducedDim(PAG_sceset_qc_norm_filt_corr, dimred = "tSNE_cv2_corrected_perplexity_15",
                                           colour_by = "SNN_clusters_cv2_rank_k7", shape_by = "cell.type",  text_by = "SNN_clusters_cv2_rank_k7"
                                           ) + labs(title = "t-SNE (cv2) - Rank-Walktrap (k=7)", x = "t-SNE 1", y = "t-SNE 2")

# Using HVG from CV2 with jaccard-based weights
tSNE_cv2_JaccardLouvain_k8 <- plotReducedDim(PAG_sceset_qc_norm_filt_corr, dimred = "tSNE_cv2_corrected_perplexity_15",
                                             colour_by = "SNN_clusters_cv2_jaccard_k8", shape_by = "cell.type", text_by = "SNN_clusters_cv2_jaccard_k8"
                                             ) + labs(title = "t-SNE (cv2) - Jaccard-Louvain (k=8)", x = "t-SNE 1", y = "t-SNE 2")
tSNE_cv2_JaccardLouvain_k10 <- plotReducedDim(PAG_sceset_qc_norm_filt_corr, dimred = "tSNE_cv2_corrected_perplexity_15",
                                              colour_by = "SNN_clusters_cv2_jaccard_k10", shape_by = "cell.type", text_by = "SNN_clusters_cv2_jaccard_k10"
                                              ) + labs(title = "t-SNE (cv2) - Jaccard-Louvain (k=10)", x = "t-SNE 1", y = "t-SNE 2")

## Using UMAP (var)
UMAP_var_PAGsubdivisions <- plotReducedDim(PAG_sceset_qc_norm_filt_corr, dimred = "UMAP_var_corrected_10_neighbors_0.01_min_dist",
                                           colour_by = "PAG.arearegistration", shape_by = "cell.type"
                                           ) + labs(title = "UMAP (var) - PAG subdivisions", x = "UMAP 1", y = "UMAP 2")

# Using HVG from Var with rank-based weights
UMAP_var_RankWalktrap_k8 <- plotReducedDim(PAG_sceset_qc_norm_filt_corr, dimred = "UMAP_var_corrected_10_neighbors_0.01_min_dist",
                                           colour_by = "SNN_clusters_var_rank_k8", shape_by = "cell.type", text_by = "SNN_clusters_var_rank_k8"
                                           ) + labs(title = "UMAP (var) - Rank-Walktrap (k=8)", x = "UMAP 1", y = "UMAP 2")

# Using HVG from Var with jaccard-based weights
UMAP_var_JaccardLouvain_k10 <- plotReducedDim(PAG_sceset_qc_norm_filt_corr, dimred = "UMAP_var_corrected_10_neighbors_0.01_min_dist",
                                              colour_by = "SNN_clusters_var_jaccard_k10", shape_by = "cell.type", text_by = "SNN_clusters_var_jaccard_k10"
                                              ) + labs(title = "UMAP (var) - Jaccard-Louvain (k=10)", x = "UMAP 1", y = "UMAP 2")

## Using UMAP (cv2) - 10_neighbors_0.05_min_dist
# Use "UMAP_cv2_corrected_10_neighbors_0.05_min_dist" or "UMAP_cv2_corrected_15_neighbors_0.01_min_dist"
UMAP_cv2_10_005_PAGsubdivisions <- plotReducedDim(PAG_sceset_qc_norm_filt_corr, dimred = "UMAP_cv2_corrected_10_neighbors_0.05_min_dist",
                                                  colour_by = "PAG.arearegistration", shape_by = "cell.type"
                                                  ) + labs(title = "UMAP (cv2) - PAG subdivisions", x = "UMAP 1", y = "UMAP 2")

# Using HVG from CV2 with rank-based weights
UMAP_cv2_10_005_RankWalktrap_k7 <- plotReducedDim(PAG_sceset_qc_norm_filt_corr, dimred = "UMAP_cv2_corrected_10_neighbors_0.05_min_dist",
                                                  colour_by = "SNN_clusters_cv2_rank_k7", shape_by = "cell.type", text_by = "SNN_clusters_cv2_rank_k7"
                                                  ) + labs(title = "UMAP (cv2) - Rank-Walktrap (k=7)", x = "UMAP 1", y = "UMAP 2")

# Using HVG from CV2 with jaccard-based weights
UMAP_cv2_10_005_JaccardLouvain_k8 <- plotReducedDim(PAG_sceset_qc_norm_filt_corr, dimred = "UMAP_cv2_corrected_10_neighbors_0.05_min_dist",
                                                    colour_by = "SNN_clusters_cv2_jaccard_k8", shape_by = "cell.type", text_by = "SNN_clusters_cv2_jaccard_k8"
                                                    ) + labs(title = "UMAP (cv2) - Jaccard-Louvain (k=8)", x = "UMAP 1", y = "UMAP 2")
UMAP_cv2_10_005_JaccardLouvain_k10 <- plotReducedDim(PAG_sceset_qc_norm_filt_corr, dimred = "UMAP_cv2_corrected_10_neighbors_0.05_min_dist",
                                                     colour_by = "SNN_clusters_cv2_jaccard_k10", shape_by = "cell.type", text_by = "SNN_clusters_cv2_jaccard_k10"
                                                     ) + labs(title = "UMAP (cv2) - Jaccard-Louvain (k=10)", x = "UMAP 1", y = "UMAP 2")

## Using UMAP (cv2) - 15_neighbors_0.01_min_dist
# Use "UMAP_cv2_corrected_10_neighbors_0.05_min_dist" or "UMAP_cv2_corrected_15_neighbors_0.01_min_dist"
UMAP_cv2_15_001_PAGsubdivisions <- plotReducedDim(PAG_sceset_qc_norm_filt_corr, dimred = "UMAP_cv2_corrected_15_neighbors_0.01_min_dist",
                                                  colour_by = "PAG.arearegistration", shape_by = "cell.type"
                                                  ) + labs(title = "UMAP (cv2) - PAG subdivisions", x = "UMAP 1", y = "UMAP 2")

# Using HVG from CV2 with rank-based weights
UMAP_cv2_15_001_RankWalktrap_k7 <- plotReducedDim(PAG_sceset_qc_norm_filt_corr, dimred = "UMAP_cv2_corrected_15_neighbors_0.01_min_dist",
                                                  colour_by = "SNN_clusters_cv2_rank_k7", shape_by = "cell.type", text_by = "SNN_clusters_cv2_rank_k7"
                                                  ) + labs(title = "UMAP (cv2) - Rank-Walktrap (k=7)", x = "UMAP 1", y = "UMAP 2")

# Using HVG from CV2 with jaccard-based weights
UMAP_cv2_15_001_JaccardLouvain_k8 <- plotReducedDim(PAG_sceset_qc_norm_filt_corr, dimred = "UMAP_cv2_corrected_15_neighbors_0.01_min_dist",
                                                    colour_by = "SNN_clusters_cv2_jaccard_k8", shape_by = "cell.type", text_by = "SNN_clusters_cv2_jaccard_k8"
                                                    ) + labs(title = "UMAP (cv2) - Jaccard-Louvain (k=8)", x = "UMAP 1", y = "UMAP 2")

UMAP_cv2_15_001_JaccardLouvain_k10 <- plotReducedDim(PAG_sceset_qc_norm_filt_corr, dimred = "UMAP_cv2_corrected_15_neighbors_0.01_min_dist",
                                                     colour_by = "SNN_clusters_cv2_jaccard_k10", shape_by = "cell.type", text_by = "SNN_clusters_cv2_jaccard_k10"
                                                     ) + labs(title = "UMAP (cv2) - Jaccard-Louvain (k=10)", x = "UMAP 1", y = "UMAP 2")

# Prepare and save composed plots
library(patchwork)
tSNE_var_SNN_clusters <- (tSNE_var_PAGsubdivisions + tSNE_var_RankWalktrap_k8 + tSNE_var_PAGsubdivisions + tSNE_var_JaccardLouvain_k10) +
  plot_annotation(title = "Graph-based clustering on tSNE (var)", tag_levels = "A", 
                  theme = theme(plot.title = element_text(size = 18), plot.tag = element_text(size = 14))) + plot_layout(ncol = 2)

tSNE_cv2_SNN_clusters <- (tSNE_cv2_PAGsubdivisions + tSNE_cv2_RankWalktrap_k7 + tSNE_cv2_JaccardLouvain_k8 + tSNE_cv2_JaccardLouvain_k10) +
  plot_annotation(title = "Graph-based clustering on tSNE (cv2)", tag_levels = "A", 
                  theme = theme(plot.title = element_text(size = 18), plot.tag = element_text(size = 14))) + plot_layout(ncol = 2)

UMAP_var_SNN_clusters <- (UMAP_var_PAGsubdivisions + UMAP_var_RankWalktrap_k8 + UMAP_var_PAGsubdivisions + UMAP_var_JaccardLouvain_k10) +
  plot_annotation(title = "Graph-based clustering on UMAP (var)", tag_levels = "A", 
                  theme = theme(plot.title = element_text(size = 18), plot.tag = element_text(size = 14))) + plot_layout(ncol = 2)
...
# Plot them
tSNE_var_SNN_clusters
tSNE_cv2_SNN_clusters
UMAP_var_SNN_clusters
UMAP_cv2_10_005_SNN_clusters
UMAP_cv2_15_001_SNN_clusters

# Save them
ggsave(filename = str_c(date, "_SNN_clusters_tSNE_var_composed.pdf"),
...
ggsave(filename = str_c(date, "_SNN_clusters_tSNE_cv2_composed.pdf"),
...
ggsave(filename = str_c(date, "_SNN_clusters_UMAP_var_composed.pdf"),
...
ggsave(filename = str_c(date, "_SNN_clusters_UMAP_cv2_10_005_composed.pdf"),
       plot = UMAP_cv2_10_005_SNN_clusters,
       device = "pdf", # or one of "eps", "ps", "tex" (pictex), "pdf", "jpeg", "tiff", "png", "bmp", "svg" or "wmf" (windows only).
       path = path_for_figures,
       width = 12, height = 9, units = "in", dpi = 300,
       family = "Helvetica", bg = "transparent"
...
ggsave(filename = str_c(date, "_SNN_clusters_UMAP_cv2_15_001_composed.pdf"),
...
After visualizing the clustering results embedded in our reduced dimensions we can clearly confirm two observations from the previous steps: (1) the UMAP visualizations are significantly better than the t-SNE plots for our data, and (2) using the PCs obtained from HVGs after modeling the CV2-mean trend provide more resolution than those from modeling Var-mean trend for both clustering and dimensionality reduction.
...
```{r}
ratio_var_rank_k8 <- clusterModularity(graph_var_rank_k8, clusters_var_rank_k8, as.ratio = TRUE)
ratio_var_rank_k8

ratio_var_jaccard_k10 <- clusterModularity(graph_var_jaccard_k10, clusters_var_jaccard_k10, as.ratio = TRUE)
ratio_var_jaccard_k10
...
cluster_modularity_var_jaccard_k10 <- pheatmap(log2(ratio_var_jaccard_k10 + 1), main = "Cluster Modularity (var) - Jaccard Louvain (k=10)",
                                               cluster_cols = FALSE, cluster_rows = FALSE, angle_col = 0, 
                                               color = rev(viridis::magma(100)), # color = colorRampPalette(c("white", "blue"))(100) or color = rev(viridis::magma(100))
                                               legend_breaks = c(0, 1, 2, 3, 4, 5, 6), breaks = seq(0, 6, length = 100))

cluster_modularity_cv2_rank_k7 <- pheatmap(log2(ratio_cv2_rank_k7 + 1), main = "Cluster Modularity (cv2) - Rank Walktrap (k=7)",
                                           cluster_cols = FALSE, cluster_rows = FALSE, angle_col = 0, 
                                           color = rev(viridis::magma(100)), # color = colorRampPalette(c("white", "blue"))(100) or color = rev(viridis::magma(100))
                                           legend_breaks = c(0, 1, 2, 3, 4, 5, 6), breaks = seq(0, 6, length = 100))

cluster_modularity_cv2_jaccard_k8 <- pheatmap(log2(ratio_cv2_jaccard_k8 + 1), main = "Cluster Modularity (cv2) - Jaccard Louvain (k=8)",
                                              cluster_cols = FALSE, cluster_rows = FALSE, angle_col = 0,
                                              color = rev(viridis::magma(100)), # color = colorRampPalette(c("white", "blue"))(100) or color = rev(viridis::magma(100))
                                              legend_breaks = c(0, 1, 2, 3, 4, 5, 6), breaks = seq(0, 6, length = 100))

cluster_modularity_cv2_jaccard_k10 <- pheatmap(log2(ratio_cv2_jaccard_k10 + 1), main = "Cluster Modularity (cv2) - Jaccard Louvain (k=10)",
                                               cluster_cols = FALSE, cluster_rows = FALSE, angle_col = 0,
                                               color = rev(viridis::magma(100)), # color = colorRampPalette(c("white", "blue"))(100) or color = rev(viridis::magma(100))
                                               legend_breaks = c(0, 1, 2, 3, 4, 5, 6), breaks = seq(0, 6, length = 100))

# Save them
ggsave(filename = str_c(date, "_SNN_cluster_modularity_var_rank_k8.pdf"),
       plot = cluster_modularity_var_rank_k8,
       device = "pdf", # or one of "eps", "ps", "tex" (pictex), "pdf", "jpeg", "tiff", "png", "bmp", "svg" or "wmf" (windows only).
       path = path_for_figures,
       width = 12, height = 9, units = "in", dpi = 300,
       family = "Helvetica", bg = "transparent"
...
ggsave(filename = str_c(date, "_cluster_modularity_cv2_jaccard_k8.pdf"),
...
ggsave(filename = str_c(date, "_cluster_modularity_cv2_jaccard_k10.pdf"),
       plot = cluster_modularity_cv2_jaccard_k10,
       device = "pdf", # or one of "eps", "ps", "tex" (pictex), "pdf", "jpeg", "tiff", "png", "bmp", "svg" or "wmf" (windows only).
       path = path_for_figures,
       width = 12, height = 9, units = "in", dpi = 300,
       family = "Helvetica", bg = "transparent"
...
These heatmaps are most useful when examined side by side with the clustering results embedded in the UMAP plots, as we can then assess the relationship between nearby and far apart clusters. For example, in _"Cluster Modularity (k=8) - Jaccard Louvain (cv2)"_ we can see that cluster 11 has the highest score and doesn't relate to any other, which makes sense as it is the one with macrophage markers. However, clusters 7 and 8 have relatively low diagonal scores and are related to clusters 2, 3, and 4, which makes sense as these are all VGluT2 cells sitting close to each other.
...
```{r}
# Write functions for var approaches
myClusterFUN_var_rank <- function(x, chosen_k) {
  g <- buildSNNGraph(x, k = chosen_k, type = "rank",
                     subset.row = metadata(PAG_sceset_qc_norm_filt_corr)$hvg_var_out_no_spikes_block_filt,
                     assay.type = "logcounts", use.dimred = "PCA_HVG_var_logcounts_25")
  igraph::cluster_walktrap(g)$membership
}

myClusterFUN_var_jaccard <- function(x, chosen_k) {
...
myClusterFUN_cv2_rank <- function(x, chosen_k) {
  g <- buildSNNGraph(x, k = chosen_k, type = "rank",
                     subset.row = metadata(PAG_sceset_qc_norm_filt_corr)$hvg_cv2_out_no_spikes_filt,
                     assay.type = "logcounts", use.dimred = "PCA_HVG_cv2_logcounts_25")
  igraph::cluster_walktrap(g)$membership
}
...
set.seed(1714)
start_time <- Sys.time() # Takes 2 min with 50 iterations, and 35min with a 1000 iterations
coassign_var_rank_k8 <- suppressWarnings(bootstrapCluster(PAG_sceset_qc_norm_filt_corr,
                                                          FUN = myClusterFUN_var_rank,
                                                          clusters = clusters_var_rank_k8,
                                                          iterations = 1000, # default is 20
                                                          chosen_k = 8))
dim(coassign_var_rank_k8)

coassign_var_jaccard_k10 <- suppressWarnings(bootstrapCluster(PAG_sceset_qc_norm_filt_corr,
                                                              FUN = myClusterFUN_var_jaccard,
                                                              clusters = clusters_var_jaccard_k10,
                                                              iterations = 1000, # default is 20
                                                              chosen_k = 10))
dim(coassign_var_jaccard_k10)
...
coassign_cv2_jaccard_k10 <- suppressWarnings(bootstrapCluster(PAG_sceset_qc_norm_filt_corr,
                                                              FUN = myClusterFUN_cv2_jaccard,
                                                              clusters = clusters_cv2_jaccard_k10,
                                                              iterations = 1000, # default is 20
                                                              chosen_k = 10))
dim(coassign_cv2_jaccard_k10)
end_time <- Sys.time()
end_time - start_time
```
...
cluster_stability_var_jaccard_k10 <- pheatmap(coassign_var_jaccard_k10, main = "Cluster Stability (var) - Jaccard Louvain (k=10)",
                                              cluster_rows = FALSE, cluster_cols = FALSE, angle_col = 0,
                                              color = rev(viridis::magma(100)), # colorRampPalette(c("white", "blue"))(100)
                                              legend_breaks = c(0, 0.2, 0.4, 0.6, 0.8, 1), breaks = seq(0, 1, length = 100)) 

cluster_stability_cv2_rank_k7 <- pheatmap(coassign_cv2_rank_k7, main = "Cluster Stability (cv2) - Rank Walktrap (k=7)",
                                          cluster_rows = FALSE, cluster_cols = FALSE, angle_col = 0,
                                          color = rev(viridis::magma(100)), # colorRampPalette(c("white", "blue"))(100)
                                          legend_breaks = c(0, 0.2, 0.4, 0.6, 0.8, 1), breaks = seq(0, 1, length = 100))

cluster_stability_cv2_jaccard_k8 <- pheatmap(coassign_cv2_jaccard_k8, main = "Cluster Stability (cv2) - Jaccard Louvain (k=8)",
                                             cluster_rows = FALSE, cluster_cols = FALSE, angle_col = 0,
                                             color = rev(viridis::magma(100)), # colorRampPalette(c("white", "blue"))(100)
                                             legend_breaks = c(0, 0.2, 0.4, 0.6, 0.8, 1), breaks = seq(0, 1, length = 100))

cluster_stability_cv2_jaccard_k10 <- pheatmap(coassign_cv2_jaccard_k10, main = "Cluster Stability (cv2) - Jaccard Louvain (k=10)",
                                              cluster_rows = FALSE, cluster_cols = FALSE, angle_col = 0,
                                              color = rev(viridis::magma(100)), # colorRampPalette(c("white", "blue"))(100)
                                              legend_breaks = c(0, 0.2, 0.4, 0.6, 0.8, 1), breaks = seq(0, 1, length = 100))

# Save them
ggsave(filename = str_c(date, "_SNN_cluster_stability_var_rank_k8.pdf"),
       plot = cluster_stability_var_rank_k8,
       device = "pdf", # or one of "eps", "ps", "tex" (pictex), "pdf", "jpeg", "tiff", "png", "bmp", "svg" or "wmf" (windows only).
       path = path_for_figures,
       width = 12, height = 9, units = "in", dpi = 300,
       family = "Helvetica", bg = "transparent"
       )
ggsave(filename = str_c(date, "_SNN_cluster_stability_var_jaccard_k10.pdf"),
       plot = cluster_stability_var_jaccard_k10,
       device = "pdf", # or one of "eps", "ps", "tex" (pictex), "pdf", "jpeg", "tiff", "png", "bmp", "svg" or "wmf" (windows only).
       path = path_for_figures,
       width = 12, height = 9, units = "in", dpi = 300,
       family = "Helvetica", bg = "transparent"
...
ggsave(filename = str_c(date, "_SNN_cluster_stability_cv2_jaccard_k10.pdf"),
       plot = cluster_stability_cv2_jaccard_k10,
       device = "pdf", # or one of "eps", "ps", "tex" (pictex), "pdf", "jpeg", "tiff", "png", "bmp", "svg" or "wmf" (windows only).
       path = path_for_figures,
       width = 12, height = 9, units = "in", dpi = 300,
       family = "Helvetica", bg = "transparent"
...
```{r}
summary(PAG_sceset_qc_norm_filt_corr$SNN_clusters_var_rank_k8)
summary(PAG_sceset_qc_norm_filt_corr$SNN_clusters_var_jaccard_k10)
...
summary(PAG_sceset_qc_norm_filt_corr$SNN_clusters_cv2_jaccard_k10)
```
...
gaps_var <- clusGap(reducedDim(PAG_sceset_qc_norm_filt_corr, "PCA_HVG_var_logcounts_25"), kmeans, K.max = 20)
...
gaps_cv2 <- clusGap(reducedDim(PAG_sceset_qc_norm_filt_corr, "PCA_HVG_cv2_logcounts_25"), kmeans, K.max = 20)
...
It is a good idea to run this with different seeds to verify that the results are stable. For the CV2 scenario, we can see that the suggested `k = 1` seems to be an artifact, and a value around `k = 10` or `k = 13` would perhaps be a better choice.
...
cluster_kmeans_var <- kmeans(reducedDim(PAG_sceset_qc_norm_filt_corr, "PCA_HVG_var_logcounts_25"), centers = 8, iter.max = 50, nstart = 5)
PAG_sceset_qc_norm_filt_corr$kmeans_clusters_var <- factor(cluster_kmeans_var$cluster)

cluster_kmeans_cv2 <- kmeans(reducedDim(PAG_sceset_qc_norm_filt_corr, "PCA_HVG_cv2_logcounts_25"), centers = 10, iter.max = 50, nstart = 5)
...
```{r}
# t-SNE using PCA from HVG_var_logcounts
tSNE_var_kmeans <- plotReducedDim(PAG_sceset_qc_norm_filt_corr, dimred = "tSNE_var_corrected_perplexity_15",
                                  colour_by = "kmeans_clusters_var", shape_by = "cell.type", text_by = "kmeans_clusters_var"
                                  ) + labs(title = "tSNE (var) - K-means (centers=8)", x = "t-SNE 1", y = "t-SNE 2")

# t-SNE using PCA from HVG_cv2_logcounts
tSNE_cv2_kmeans <- plotReducedDim(PAG_sceset_qc_norm_filt_corr, dimred = "tSNE_cv2_corrected_perplexity_15",
                                  colour_by = "kmeans_clusters_cv2", shape_by = "cell.type", text_by = "kmeans_clusters_cv2"
                                  ) + labs(title = "tSNE (cv2) - K-means (centers=10)", x = "t-SNE 1", y = "t-SNE 2")

# UMAP using PCA from HVG_var_logcounts
UMAP_var_kmeans <- plotReducedDim(PAG_sceset_qc_norm_filt_corr, "UMAP_var_corrected_10_neighbors_0.01_min_dist", 
                                  colour_by = "kmeans_clusters_var", shape_by = "cell.type", text_by = "kmeans_clusters_var"
                                  ) + labs(title = "UMAP (var) - K-means (centers=8)", x = "UMAP 1", y = "UMAP 2")

# UMAP using PCA from HVG_cv2_logcounts
UMAP_cv2_10_005_kmeans <- plotReducedDim(PAG_sceset_qc_norm_filt_corr, "UMAP_cv2_corrected_10_neighbors_0.05_min_dist",
                                         colour_by = "kmeans_clusters_cv2", shape_by = "cell.type", text_by = "kmeans_clusters_cv2"
                                         )  + labs(title = "UMAP (cv2) - K-means (centers=10)", x = "UMAP 1", y = "UMAP 2")

UMAP_cv2_15_001_kmeans <- plotReducedDim(PAG_sceset_qc_norm_filt_corr, "UMAP_cv2_corrected_15_neighbors_0.01_min_dist",
                                         colour_by = "kmeans_clusters_cv2", shape_by = "cell.type", text_by = "kmeans_clusters_cv2"
                                         )  + labs(title = "UMAP (cv2) - K-means (centers=10)", x = "UMAP 1", y = "UMAP 2")

# Prepare composed plots
library(patchwork)
tSNE_kmeans_clusters <- (tSNE_var_PAGsubdivisions + tSNE_var_kmeans +
                           tSNE_cv2_PAGsubdivisions + tSNE_cv2_kmeans) +
  plot_annotation(title = "K-means clustering on tSNE", tag_levels = "A", 
                  theme = theme(plot.title = element_text(size = 18), plot.tag = element_text(size = 14))) + plot_layout(ncol = 2)

UMAP_kmeans_clusters <- (UMAP_var_PAGsubdivisions + UMAP_var_kmeans +
                           UMAP_cv2_10_005_PAGsubdivisions + UMAP_cv2_10_005_kmeans +
                           UMAP_cv2_15_001_PAGsubdivisions + UMAP_cv2_15_001_kmeans) +
...
# Save them
ggsave(filename = str_c(date, "_kmeans_tSNE_clusters_composed.pdf"),
       plot = tSNE_kmeans_clusters,
       device = "pdf", # or one of "eps", "ps", "tex" (pictex), "pdf", "jpeg", "tiff", "png", "bmp", "svg" or "wmf" (windows only).
       path = path_for_figures,
       width = 12, height = 9, units = "in", dpi = 300,
       family = "Helvetica", bg = "transparent"
...
overcluster_kmeans_var <- kmeans(reducedDim(PAG_sceset_qc_norm_filt_corr, "PCA_HVG_var_logcounts_25"), centers = 20, iter.max = 50, nstart = 5)
PAG_sceset_qc_norm_filt_corr$kmeans_overcluster_var <- factor(overcluster_kmeans_var$cluster)

overcluster_kmeans_cv2 <- kmeans(reducedDim(PAG_sceset_qc_norm_filt_corr, "PCA_HVG_cv2_logcounts_25"), centers = 20, iter.max = 50, nstart = 5)
...
```{r}
# t-SNE using PCA from HVG_var_logcounts
tSNE_var_kmeans_overcluster <- plotReducedDim(PAG_sceset_qc_norm_filt_corr, dimred = "tSNE_var_corrected_perplexity_15",
                                              colour_by = "kmeans_overcluster_var", shape_by = "cell.type", text_by = "kmeans_overcluster_var"
                                              ) + labs(title = "tSNE (var) - K-means (centers=20)", x = "t-SNE 1", y = "t-SNE 2")

# t-SNE using PCA from HVG_cv2_logcounts
tSNE_cv2_kmeans_overcluster <- plotReducedDim(PAG_sceset_qc_norm_filt_corr, dimred = "tSNE_cv2_corrected_perplexity_15",
                                              colour_by = "kmeans_overcluster_cv2", shape_by = "cell.type", text_by = "kmeans_overcluster_cv2"
                                              ) + labs(title = "tSNE (cv2) - K-means (centers=20)", x = "t-SNE 1", y = "t-SNE 2")

# UMAP using PCA from HVG_var_logcounts
UMAP_var_kmeans_overcluster <- plotReducedDim(PAG_sceset_qc_norm_filt_corr, "UMAP_var_corrected_10_neighbors_0.01_min_dist",
                                              colour_by = "kmeans_overcluster_var", shape_by = "cell.type", text_by = "kmeans_overcluster_var"
                                              ) + labs(title = "UMAP (var) - K-means (centers=20)", x = "UMAP 1", y = "UMAP 2")

# UMAP using PCA from HVG_cv2_logcounts
UMAP_cv2_10_005_kmeans_overcluster <- plotReducedDim(PAG_sceset_qc_norm_filt_corr, "UMAP_cv2_corrected_10_neighbors_0.05_min_dist",
                                                     colour_by = "kmeans_overcluster_cv2", shape_by = "cell.type", text_by = "kmeans_overcluster_cv2"
                                                     )  + labs(title = "UMAP (cv2) - K-means (centers=20)", x = "UMAP 1", y = "UMAP 2")

UMAP_cv2_15_001_kmeans_overcluster <- plotReducedDim(PAG_sceset_qc_norm_filt_corr, "UMAP_cv2_corrected_15_neighbors_0.01_min_dist",
                                                    colour_by = "kmeans_overcluster_cv2", shape_by = "cell.type", text_by = "kmeans_overcluster_cv2"
                                                    )  + labs(title = "UMAP (cv2) - K-means (centers=20)", x = "UMAP 1", y = "UMAP 2")

# Prepare composed plots
library(patchwork)
tSNE_kmeans_overclusters <- (tSNE_var_PAGsubdivisions + tSNE_var_kmeans_overcluster +
                               tSNE_cv2_PAGsubdivisions + tSNE_cv2_kmeans_overcluster) +
  plot_annotation(title = "K-means overclustering on tSNE", tag_levels = "A", 
                  theme = theme(plot.title = element_text(size = 18), plot.tag = element_text(size = 14))) + plot_layout(ncol = 2)

UMAP_kmeans_overclusters <- (UMAP_var_PAGsubdivisions + UMAP_var_kmeans_overcluster +
                               UMAP_cv2_10_005_PAGsubdivisions + UMAP_cv2_10_005_kmeans_overcluster +
                               UMAP_cv2_15_001_PAGsubdivisions + UMAP_cv2_15_001_kmeans_overcluster) +
  plot_annotation(title = "K-means overclustering on UMAP", tag_levels = "A", 
                  theme = theme(plot.title = element_text(size = 18), plot.tag = element_text(size = 14))) + plot_layout(ncol = 2)

# Plot them
tSNE_kmeans_overclusters
UMAP_kmeans_overclusters

# Save them
ggsave(filename = str_c(date, "_kmeans_tSNE_overclusters_composed.pdf"),
       plot = tSNE_kmeans_overclusters,
       device = "pdf", # or one of "eps", "ps", "tex" (pictex), "pdf", "jpeg", "tiff", "png", "bmp", "svg" or "wmf" (windows only).
       path = path_for_figures,
       width = 12, height = 9, units = "in", dpi = 300,
       family = "Helvetica", bg = "transparent"
...
library(patchwork)
UMAP_cv2_10_005_kmeans_composed <- (UMAP_cv2_10_005_PAGsubdivisions + UMAP_cv2_10_005_kmeans +
                                      UMAP_cv2_10_005_PAGsubdivisions + UMAP_cv2_10_005_kmeans_overcluster) +
  plot_annotation(title = "K-means clusters on UMAP", tag_levels = "A", 
                  theme = theme(plot.title = element_text(size = 18), plot.tag = element_text(size = 14))) + plot_layout(ncol = 2)
...
ggsave(filename = str_c(date, "_kmeans_clusters_UMAP_cv2_10_005_composed.pdf"),
       plot = UMAP_cv2_10_005_kmeans_composed,
       device = "pdf", # or one of "eps", "ps", "tex" (pictex), "pdf", "jpeg", "tiff", "png", "bmp", "svg" or "wmf" (windows only).
       path = path_for_figures,
       width = 12, height = 9, units = "in", dpi = 300,
       family = "Helvetica", bg = "transparent"
...
As we can see when looking at both the tree and the embedding of the k-means clusters on the UMAP, using PCs from mean-var modeling doesn't generate a tree that reflects the origin of the cells (i.e. excitatory or inhibitory cells), whereas we do see that in the results obtained with PCs from mean-cv2 modeling. Overall we seem to obtain similar clusters to those we obtained from the graph-clustering approach, at least for the cv2 scenario.
...
# Create palette for PAGarea_celltype:
colours_PAGarea_celltype <- c(`dmpag_VGAT` = "red", `dlpag_VGAT` = "coral2", `lpag_VGAT` = "salmon", `vlpag_VGAT` = "darksalmon",
                              `dmpag_VGluT2` = "cornflowerblue", `dlpag_VGluT2` = "cyan3", `lpag_VGluT2` = "aquamarine3", `vlpag_VGluT2` = "cadetblue3")

## Using PCA from HVG_var_logcounts
# Making a distance matrix
PAG_dist_var <- dist(reducedDim(PAG_sceset_qc_norm_filt_corr, "PCA_HVG_var_logcounts_25"), 
...
labels_colors(PAG_dend_var) <- colours_PAGarea_celltype[combined_fac_var][order.dendrogram(PAG_dend_var)]
plot(PAG_dend_var, main = "Dendrogram from hierarchical clustering (var)", ylab = "Height", las = 1)
legend("topright", col = colours_PAGarea_celltype, pch = 19, cex = 0.75,  bty = "n",
       legend = levels(PAG_sceset_qc_norm_filt_corr$PAGarea_celltype))


## Using PCA from HVG_cv2_logcounts
# Making a distance matrix
PAG_dist_cv2 <- dist(reducedDim(PAG_sceset_qc_norm_filt_corr, "PCA_HVG_cv2_logcounts_25"), 
                     method = "euclidean" # Usual distance between the two vectors (2 norm aka L_2), sqrt(sum((x_i - y_i)^2)).
                     ) # Alternative distance, "manhattan" -> Absolute distance between the two vectors (1 norm aka L_1).
# Building a tree
...
labels_colors(PAG_dend_cv2) <- colours_PAGarea_celltype[combined_fac_cv2][order.dendrogram(PAG_dend_cv2)]
plot(PAG_dend_cv2, main = "Dendrogram from hierarchical clustering (cv2)", ylab = "Height", las = 1)
legend("topright", col = colours_PAGarea_celltype, pch = 19, cex = 0.75,  bty = "n",
       legend = levels(PAG_sceset_qc_norm_filt_corr$PAGarea_celltype))

## Save plots
pdf(file = str_c(path_for_figures, date, "_hierarchical_dendrogram_var.pdf"),
    width = 12, height = 9,
    family = "Helvetica", bg = "transparent")
labels_colors(PAG_dend_var) <- colours_PAGarea_celltype[combined_fac_var][order.dendrogram(PAG_dend_var)]
plot(PAG_dend_var, main = "Dendrogram from hierarchical clustering (var)", ylab = "Height", las = 1)
...
pdf(file = str_c(path_for_figures, date, "_hierarchical_dendrogram_cv2.pdf"),
    width = 12, height = 9,
    family = "Helvetica", bg = "transparent")
labels_colors(PAG_dend_cv2) <- colours_PAGarea_celltype[combined_fac_var][order.dendrogram(PAG_dend_cv2)]
plot(PAG_dend_cv2, main = "Dendrogram from hierarchical clustering (cv2)", ylab = "Height", las = 1)
legend("topright", col = colours_PAGarea_celltype, pch = 19, cex = 0.75,  bty = "n",
       legend = levels(PAG_sceset_qc_norm_filt_corr$PAGarea_celltype))
dev.off()
...
PAG_clust_cv2_cutree_k <- cutree(PAG_tree_cv2, # Use either "k" or "h"
                                 ...
                                 #h = 235) # height where the tree should be cut
...
plot(PAG_dend_cv2, main = "Dendrogram from hierarchical clustering (cv2)", sub = "Tree Cut using h=235", ylab = "Height", las = 1)
abline(h = 235, col = "red")
legend("topright", col = cluster_colours_10, pch = 19, cex = 0.75,  bty = "n",
...
```{r}
# UMAP using PCA from HVG_var_logcounts
UMAP_var_cutreedynamic <- plotReducedDim(PAG_sceset_qc_norm_filt_corr, "UMAP_var_corrected_10_neighbors_0.01_min_dist",
...
UMAP_var_cutree_k <- plotReducedDim(PAG_sceset_qc_norm_filt_corr, "UMAP_var_corrected_10_neighbors_0.01_min_dist",
...
UMAP_var_cutree_h <- plotReducedDim(PAG_sceset_qc_norm_filt_corr, "UMAP_var_corrected_10_neighbors_0.01_min_dist",
                                    colour_by = "cutree_clusters_var_h", shape_by = "cell.type", text_by = "cutree_clusters_var_h"
                                    ) + labs(title = "UMAP (var) - Tree Cut (h=290)", x = "UMAP 1", y = "UMAP 2")

# UMAP_10_005 using PCA from HVG_cv2_logcounts
UMAP_cv2_10_005_cutreedynamic <- plotReducedDim(PAG_sceset_qc_norm_filt_corr, "UMAP_cv2_corrected_10_neighbors_0.05_min_dist",
                                                colour_by = "cutreedynamic_clusters_cv2", shape_by = "cell.type", text_by = "cutreedynamic_clusters_cv2"
                                                )  + labs(title = "UMAP (cv2) - Dynamic Tree Cut", x = "UMAP 1", y = "UMAP 2")

UMAP_cv2_10_005_cutree_k <- plotReducedDim(PAG_sceset_qc_norm_filt_corr, "UMAP_cv2_corrected_10_neighbors_0.05_min_dist",
...
UMAP_cv2_10_005_cutree_h <- plotReducedDim(PAG_sceset_qc_norm_filt_corr, "UMAP_cv2_corrected_10_neighbors_0.05_min_dist",
                                           colour_by = "cutree_clusters_cv2_h", shape_by = "cell.type", text_by = "cutree_clusters_cv2_h"
                                           )  + labs(title = "UMAP (cv2) - Tree Cut (h=235)", x = "UMAP 1", y = "UMAP 2")

# UMAP_15_001 using PCA from HVG_cv2_logcounts
UMAP_cv2_15_001_cutreedynamic <- plotReducedDim(PAG_sceset_qc_norm_filt_corr, "UMAP_cv2_corrected_15_neighbors_0.01_min_dist",
                                                colour_by = "cutreedynamic_clusters_cv2", shape_by = "cell.type", text_by = "cutreedynamic_clusters_cv2"
                                                )  + labs(title = "UMAP (cv2) -  Dynamic Tree Cut", x = "UMAP 1", y = "UMAP 2")

UMAP_cv2_15_001_cutree_k <- plotReducedDim(PAG_sceset_qc_norm_filt_corr, "UMAP_cv2_corrected_15_neighbors_0.01_min_dist",
                                           colour_by = "cutree_clusters_cv2_k", shape_by = "cell.type", text_by = "cutree_clusters_cv2_k"
                                           )  + labs(title = "UMAP (cv2) - Tree Cut (k=10)", x = "UMAP 1", y = "UMAP 2")

UMAP_cv2_15_001_cutree_h <- plotReducedDim(PAG_sceset_qc_norm_filt_corr, "UMAP_cv2_corrected_15_neighbors_0.01_min_dist",
                                           colour_by = "cutree_clusters_cv2_h", shape_by = "cell.type", text_by = "cutree_clusters_cv2_h"
                                           )  + labs(title = "UMAP (cv2) - Tree Cut (h=235)", x = "UMAP 1", y = "UMAP 2")

# Prepare composed plots
library(patchwork)
UMAP_var_hierarchical_clusters <- (UMAP_var_PAGsubdivisions + UMAP_var_cutreedynamic + UMAP_var_cutree_k + UMAP_var_cutree_h) +
  plot_annotation(title = "Hierarchical clustering on UMAP (var)", tag_levels = "A", 
                  theme = theme(plot.title = element_text(size = 18), plot.tag = element_text(size = 14))) + plot_layout(ncol = 2)

UMAP_cv2_10_005_hierarchical_clusters <- (UMAP_cv2_10_005_PAGsubdivisions + UMAP_cv2_10_005_cutreedynamic + UMAP_cv2_10_005_cutree_k + UMAP_cv2_10_005_cutree_h) +
  plot_annotation(title = "Hierarchical clustering on UMAP (cv2)", tag_levels = "A", 
                  theme = theme(plot.title = element_text(size = 18), plot.tag = element_text(size = 14))) + plot_layout(ncol = 2)

UMAP_cv2_15_001_hierarchical_clusters <- (UMAP_cv2_15_001_PAGsubdivisions + UMAP_cv2_15_001_cutreedynamic + UMAP_cv2_15_001_cutree_k + UMAP_cv2_15_001_cutree_h) +
  plot_annotation(title = "Hierarchical clustering on UMAP (cv2)", tag_levels = "A", 
                  theme = theme(plot.title = element_text(size = 18), plot.tag = element_text(size = 14))) + plot_layout(ncol = 2)
...
UMAP_var_hierarchical_clusters
UMAP_cv2_10_005_hierarchical_clusters
UMAP_cv2_15_001_hierarchical_clusters

# Save them
ggsave(filename = str_c(date, "_hierarchical_UMAP_var_clusters_composed.pdf"),
       plot = UMAP_var_hierarchical_clusters,
       device = "pdf", # or one of "eps", "ps", "tex" (pictex), "pdf", "jpeg", "tiff", "png", "bmp", "svg" or "wmf" (windows only).
       path = path_for_figures,
       width = 12, height = 9, units = "in", dpi = 300,
       family = "Helvetica", bg = "transparent"
...
ggsave(filename = str_c(date, "_hierarchical_UMAP_cv2_10_005_clusters_composed.pdf"),
       plot = UMAP_cv2_10_005_hierarchical_clusters,
       device = "pdf", # or one of "eps", "ps", "tex" (pictex), "pdf", "jpeg", "tiff", "png", "bmp", "svg" or "wmf" (windows only).
       path = path_for_figures,
       width = 12, height = 9, units = "in", dpi = 300,
       family = "Helvetica", bg = "transparent"
...
ggsave(filename = str_c(date, "_hierarchical_UMAP_cv2_15_001_clusters_composed.pdf"),
       plot = UMAP_cv2_15_001_hierarchical_clusters,
       device = "pdf", # or one of "eps", "ps", "tex" (pictex), "pdf", "jpeg", "tiff", "png", "bmp", "svg" or "wmf" (windows only).
       path = path_for_figures,
       width = 12, height = 9, units = "in", dpi = 300,
       family = "Helvetica", bg = "transparent"
...
pdf(file = str_c(path_for_figures, date, "_hierarchical_dendrograms.pdf"),
    width = 12, height = 9,
    family = "Helvetica", bg = "transparent")
par(mfrow = c(2,2))
labels_colors(PAG_dend_var) <- cluster_colours_10[PAG_clust_var_cutreeDynamic][order.dendrogram(PAG_dend_var)]
...
table(Cluster = PAG_sceset_qc_norm_filt_corr$cutree_clusters_var_k, PAGarea_Celltype = PAG_sceset_qc_norm_filt_corr$PAGarea_celltype)
...
table(Cluster = PAG_sceset_qc_norm_filt_corr$cutree_clusters_cv2_k, PAGarea_Celltype = PAG_sceset_qc_norm_filt_corr$PAGarea_celltype)
```
...
plotHeatmap(PAG_sceset_qc_norm_filt_corr, 
            ...
            cluster_cols = PAG_tree_var, colour_columns_by = c("cutree_clusters_var_k", "PAG.areacollection", "cell.type"),
...
plotHeatmap(PAG_sceset_qc_norm_filt_corr, 
            ...
            cluster_cols = PAG_tree_cv2, colour_columns_by = c("cutree_clusters_cv2_k", "PAG.areacollection", "cell.type"),
            cluster_rows = FALSE, center = TRUE, symmetric = TRUE,
...
We can also notice that in the HVG_var scenario, the dendrogram first splits away a subset of VGluT2 cells, and only the scecond split is between VGAT and the remaining VGluT2 cells. This does not happen with the HVG_cv2 scenario, where the first split is actually VGAT and VGlUT2. This is yet another reason (together with the results from t-SNE and UMAP, and the other clustering approaches) to select HVGs using the mean-cv2 trend instead of the mean-var trend.
...
plot(sil_var_cutreeDynamic, main = paste("Silhouette from Hierarchical (Dynamic Tree Cut):", length(unique(PAG_clust_var_cutreeDynamic)), "clusters"),
...
plot(sil_cv2_cutreeDynamic, main = paste("Silhouette from Hierarchical (Dynamic Tree Cut):", length(unique(PAG_clust_var_cutreeDynamic)), "clusters"),
...
plot(sil_var_cutree, main = paste("Silhouette from Hierarchical (Cutree):", length(unique(PAG_clust_var_cutree_k)), "clusters"),
...
plot(sil_cv2_cutree, main = paste("Silhouette from Hierarchical (Cutree):", length(unique(PAG_clust_cv2_cutree_k)), "clusters"),
     border = sil_cols_cv2_cutree, col = sil_cols_cv2_cutree, do.col.sort = FALSE)

# Save plots:
pdf(file = str_c(path_for_figures, date, "_hierarchical_silhouettes.pdf"),
    width = 16, height = 12,
    family = "Helvetica", bg = "transparent")
par(mfrow = c(2,2))
plot(sil_var_cutreeDynamic, main = paste("Silhouette from Hierarchical (Dynamic Tree Cut):", length(unique(PAG_clust_var_cutreeDynamic)), "clusters"),
...
head(names(metadata(PAG_sceset_qc_norm_filt_corr)$sc3$kmeans))
...
for(chosen_k in range_of_k) {
  UMAP_cv2_10_005_sc3 <- plotReducedDim(PAG_sceset_qc_norm_filt_corr, "UMAP_cv2_corrected_10_neighbors_0.05_min_dist",
                                        colour_by = paste0("sc3_", chosen_k, "_clusters"), shape_by = "cell.type",
                                        text_by = paste0("sc3_", chosen_k, "_clusters"))  + labs(title = sprintf("UMAP (cv2) - SC3 (k=%s)", chosen_k), x = "UMAP 1", y = "UMAP 2")
  
  UMAP_cv2_15_001_sc3 <- plotReducedDim(PAG_sceset_qc_norm_filt_corr, "UMAP_cv2_corrected_15_neighbors_0.01_min_dist",
                                        colour_by = paste0("sc3_", chosen_k, "_clusters"), shape_by = "cell.type",
                                        text_by = paste0("sc3_", chosen_k, "_clusters"))  + labs(title = sprintf("UMAP (cv2) - SC3 (k=%s)", chosen_k), x = "UMAP 1", y = "UMAP 2")
  
  multiplot(UMAP_cv2_10_005_PAGsubdivisions, UMAP_cv2_15_001_PAGsubdivisions, UMAP_cv2_10_005_sc3, UMAP_cv2_15_001_sc3, cols = 2)
}
```

After we have explored the results for the different range of `k`, we can then replicate the relevant plots to summarise our results. From the UMAP embeddings above we can conclude that `k=7` or `k=9` seem to be best.
```{r}
## SC3 Clusters (k = 7)
UMAP_cv2_10_005_sc3_k7 <- plotReducedDim(PAG_sceset_qc_norm_filt_corr, "UMAP_cv2_corrected_10_neighbors_0.05_min_dist",
                                         colour_by = "sc3_7_clusters", shape_by = "cell.type", text_by = "sc3_7_clusters"
                                         )  + labs(title = "UMAP (cv2) - SC3 (k=7)", x = "UMAP 1", y = "UMAP 2")
  
UMAP_cv2_15_001_sc3_k7 <- plotReducedDim(PAG_sceset_qc_norm_filt_corr, "UMAP_cv2_corrected_15_neighbors_0.01_min_dist",
                                         colour_by = "sc3_7_clusters", shape_by = "cell.type", text_by = "sc3_7_clusters"
                                         )  + labs(title = "UMAP (cv2) - SC3 (k=7)", x = "UMAP 1", y = "UMAP 2")

## SC3 Clusters (k = 9)
UMAP_cv2_10_005_sc3_k9 <- plotReducedDim(PAG_sceset_qc_norm_filt_corr, "UMAP_cv2_corrected_10_neighbors_0.05_min_dist",
                                         colour_by = "sc3_9_clusters", shape_by = "cell.type", text_by = "sc3_9_clusters"
                                         )  + labs(title = "UMAP (cv2) - SC3 (k=9)", x = "UMAP 1", y = "UMAP 2")
  
UMAP_cv2_15_001_sc3_k9 <- plotReducedDim(PAG_sceset_qc_norm_filt_corr, "UMAP_cv2_corrected_15_neighbors_0.01_min_dist",
                                         colour_by = "sc3_9_clusters", shape_by = "cell.type", text_by = "sc3_9_clusters"
                                         )  + labs(title = "UMAP (cv2) - SC3 (k=9)", x = "UMAP 1", y = "UMAP 2")

# Prepare composed plots
library(patchwork)
UMAP_sc3_k7_clusters <- (UMAP_cv2_10_005_PAGsubdivisions + UMAP_cv2_10_005_sc3_k7 +
                           UMAP_cv2_15_001_PAGsubdivisions + UMAP_cv2_15_001_sc3_k7) +
  plot_annotation(title = "SC3 clustering on UMAP", tag_levels = "A", 
                  theme = theme(plot.title = element_text(size = 18), plot.tag = element_text(size = 14))) + plot_layout(ncol = 2)

UMAP_sc3_k9_clusters <- (UMAP_cv2_10_005_PAGsubdivisions + UMAP_cv2_10_005_sc3_k9 +
                           UMAP_cv2_15_001_PAGsubdivisions + UMAP_cv2_15_001_sc3_k9) +
  plot_annotation(title = "SC3 clustering on UMAP", tag_levels = "A", 
                  theme = theme(plot.title = element_text(size = 18), plot.tag = element_text(size = 14))) + plot_layout(ncol = 2)

# Plot them
UMAP_sc3_k7_clusters
UMAP_sc3_k9_clusters

# Save them
ggsave(filename = str_c(date, "_sc3_UMAP_clusters_k7_composed.pdf"),
       plot = UMAP_sc3_k7_clusters,
       device = "pdf", # or one of "eps", "ps", "tex" (pictex), "pdf", "jpeg", "tiff", "png", "bmp", "svg" or "wmf" (windows only).
       path = path_for_figures,
       width = 12, height = 9, units = "in", dpi = 300,
       family = "Helvetica", bg = "transparent"
...
ggsave(filename = str_c(date, "_sc3_UMAP_clusters_k9_composed.pdf"),
       plot = UMAP_sc3_k9_clusters,
       device = "pdf", # or one of "eps", "ps", "tex" (pictex), "pdf", "jpeg", "tiff", "png", "bmp", "svg" or "wmf" (windows only).
       path = path_for_figures,
       width = 12, height = 9, units = "in", dpi = 300,
       family = "Helvetica", bg = "transparent"
...
In addition to the UMAP embeddings, we can use the following SC3 output plots to explore our results in a bit more depths:

* The _Consensus Matrix_ is a NxN matrix, where N is the number of cells. It represents similarity between the cells based on the averaging of clustering results from all combinations of clustering parameters. Similarity 0 (blue) means that the two cells are always assigned to different clusters. In contrast, similarity 1 (red) means that the two cells are always assigned to the same cluster. The consensus matrix is clustered by hierarchical clustering and has a diagonal-block structure. Intuitively, the perfect clustering is achieved when all diagonal blocks are completely red and all off-diagonal elements are completely blue. 
...
```{r}
# Save the plots adjusting their size so they don't get cropped
best_k <- list(7, 9) # Set the chosen K for the following plots
for(chosen_k in best_k) {
  # Consensus Matrix
  consensus <- sc3_plot_consensus(PAG_sceset_qc_norm_filt_corr, k = chosen_k,
                                  show_pdata = c("cell.type", "PAG.arearegistration", paste0("sc3_", chosen_k, "_clusters")))
  consensus
  ggsave(filename = str_c(date, "sc3_consensus_k", chosen_k, ".pdf"), 
         plot = consensus,
         device = "pdf", # or one of "eps", "ps", "tex" (pictex), "pdf", "jpeg", "tiff", "png", "bmp", "svg" or "wmf" (windows only).
         path = path_for_figures,
         width = 12, height = 9, units = "in", dpi = 300,
  ...
  silhouette
  ggsave(filename = str_c(date, "sc3_silhouette_k", chosen_k, ".pdf"), 
         plot = silhouette,
         device = "pdf", # or one of "eps", "ps", "tex" (pictex), "pdf", "jpeg", "tiff", "png", "bmp", "svg" or "wmf" (windows only).
         path = path_for_figures,
         width = 12, height = 9, units = "in", dpi = 300,
  ...
  cluster_stability
  ggsave(filename = str_c(date, "sc3_stability_k", chosen_k, ".pdf"), 
  ...
  expression_panel <- sc3_plot_expression(PAG_sceset_qc_norm_filt_corr, k = chosen_k,
                                          show_pdata = c("cell.type", "PAG.arearegistration", paste0("sc3_", chosen_k, "_clusters")))
  expression_panel
  ggsave(filename = str_c(date, "sc3_expression_k", chosen_k, ".pdf"), 
  ...
  DE_genes <- sc3_plot_de_genes(PAG_sceset_qc_norm_filt_corr, k = chosen_k,
                                ...
                                show_pdata = c("cell.type", "PAG.arearegistration", paste0("sc3_", chosen_k, "_clusters")))
  DE_genes
  ggsave(filename = str_c(date, "sc3_DEgenes_k", chosen_k, ".pdf"), 
         plot = DE_genes,
         device = "pdf", # or one of "eps", "ps", "tex" (pictex), "pdf", "jpeg", "tiff", "png", "bmp", "svg" or "wmf" (windows only).
         path = path_for_figures,
         width = 12, height = 18, units = "in", dpi = 300,
  ...
  markers <- sc3_plot_markers(PAG_sceset_qc_norm_filt_corr, k = chosen_k,
                              ...
                              show_pdata = c("cell.type", "PAG.arearegistration", paste0("sc3_", chosen_k, "_clusters")))
  markers
  ggsave(filename = str_c(date, "sc3_markers_k", chosen_k, ".pdf"), 
         plot = markers,
         device = "pdf", # or one of "eps", "ps", "tex" (pictex), "pdf", "jpeg", "tiff", "png", "bmp", "svg" or "wmf" (windows only).
         path = path_for_figures,
         width = 12, height = 18, units = "in", dpi = 300,
...
As a final step, we can plot the chosen clustering solutions side by side so we can compare them and make a final decision regarding which cluster IDs to use for differential expression:
```{r}
## Graph-based Clustering
tSNE_var_SNN_clusters
tSNE_cv2_SNN_clusters
...
paste0("Macrophage cluster has ", sum(PAG_sceset_qc_norm_filt_corr$SNN_clusters_cv2_rank_k7==8), " cells in SNN_rank_k7 method")
paste0("Macrophage cluster has ", sum(PAG_sceset_qc_norm_filt_corr$SNN_clusters_cv2_jaccard_k8==11), " cells in SNN_louvain_k8 method")
paste0("Macrophage cluster has ", sum(PAG_sceset_qc_norm_filt_corr$SNN_clusters_cv2_jaccard_k10==9), " cells in SNN_louvain_k10 method")
paste0("Macrophage cluster has ", sum(PAG_sceset_qc_norm_filt_corr$kmeans_clusters_cv2==7), " cells in kmeans method")
paste0("Macrophage cluster has ", sum(PAG_sceset_qc_norm_filt_corr$cutree_clusters_cv2_k==5), " cells in hierarchical method")
paste0("Macrophage cluster has ", sum(PAG_sceset_qc_norm_filt_corr$sc3_7_clusters==7), " cells in SC3_k7 method")
paste0("Macrophage cluster has ", sum(PAG_sceset_qc_norm_filt_corr$sc3_9_clusters==5), " cells in SC3_k9 method")

# Use the which() function to get the indices of the cells assigned to the macrophage-enriched cluster:
which(PAG_sceset_qc_norm_filt_corr$SNN_clusters_cv2_rank_k7==8)
which(PAG_sceset_qc_norm_filt_corr$SNN_clusters_cv2_jaccard_k8==11)
which(PAG_sceset_qc_norm_filt_corr$SNN_clusters_cv2_jaccard_k10==9)
which(PAG_sceset_qc_norm_filt_corr$kmeans_clusters_cv2==7)
which(PAG_sceset_qc_norm_filt_corr$cutree_clusters_cv2_k==5)
which(PAG_sceset_qc_norm_filt_corr$sc3_9_clusters==5)
which(PAG_sceset_qc_norm_filt_corr$sc3_7_clusters==7)

# Localise and zoom in on the macrophage-enriched cluster
UMAP_cv2_15_001_JaccardLouvain_k8
plotReducedDim(PAG_sceset_qc_norm_filt_corr, dimred = "UMAP_cv2_corrected_15_neighbors_0.01_min_dist",
               colour_by = "Cd68", text_by = "SNN_clusters_cv2_jaccard_k8", shape_by = "cell.type"
               ) + labs(title = "UMAP (cv2) - Jaccard-Louvain (k=8)", x = "UMAP 1", y = "UMAP 2")
UMAP_cv2_15_001_PAGsubdivisions + xlim(-4.16,-4) + ylim(-7.47,-7.32)

# For each clustering approach, plot only the cells assigned to the macrophage-enriched cluster:
plotReducedDim(PAG_sceset_qc_norm_filt_corr[, PAG_sceset_qc_norm_filt_corr$SNN_clusters_cv2_rank_k7==8], dimred = "UMAP_cv2_corrected_15_neighbors_0.01_min_dist",
               colour_by = "Cd68", shape_by = "cell.type") + labs(title = "UMAP (cv2) - Rank-Walktrap (k=7)", x = "UMAP 1", y = "UMAP 2")
plotReducedDim(PAG_sceset_qc_norm_filt_corr[, PAG_sceset_qc_norm_filt_corr$SNN_clusters_cv2_jaccard_k8==11], dimred = "UMAP_cv2_corrected_15_neighbors_0.01_min_dist",
               colour_by = "Cd68", shape_by = "cell.type") + labs(title = "UMAP (cv2) - Jaccard-Louvain (k=8)", x = "UMAP 1", y = "UMAP 2")
plotReducedDim(PAG_sceset_qc_norm_filt_corr[, PAG_sceset_qc_norm_filt_corr$SNN_clusters_cv2_jaccard_k10==9], dimred = "UMAP_cv2_corrected_15_neighbors_0.01_min_dist",
               colour_by = "Cd68", shape_by = "cell.type") + labs(title = "UMAP (cv2) - Jaccard-Louvain (k=10)", x = "UMAP 1", y = "UMAP 2")
plotReducedDim(PAG_sceset_qc_norm_filt_corr[, PAG_sceset_qc_norm_filt_corr$kmeans_clusters_cv2==7], dimred = "UMAP_cv2_corrected_15_neighbors_0.01_min_dist",
               colour_by = "Cd68", shape_by = "cell.type") + labs(title = "UMAP (cv2) - kmeans", x = "UMAP 1", y = "UMAP 2")
plotReducedDim(PAG_sceset_qc_norm_filt_corr[, PAG_sceset_qc_norm_filt_corr$cutree_clusters_cv2_k==5], dimred = "UMAP_cv2_corrected_15_neighbors_0.01_min_dist",
               colour_by = "Cd68", shape_by = "cell.type") + labs(title = "UMAP (cv2) - Hierarchical (k=10)", x = "UMAP 1", y = "UMAP 2")
plotReducedDim(PAG_sceset_qc_norm_filt_corr[, PAG_sceset_qc_norm_filt_corr$sc3_7_clusters==7], dimred = "UMAP_cv2_corrected_15_neighbors_0.01_min_dist",
               colour_by = "Cd68", shape_by = "cell.type") + labs(title = "UMAP (cv2) - SC3 (k=7)", x = "UMAP 1", y = "UMAP 2")
plotReducedDim(PAG_sceset_qc_norm_filt_corr[, PAG_sceset_qc_norm_filt_corr$sc3_9_clusters==5], dimred = "UMAP_cv2_corrected_15_neighbors_0.01_min_dist",
               colour_by = "Cd68", shape_by = "cell.type") + labs(title = "UMAP (cv2) - SC3 (k=9)", x = "UMAP 1", y = "UMAP 2")
```

We can see that this is indeed true for our Graph-based and Hierarchical clustering approaches, as they assign the same 12 cells to the macrophage-enriched cluster. However, looking at the results of the K-means and SC3 (which is also k-means based) clustering we can see that they both assign extra cells to the macrophage-enriched cluster. From this we can conclude that K-means clustering doesn't do as good job as Graph-based or Hierarchical clustering for our dataset, and we will continue investigating the results of the latter two.
```{r}
# Plot UMAPs without `text_by`:
plotReducedDim(PAG_sceset_qc_norm_filt_corr, dimred = "UMAP_cv2_corrected_15_neighbors_0.01_min_dist",
               colour_by = "SNN_clusters_cv2_rank_k7", shape_by = "cell.type") + labs(title = "UMAP (cv2) - Rank-Walktrap (k=7)", x = "UMAP 1", y = "UMAP 2")
plotReducedDim(PAG_sceset_qc_norm_filt_corr, dimred = "UMAP_cv2_corrected_15_neighbors_0.01_min_dist",
               colour_by = "SNN_clusters_cv2_jaccard_k8", shape_by = "cell.type") + labs(title = "UMAP (cv2) - Jaccard-Louvain (k=8)", x = "UMAP 1", y = "UMAP 2")
plotReducedDim(PAG_sceset_qc_norm_filt_corr, dimred = "UMAP_cv2_corrected_15_neighbors_0.01_min_dist",
               colour_by = "SNN_clusters_cv2_jaccard_k10", shape_by = "cell.type") + labs(title = "UMAP (cv2) - Jaccard-Louvain (k=10)", x = "UMAP 1", y = "UMAP 2")
plotReducedDim(PAG_sceset_qc_norm_filt_corr, dimred = "UMAP_cv2_corrected_15_neighbors_0.01_min_dist",
               colour_by = "cutree_clusters_cv2_k", shape_by = "cell.type") + labs(title = "UMAP (cv2) - Hierarchical (k=10)", x = "UMAP 1", y = "UMAP 2")
```

Finally, we can visually inspect the degree of agreement between the clustering assignments and the UMAP embedding of the cluster IDs. In the plots above, we can see that the best representation is the one obtained from Graph-based clustering with the Jaccard-Louvain approach, as it is apparent from the cleanest separation of inhibitory subclusters and the main excitatory subclusters. Thus, we will focus on the results obtained from Graph-based clustering with the Jaccard-Louvain approach for our Differential Expression analysis.
