...
```{r}
library(DESeq2)
PAG_DESeq_set_simple <- DESeqDataSetFromMatrix(countData = assay(PAG_sceset_qc_norm_filt_corr, "counts_rounded"), 
                                               colData = colData(PAG_sceset_qc_norm_filt_corr),
                                               design = ~ PAG.APaxis + cell.type + PAG.arearegistration)

# To check differences across PAG subdivisions within a cell type
...
As we have already specified an experimental design when we created the _DESeqDataSet_, we can run the differential expression pipeline on the raw counts with a single call to the function `DESeq`. This function will carry out: the estimation of size factors (controlling for differences in the sequencing depth of the samples), the estimation of dispersion values for each gene, and fitting a generalized linear model. A _DESeqDataSet_ is returned that contains all the fitted parameters within it, and we can plot the estimated dispersions from it.
```{r}
# Filter genes to speed things up:
keep_simple <- rowSums(counts(PAG_DESeq_set_simple) >= 10) >= 10 # Keep genes with at least 10 copies in at least 10 cells.
summary(keep_simple)
PAG_DESeq_set_simple <- PAG_DESeq_set_simple[keep_simple,]

start_time <- Sys.time() # Takes around 30min
PAG_DESeq_set <- DESeq2::DESeq(PAG_DESeq_set, minReplicatesForReplace = Inf, sfType = "poscounts", parallel = TRUE, BPPARAM = MulticoreParam(4))
DESeq2::plotDispEsts(PAG_DESeq_set)
end_time <- Sys.time()
end_time - start_time

# Do the same for the group design:
keep_grouped <- rowSums(counts(PAG_DESeq_set_grouped) >= 10) >= 10 # Keep genes with at least 10 copies in at least 10 cells.
length(keep_grouped)
PAG_DESeq_set_grouped <- PAG_DESeq_set_grouped[keep_grouped,]

start_time <- Sys.time() # Takes around 30min
PAG_DESeq_set_grouped <- DESeq2::DESeq(PAG_DESeq_set_grouped, minReplicatesForReplace = Inf, sfType = "poscounts", parallel = TRUE, BPPARAM = MulticoreParam(4))
DESeq2::plotDispEsts(PAG_DESeq_set_grouped)
end_time <- Sys.time()
end_time - start_time

# Do the same for full design:
keep <- rowSums(counts(PAG_DESeq_set) >= 10) >= 10 # Keep genes with at least 10 copies in at least 10 cells.
summary(keep)
PAG_DESeq_set <- PAG_DESeq_set[keep,]
...
saveRDS(PAG_DESeq_set, file = "PAG_DESeq_set.rds")
saveRDS(PAG_DESeq_set_grouped, file = "PAG_DESeq_set_grouped.rds")
print("Saved!")
```
