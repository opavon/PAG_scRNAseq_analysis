...
graph_cv2_rank <- buildSNNGraph(PAG_sceset_qc_norm_filt_corr,
                                ...
                                subset.row = metadata(PAG_sceset_qc_norm_filt)$hvg_cv2_out_no_spikes_filt,
                                assay.type = "logcounts",
...
# Using HVG from Var with rank-based weights
plotReducedDim(PAG_sceset_qc_norm_filt_corr, dimred = "UMAP_var_logcounts_10_neighbors_0.05_min_dist",
               colour_by = "SNN_clusters_var_rank", shape_by = "cell.type", text_by = "SNN_clusters_var_rank") + ggtitle("UMAP_var_logcounts_10_neighbors_0.05_min_dist")

# Using HVG from Var with jaccard-based weights
plotReducedDim(PAG_sceset_qc_norm_filt_corr, dimred = "UMAP_var_logcounts_10_neighbors_0.05_min_dist",
               colour_by = "SNN_clusters_var_jaccard", shape_by = "cell.type", text_by = "SNN_clusters_var_jaccard") + ggtitle("UMAP_var_logcounts_10_neighbors_0.05_min_dist")
...
# Using HVG from CV2 with rank-based weights
plotReducedDim(PAG_sceset_qc_norm_filt_corr, dimred = "UMAP_var_logcounts_10_neighbors_0.05_min_dist",
               colour_by = "SNN_clusters_cv2_rank", shape_by = "cell.type", text_by = "SNN_clusters_cv2_rank") + ggtitle("UMAP_var_logcounts_10_neighbors_0.05_min_dist")

# Using HVG from CV2 with jaccard-based weights
plotReducedDim(PAG_sceset_qc_norm_filt_corr, dimred = "UMAP_var_logcounts_10_neighbors_0.05_min_dist",
               colour_by = "SNN_clusters_cv2_jaccard", shape_by = "cell.type", text_by = "SNN_clusters_cv2_jaccard") + ggtitle("UMAP_var_logcounts_10_neighbors_0.05_min_dist")
```
...
We compute a cell-cell distance matrix using the top PCs and we apply hierarchical clustering with Ward’s method. We can then inspect the resulting tree. While both Ward’s method and complete linkage (`hclust()`’s default) yield compact clusters, we prefer the former it is less affected by differences in variance between clusters. This yields a dendrogram that groups together cells with similar expression patterns across the chosen genes.
```{r}
PAG_dist <- dist(reducedDim(PAG_sceset_qc_norm_filt_corr, "PCA")) # Making a distance matrix
PAG_tree <- hclust(PAG_dist, "ward.D2") # Building a tree
plot(PAG_tree)
...
labels_colors(dend) <- c(`20160113.wild` = "blue",
...
To obtain explicit clusters, we “cut” the tree by removing internal branches such that every subtree represents a distinct cluster. This is most simply done by removing internal branches above a certain height of the tree, as performed by the `cutree()` function. We can use the `dynamicTreeCut` package, which uses the shape of the branches to obtain a more suitable partitioning for complex dendrograms.
```{r}
library(dynamicTreeCut)

# minClusterSize needs to be turned down for small datasets.
# deepSplit controls the resolution of the partitioning.
clust.416b <- cutreeDynamic(PAG_tree, distM = as.matrix(PAG_dist),
...
This generally corresponds well to the grouping of cells on a t-SNE plot.
```{r}
...
We can also compare the cluster identities to batch to check the clustering is not driven by batch effects. We can also compare to other interesting metadata to see what can be driving the clusters.
```{r}
