...
We have two types of cells (VGAT and VGluT2), across four different anatomical subdivisions. We want to find differences in expression (1) across cell-types, (2) across subdivisions within a cell-type, and (3) across subdivisions irrespective of cell-type. Importantly, each transgenic mouse will only allow us to capture one of the two cell-types, so the mouse ID will be confounded with one or the other cell.type. We want to control for this, as we are not interested in differences across mice, only across cell-types. 

* An example of such design is obtained by writing `~ cell.type + mouse.id`. By including `cell.type`, terms will be added to the model which account for differences across VGAT and VGlut2 cells, and by adding `mouse.id` we get a single term which explains the differences between animals. We could also have `~ PAG.area + cell.type + mouse.id`, to find differences across subdivisions while controling both for cell.type and mouse.id.
* __Note__: it will be helpful for us if the first level of a factor is the reference level (e.g. control, or untreated samples). The reason is that by specifying this, functions further in the pipeline can be used and will give comparisons such as 'treatment vs control', without needing to specify additional arguments.
* If the research aim is to determine for which genes the effect of treatment is different across groups, then interaction terms can be included and tested using a design such as `~ group + treatment + group:treatment`. See the vignettes of DESeq2 and edgeR for more examples.

```{r}
# We can relevel any metadata factor as follows:
#colData(sg)$dex <- relevel(colData(sg)$dex, ref = "untrt")
#colData(sg)$dex
# In our case it doesn't make much sense, as we are not comparing treatments or conditions in a time sequence, and there is no clear reference level for cell.type or mouse.id.
```

Starting from a _SummarizedExperiment_ object:
```{r}
library(DESeq2)
...
# We would like to add the size factors we calculated with scran instead of using edgeR::calcNormFactors to calculate TMM, as this is a method for bulk RNA-seq.
```
