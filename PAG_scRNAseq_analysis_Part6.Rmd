...
```{r}
resultsNames(PAG_DESeq_set_3)
levels(PAG_sceset_qc_norm_filt_corr$PAGarea_celltype)
PAG_DESeq_results_3 <- DESeq2::results(PAG_DESeq_set_3, 
                                       contrast = c("PAGarea_celltype", "dmpag_VGAT", "dmpag_VGluT2"), 
                                       listValues = c(1, -1/7),  lfcThreshold = 0, alpha = 0.05, pAdjustMethod = "BH", parallel = TRUE)
head(PAG_DESeq_results_3)
#mcols(PAG_DESeq_results_3, use.names = TRUE)
```
...
```{r}
summary(PAG_DESeq_results_3)
hist(PAG_DESeq_results_3$pvalue)

## We can also add a couple of extra columns that will be useful for the interactive visualization later
PAG_DESeq_results_3$log10BaseMean <- log10(PAG_DESeq_results_3$baseMean)
PAG_DESeq_results_3$mlog10PValue <- -log10(PAG_DESeq_results_3$pvalue)
rowData(PAG_DESeq_set_3)$log10Dispersion <- log10(rowData(PAG_DESeq_set_3)$dispersion)
rowData(PAG_DESeq_set_3)$DESeq2_dmpagVGAT_vs_dmpagVGluT2 <- PAG_DESeq_results_3 # Or any other comparison you called using results
```
...
table(PAG_DESeq_results_3_05$padj < 0.05)
```
...
```{r}
sum(PAG_DESeq_results_3_05$padj < 0.05, na.rm = TRUE)
```
...
```{r}
resSig <- subset(PAG_DESeq_results_3_05, padj < 0.05)
...
topGene <- rownames(PAG_DESeq_results_1)[which.min(PAG_DESeq_results_1$padj)]
...
```{r}
library("apeglm")
resultsNames(PAG_DESeq_set_3) # Check to see which coef to choose
```

```{r}
PAG_DESeq_results_1_LFC <- lfcShrink(PAG_DESeq_set_1, coef="cell.type_VGluT2_vs_VGAT", type="apeglm")
PAG_DESeq_results_1_LFC
DESeq2::plotMA(PAG_DESeq_results_1_LFC, ylim = c(-5, 5))
```
...
```{r}
plotMA(PAG_DESeq_results_3, ylim = c(-5,5))
topGene <- rownames(PAG_DESeq_results_3)[which.min(PAG_DESeq_results_3$padj)]
with(PAG_DESeq_results_3[topGene, ], {
...
```{r}
PAG_DESeq_results_3_ordered <- PAG_DESeq_results_3[order(PAG_DESeq_results_3$padj), ]
head(PAG_DESeq_results_3_ordered)

PAG_DESeq_results_3_ordered_DF <- as.data.frame(PAG_DESeq_results_3_ordered)[seq_len(500), ] # Exporting top 100 genes
#write.table(cbind(id = rownames(PAG_DESeq_results_1_ordered_DF), PAG_DESeq_results_1_ordered_DF), 
#            file = "PAG_DE_results.txt", quote = FALSE, sep = "\t", row.names = FALSE)

write.csv(PAG_DESeq_results_3_ordered_DF, file = "PAG_DESeq_results_3_ordered_DF.csv")
```
