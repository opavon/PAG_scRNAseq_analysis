...
### edgeR
`edgeR` is a method originally designed for bulkRNAis. It is based on a negative binomial model of gene expression and uses a generalized linear model (GLM) framework, that enables us to include other factors such as `batch` to the model.
```{r}
...
## Step 8.2 | EBI-EMBL Differential Expression RNAseq workflow (Charlotte Soneson)
...
We want to find differences in expression of genes (1) across *cell-types*, (2) across *subdivisions within a cell-type*, and (3) across *subdivisions irrespective of cell-type*. Importantly, each transgenic mouse (`mouse.id`) only allows us to capture one of the two cell-types, so the _mouse ID_ will be confounded with one or the other _cell type_. We want to control for this, as we are not interested in differences across mice, only across cell-types. 
...
```{r}
library(DESeq2)
PAG_DESeq_set <- DESeqDataSetFromMatrix(countData = assay(PAG_sceset_qc_norm_filt_corr, "counts_rounded"), 
                                        colData = colData(PAG_sceset_qc_norm_filt_corr),
                                        design = ~ mouse.sex + batch.processing + batch.sequencing_round + PAG.APaxis + cell.type + PAG.arearegistration)

# To check 
PAG_DESeq_set_grouped <- DESeqDataSetFromMatrix(countData = assay(PAG_sceset_qc_norm_filt_corr, "counts_rounded"), 
                                                colData = colData(PAG_sceset_qc_norm_filt_corr),
                                                design = ~ mouse.sex + batch.processing + batch.sequencing_round + PAG.APaxis + group)
```
...
Specifying `blind = FALSE` means that differences between `mouse.id` and `cell.type` (the variables in the _design_) will not contribute to the expected variance-mean trend of the experiment. The experimental design is not used directly in the transformation, only in estimating the global amount of variability in the counts. For a fully unsupervised transformation, one can set blind = TRUE (which is the default).

***
__Sample distances:__
...
rownames(sampleDistMatrix) <- paste( PAG_DESeq_vsd$mouse.id, PAG_DESeq_vsd$cell.type, sep = " - " )
...
`DESeq_results` is a _DataFrame_ object, so it contains metadata with information:

* The first column, `baseMean`, is a just the average of the normalized count values, dividing by size factors, taken over all samples in the _DESeqDataSet_. The remaining four columns refer to a specific contrast, namely the comparison of the `VGAT` level over the `VGluT2` level for the factor variable `cell.type`.
...
## We also add a couple of extra columns that will be useful for the interactive visualization later
...
rowData(PAG_DESeq_set)$DESeq2_VGAT_vs_VGluT2 <- PAG_DESeq_results
```
...
In high-throughput biology, we are careful to not use the p values directly as evidence against the null hypothesis, but to correct for multiple testing. _DESeq2_ and _edgeR_ use the Benjamini-Hochberg (BH) adjustment (Benjamini and Hochberg 1995) as implemented in the base R `p.adjust` function; in brief, this method calculates for each gene an adjusted p value that answers the following question: if one called significant all genes with an adjusted p value less than or equal to this gene's adjusted p value threshold, what would be the fraction of false positives (the false discovery rate, FDR) among them, in the sense of the calculation outlined above? These values, called the BH-adjusted p values, are given in the column `padj` of the `res` object from _DESeq2_, and in the `FDR` column in the `TopTags` object from _edgeR_.
...
edgeR is designed for bulkRNA and is based on a negative binomial model of gene expression and uses a generalized linear model (GLM) framework, the enables us to include other factors such as batch to the model. We have a `DGEList` object containing all the necessary information. We first define a design matrix, using the same formula syntax as we did for DESeq2.
```{r}
names(PAG_DGE_set)
PAG_DGE_design <- model.matrix(~ mouse.id + cell.type,
                               data = PAG_DGE_set$samples)
```
...
```{r}
DGE_genes_keep <- edgeR::filterByExpr(PAG_DGE_set, PAG_DGE_design)
PAG_DGE_set <- PAG_DGE_set[DGE_genes_keep, ]
PAG_DGE_set <- edgeR::estimateDisp(PAG_DGE_set, PAG_DGE_design)
edgeR::plotBCV(PAG_DGE_set)
```
...
```{r}
fit <- edgeR::glmQLFit(PAG_DGE_set, PAG_DGE_design)
qlf <- edgeR::glmQLFTest(fit, coef = ncol(PAG_DGE_design))

PAG_edgeR_results_all <- edgeR::topTags(qlf, n = nrow(PAG_DGE_set), sort.by = "none") # all genes
hist(PAG_edgeR_results_all$table$PValue)

PAG_edgeR_results_tt <- edgeR::topTags(qlf, n = nrow(PAG_DGE_set), p.value = 0.1) # genes with adj.p<0.1
...
PAG_edgeR_results_treat_tt <- edgeR::topTags(PAG_edgeR_results_treat, n = nrow(PAG_DGE_set), sort.by = "none")
```
...
### 8.2.3 | Comparing DESeq2 and edgeR results:
...
## Step 8.3 | simpleSingleCell Workflow - Limma-voom
...
### 10.4.3 | Creating a design matrix and contrasts
...
### 10.4.4 | Removing heteroscedascity from count data and linear modelling
...
### 10.4.5 | Examining the number of DE genes
...
### 10.4.6 | Useful graphical representations of differential expression results
