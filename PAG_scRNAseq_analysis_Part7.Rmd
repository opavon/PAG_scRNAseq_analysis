...
path_for_figures <- "D:/Dropbox (UCL - SWC)/Project_transcriptomics/figures_in_progress/R_figures_Part7_DE/"
date <- Sys.Date()
date <- gsub("-", "_", date)
...
results_name <- "SNN_clusters_cv2_jaccard_k8_" # one of "SNN_clusters_cv2_jaccard_k8_", "celltype_", "PAGarea_", "PAGarea_celltype_", "celltype_PAGAPaxis_", "VGAT_VGluT2_expression"
...
# Set the string to be added to the filename when saving the results:
results_name <- "celltype_" # one of "SNN_clusters_cv2_jaccard_k8_", "celltype_", "PAGarea_", "PAGarea_celltype_", "celltype_PAGAPaxis_", "VGAT_VGluT2_expression"
...
_We can repeat the same strategy used for `PAG.arearegistration`, but this time using only either VGAT+ or VGluT2+ neurons. This will test for differences across subdivisions within a particular cell type, thus highlighting genes relevant to either inhibitory or excitatory neurons, which will be less stringent than taking all the groups together._
...
```{r}
# Set the groups to be compared (cluster IDs or metadata slot of interest):
groups <- PAG_sceset_qc_norm_filt_corr_clust_de[,PAG_sceset_qc_norm_filt_corr_clust_de$PAG.arearegistration=="dmpag"]$cell.type

# Set the string to be added to the filename when saving the results:
results_name <- "celltype_dmpag_" # one of "SNN_clusters_cv2_jaccard_k8_", "celltype_", "PAGarea_", "PAGarea_celltype_", "celltype_PAGAPaxis_", "VGAT_VGluT2_expression"
...
for (test_choice in desired_tests){
  ...
  direction <- "up" # the direction of log-fold changes to be considered in the alternative hypothesis: "any", "up", "down".
  lfc <- 0 # positive numeric scalar specifying the log-fold change threshold to be tested against.
  block <- PAG_sceset_qc_norm_filt_corr_clust_de[,PAG_sceset_qc_norm_filt_corr_clust_de$PAG.arearegistration=="dmpag"]$mouse.sex # factor specifying the blocking level for each cell.
  design <- NULL # numeric matrix with blocking terms for uninteresting factors (not be confounded with groups).
  restrict <- NULL # the levels of groups for which to perform pairwise comparisons.
  exclude <- NULL # the levels of groups for which NOT to perform pairwise comparisons.
  
  # Run findMarkers:
  marker_genes <- findMarkers(PAG_sceset_qc_norm_filt_corr_clust_de[,PAG_sceset_qc_norm_filt_corr_clust_de$PAG.arearegistration=="dmpag"],
                              groups = groups,
                              test.type = test,
                              pval.type = comparisons,
                              direction = direction,
...
```{r}
# Set the groups to be compared (cluster IDs or metadata slot of interest):
groups <- PAG_sceset_qc_norm_filt_corr_clust_de[,PAG_sceset_qc_norm_filt_corr_clust_de$PAG.arearegistration=="dlpag"]$cell.type

# Set the string to be added to the filename when saving the results:
results_name <- "celltype_dlpag_" # one of "SNN_clusters_cv2_jaccard_k8_", "celltype_", "PAGarea_", "PAGarea_celltype_", "celltype_PAGAPaxis_", "VGAT_VGluT2_expression"
...
for (test_choice in desired_tests){
  ...
  direction <- "up" # the direction of log-fold changes to be considered in the alternative hypothesis: "any", "up", "down".
  lfc <- 0 # positive numeric scalar specifying the log-fold change threshold to be tested against.
  block <- PAG_sceset_qc_norm_filt_corr_clust_de[,PAG_sceset_qc_norm_filt_corr_clust_de$PAG.arearegistration=="dlpag"]$mouse.sex # factor specifying the blocking level for each cell.
  design <- NULL # numeric matrix with blocking terms for uninteresting factors (not be confounded with groups).
  restrict <- NULL # the levels of groups for which to perform pairwise comparisons.
  exclude <- NULL # the levels of groups for which NOT to perform pairwise comparisons.
  
  # Run findMarkers:
  marker_genes <- findMarkers(PAG_sceset_qc_norm_filt_corr_clust_de[,PAG_sceset_qc_norm_filt_corr_clust_de$PAG.arearegistration=="dlpag"],
                              groups = groups,
                              test.type = test,
                              pval.type = comparisons,
                              direction = direction,
...
```{r}
# Set the groups to be compared (cluster IDs or metadata slot of interest):
groups <- PAG_sceset_qc_norm_filt_corr_clust_de[,PAG_sceset_qc_norm_filt_corr_clust_de$PAG.arearegistration=="lpag"]$cell.type

# Set the string to be added to the filename when saving the results:
results_name <- "celltype_lpag_" # one of "SNN_clusters_cv2_jaccard_k8_", "celltype_", "PAGarea_", "PAGarea_celltype_", "celltype_PAGAPaxis_", "VGAT_VGluT2_expression"
...
for (test_choice in desired_tests){
  ...
  direction <- "up" # the direction of log-fold changes to be considered in the alternative hypothesis: "any", "up", "down".
  lfc <- 0 # positive numeric scalar specifying the log-fold change threshold to be tested against.
  block <- PAG_sceset_qc_norm_filt_corr_clust_de[,PAG_sceset_qc_norm_filt_corr_clust_de$PAG.arearegistration=="lpag"]$mouse.sex # factor specifying the blocking level for each cell.
  design <- NULL # numeric matrix with blocking terms for uninteresting factors (not be confounded with groups).
  restrict <- NULL # the levels of groups for which to perform pairwise comparisons.
  exclude <- NULL # the levels of groups for which NOT to perform pairwise comparisons.
  
  # Run findMarkers:
  marker_genes <- findMarkers(PAG_sceset_qc_norm_filt_corr_clust_de[,PAG_sceset_qc_norm_filt_corr_clust_de$PAG.arearegistration=="lpag"],
                              groups = groups,
                              test.type = test,
                              pval.type = comparisons,
                              direction = direction,
...
```{r}
# Set the groups to be compared (cluster IDs or metadata slot of interest):
groups <- PAG_sceset_qc_norm_filt_corr_clust_de[,PAG_sceset_qc_norm_filt_corr_clust_de$PAG.arearegistration=="vlpag"]$cell.type

# Set the string to be added to the filename when saving the results:
results_name <- "celltype_vlpag_" # one of "SNN_clusters_cv2_jaccard_k8_", "celltype_", "PAGarea_", "PAGarea_celltype_", "celltype_PAGAPaxis_", "VGAT_VGluT2_expression"
...
for (test_choice in desired_tests){
  ...
  direction <- "up" # the direction of log-fold changes to be considered in the alternative hypothesis: "any", "up", "down".
  lfc <- 0 # positive numeric scalar specifying the log-fold change threshold to be tested against.
  block <- PAG_sceset_qc_norm_filt_corr_clust_de[,PAG_sceset_qc_norm_filt_corr_clust_de$PAG.arearegistration=="vlpag"]$mouse.sex # factor specifying the blocking level for each cell.
  design <- NULL # numeric matrix with blocking terms for uninteresting factors (not be confounded with groups).
  restrict <- NULL # the levels of groups for which to perform pairwise comparisons.
  exclude <- NULL # the levels of groups for which NOT to perform pairwise comparisons.
  
  # Run findMarkers:
  marker_genes <- findMarkers(PAG_sceset_qc_norm_filt_corr_clust_de[,PAG_sceset_qc_norm_filt_corr_clust_de$PAG.arearegistration=="vlpag"],
                              groups = groups,
                              test.type = test,
                              pval.type = comparisons,
                              direction = direction,
...
For the `PAG.arearegistration` comparisons, 
...
```{r}
# Set the groups to be compared (cluster IDs or metadata slot of interest):
groups <- PAG_sceset_qc_norm_filt_corr_clust_de$PAG.arearegistration

# Set the string to be added to the filename when saving the results:
results_name <- "PAGarea_" # one of "SNN_clusters_cv2_jaccard_k8_", "celltype_", "PAGarea_", "PAGarea_celltype_", "celltype_PAGAPaxis_", "VGAT_VGluT2_expression"
...
We can repeat the same strategy used for `PAG.arearegistration`, but this time using only either VGAT+ or VGluT2+ neurons. This will test for differences across subdivisions within a particular cell type, thus highlighting genes relevant to either inhibitory or excitatory neurons, which will be less stringent than taking all the groups together.
...
```{r}
# Set the groups to be compared (cluster IDs or metadata slot of interest):
groups <- PAG_sceset_qc_norm_filt_corr_clust_de[,PAG_sceset_qc_norm_filt_corr_clust_de$cell.type=="VGAT"]$PAG.arearegistration

# Set the string to be added to the filename when saving the results:
results_name <- "PAGarea_vgat_" # one of "SNN_clusters_cv2_jaccard_k8_", "celltype_", "PAGarea_", "PAGarea_celltype_", "celltype_PAGAPaxis_", "VGAT_VGluT2_expression"
...
```{r}
# Set the groups to be compared (cluster IDs or metadata slot of interest):
groups <- PAG_sceset_qc_norm_filt_corr_clust_de[,PAG_sceset_qc_norm_filt_corr_clust_de$cell.type=="VGluT2"]$PAG.arearegistration

# Set the string to be added to the filename when saving the results:
results_name <- "PAGarea_vglut2_" # one of "SNN_clusters_cv2_jaccard_k8_", "celltype_", "PAGarea_", "PAGarea_celltype_", "celltype_PAGAPaxis_", "VGAT_VGluT2_expression"
...
### 7.1.6 | Marker genes for PAGarea_celltype
We will now perform pairwise comparisons to identify marker genes for `PAGarea_celltype`. For this, we continue with the `SingleCellExperiment` object we created after excluding the cells assigned to the macrophage-enriched cluster:
```{r}
dim(PAG_sceset_qc_norm_filt_corr_clust)
dim(PAG_sceset_qc_norm_filt_corr_clust_de)
```
...
For the `PAGarea_celltype` comparisons, __FILL IN__
```{r}
# Set the groups to be compared (cluster IDs or metadata slot of interest):
groups <- PAG_sceset_qc_norm_filt_corr_clust_de$PAGarea_celltype

# Set the string to be added to the filename when saving the results:
results_name <- "PAGarea_celltype_" # one of "SNN_clusters_cv2_jaccard_k8_", "celltype_", "PAGarea_", "PAGarea_celltype_", "celltype_PAGAPaxis_", "VGAT_VGluT2_expression"
...
# Initialise an empty data.frame to populate with a summary of marker genes:
PAGarea_celltype_marker_set_summary <- data.frame()
...
for (test_choice in desired_tests){
  ...
  for (comparison_choice in desired_comparisons){
    ...
    for (direction_choice in desired_directions){
      ...
      # Save results:
      saveRDS(marker_genes, file = str_c(path_for_figures, "PAGarea_celltype/", results_name, "marker_genes_", test, "_", comparisons, "_", direction, ".rds"))
      ...
      for (chosen_group in range_of_groups) {
        ...
        # Export the results to a .csv file:
        write.csv(temp_marker_set, file = str_c(path_for_figures, "PAGarea_celltype_/", "PAG_DE_results_", results_name, marker_set, ".csv"), quote = FALSE, row.names = TRUE)
        
        # Populate the summary data.frame:
        PAGarea_celltype_marker_set_summary = rbind(PAGarea_celltype_marker_set_summary,
                                                    data.frame(marker_set = marker_set,
                                                               group = levels(groups)[chosen_group],
                                                               test = test,
                                                               comparisons = comparisons,
...
# Check out the summary: 
PAGarea_celltype_marker_set_summary[PAGarea_celltype_marker_set_summary$test=="t",]
PAGarea_celltype_marker_set_summary[PAGarea_celltype_marker_set_summary$test=="wilcox",]
PAGarea_celltype_marker_set_summary[PAGarea_celltype_marker_set_summary$test=="binom",]
```
...
plotHeatmap(PAG_sceset_qc_norm_filt_corr_clust,
            ...
            colour_columns_by = c("SNN_clusters_cv2_jaccard_k10", "cell.type", "PAGarea_celltype"), # how the columns should be annotated with colours.
...
The Welch t-test is a statistical method that allows us to test for differences in expression between clusters. It is quickly computed and has good statistical properties for large numbers of cells (Soneson and Robinson 2018). The `findMarkers()` function from the `scran` package provides a convenience wrapper for marker gene identification between groups of cells, based on running `pairwiseTTests` or related functions and passing the result to `combineMarkers()`. We can use the `findMarkers()` function to perform pairwise comparisons between clusters for each gene (the function performs Welch t-tests on the normalized log-expression values for every gene and between every pair of clusters), which returns a list of `DataFrame`s containing ranked candidate markers for each cluster. The top DE genes are likely to be good candidate markers as they can effectively distinguish between cells in different clusters. Importantly, besides looking at the clustering results, we can also apply this approach to identify genes driving differences between other factors of our metadata, such as `cell.type` or `PAG.arearegistration`.
...
plotHeatmap(PAG_sceset_qc_norm_filt_corr_clust,
            ...
            symmetric = TRUE, # If TRUE, make zlim symmetric around zero (compute the maximum absolute value of zlim and multipy by c(-1, 1) to redefine zlim). Default is FALSE.
            color = NULL, # defaults to heatmap color palette: colorRampPalette(rev(brewer.pal(n = 7, name = "RdYlBu")))(100)
            colour_columns_by = c("SNN_clusters_cv2_jaccard_k10", "cell.type", "PAG.arearegistration"), # how the columns should be annotated with colours.
            show_rownames = TRUE, show_colnames = FALSE,
            cluster_rows = TRUE, cluster_cols = FALSE,
...
plotHeatmap(PAG_sceset_qc_norm_filt_corr_clust,
            ...
            columns = order(PAG_sceset_qc_norm_filt_corr_clust$PAGarea_celltype), 
            # subset of columns to show in heatmap (or column order if cluster_cols=FALSE and order_columns_by=NULL). Defaults to NULL.
            exprs_values = "logcounts", # assay of object to be used as expression values for colouring the heatmap.
            center = TRUE, # whether each row should have its mean expression centered at zero prior to plotting. Useful for examining logFC from the average across all cells.
            zlim = c(-5, 5), # upper and lower bounds for the expression values (after centering). Defaults to the range of the expression matrix.
            ...
            symmetric = TRUE, # If TRUE, make zlim symmetric around zero (compute the maximum absolute value of zlim and multipy by c(-1, 1) to redefine zlim). Default is FALSE.
            color = NULL, # defaults to heatmap color palette: colorRampPalette(rev(brewer.pal(n = 7, name = "RdYlBu")))(100)
            colour_columns_by = c("cell.type", "PAGarea_celltype"), # how the columns should be annotated with colours.
            show_rownames = TRUE, show_colnames = FALSE,
            cluster_rows = TRUE, cluster_cols = FALSE,
...
marker_genes_up_all_PAGareas <- findMarkers(PAG_sceset_qc_norm_filt_corr_clust,
                                            groups = PAG_sceset_qc_norm_filt_corr_clust$PAG.arearegistration, # clustering results for each cell.
                                            #test.type = c("t", "wilcox", "binom"), # Pairwise test to perform: t-test "t", Wilcoxon rank sum test "wilcox", binomial test "binom".
                                            pval.type = "all", # how p-values are to be combined across pairwise comparisons for a given group: "any", "some", or "all.
                                            #min.prop = NULL, # minimum proportion of significant comparisons per gene, Defaults to 0.5 when pval.type="some", otherwise zero.
                                            #log.p = FALSE, # whether log-transformed p-values/FDRs should be returned.
...
```{r}
# Create a variable with a value for each group name to iterate through:
range_of_groups <- 1:paste(length(levels(PAG_sceset_qc_norm_filt_corr_clust$PAG.arearegistration)))
# Create a separate table of results for each group and print the top 10 marker genes:
for (chosen_group in range_of_groups) {
...
plotHeatmap(PAG_sceset_qc_norm_filt_corr_clust,
            ...
            columns = order(PAG_sceset_qc_norm_filt_corr_clust$PAG.arearegistration), 
            # subset of columns to show in heatmap (or column order if cluster_cols=FALSE and order_columns_by=NULL). Defaults to NULL.
            exprs_values = "logcounts", # assay of object to be used as expression values for colouring the heatmap.
            center = TRUE, # whether each row should have its mean expression centered at zero prior to plotting. Useful for examining logFC from the average across all cells.
            zlim = c(-5, 5), # upper and lower bounds for the expression values (after centering). Defaults to the range of the expression matrix.
            ...
            symmetric = TRUE, # If TRUE, make zlim symmetric around zero (compute the maximum absolute value of zlim and multipy by c(-1, 1) to redefine zlim). Default is FALSE.
            color = NULL, # defaults to heatmap color palette: colorRampPalette(rev(brewer.pal(n = 7, name = "RdYlBu")))(100)
            colour_columns_by = c("PAG.arearegistration", "cell.type", "PAGarea_celltype"), # how the columns should be annotated with colours.
            show_rownames = TRUE, show_colnames = FALSE,
            cluster_rows = TRUE, cluster_cols = FALSE,
...
##### Across PAGarea_celltype
We can also look at subdivision-specific markers:
```{r}
marker_genes_up_all_PAGarea_celltype <- findMarkers(PAG_sceset_qc_norm_filt_corr_clust,
                                                    groups = PAG_sceset_qc_norm_filt_corr_clust$PAGarea_celltype, # clustering results for each cell.
...
marker_genes_up_all_PAGarea_celltype
```

We can quickly replot the UMAP coloured by PAG subdivision:
```{r}
UMAP_cv2_15_001_PAGsubdivisions <- plotReducedDim(PAG_sceset_qc_norm_filt_corr_clust, dimred = "UMAP_cv2_corrected_15_neighbors_0.01_min_dist",
                                                  colour_by = "PAGarea_celltype", shape_by = "cell.type") + labs(title = "UMAP (cv2) - PAG subdivisions and cell types", 
                                                                                                                     x = "UMAP 1", y = "UMAP 2")
plot(UMAP_cv2_15_001_PAGsubdivisions)
```
...
```{r}
# Create a variable with a value for each group name to iterate through:
range_of_groups <- 1:paste(length(levels(PAG_sceset_qc_norm_filt_corr_clust$PAGarea_celltype)))
# Create a separate table of results for each group and print the top 10 marker genes:
for (chosen_group in range_of_groups) {
  temp_marker_set <- marker_genes_up_all_PAGarea_celltype[[chosen_group]]
  print(paste0("marker_set_up_all_PAGarea_celltype_", chosen_group))
  print(temp_marker_set[1:10, 1:2])
  marker_set <- paste0("marker_set_up_all_PAGarea_celltype_", chosen_group)
  assign(marker_set, marker_genes_up_all_PAGarea_celltype[[chosen_group]])
}
```
...
library(pheatmap)
# dmPAG_VGAT
best_set_up_all_dmPAG_VGAT<- marker_set_up_all_PAGarea_celltype_1[1:30,]
...
# dlPAG_VGAT
best_set_up_all_dlPAG_VGAT<- marker_set_up_all_PAGarea_celltype_2[1:30,]
...
# lPAG_VGAT
best_set_up_all_lPAG_VGAT<- marker_set_up_all_PAGarea_celltype_3[1:30,]
...
# vlPAG_VGAT
best_set_up_all_vlPAG_VGAT<- marker_set_up_all_PAGarea_celltype_4[1:30,]
...
# dmPAG_VGluT2
best_set_up_all_dmPAG_VGluT2<- marker_set_up_all_PAGarea_celltype_5[1:30,]
...
# dlPAG_VGluT2
best_set_up_all_dlPAG_VGluT2<- marker_set_up_all_PAGarea_celltype_6[1:30,]
...
# lPAG_VGluT2
best_set_up_all_lPAG_VGluT2<- marker_set_up_all_PAGarea_celltype_7[1:30,]
...
# vlPAG_VGluT2
best_set_up_all_vlPAG_VGluT2<- marker_set_up_all_PAGarea_celltype_8[1:30,]
...
# "SNN_jaccard_k8_cluster_", "PAG_subdivisions_", "celltype_", "PAGarea_celltype_", "celltype_PAGAPaxis_"
