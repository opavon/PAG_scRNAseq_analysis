...
# DE genes in dlPAG cells
metadata(PAG_sceset_qc_norm_filt_corr_clust_de)$marker_set_dlpag_wilcox_some_up <- rownames(marker_set_dlpag_wilcox_some_up[marker_set_dlpag_wilcox_some_up$FDR<fdr_threshold, ])

# DE genes in lPAG cells
metadata(PAG_sceset_qc_norm_filt_corr_clust_de)$marker_set_lpag_wilcox_some_up <- rownames(marker_set_lpag_wilcox_some_up[marker_set_lpag_wilcox_some_up$FDR<fdr_threshold, ])

# DE genes in vlPAG cells
metadata(PAG_sceset_qc_norm_filt_corr_clust_de)$marker_set_vlpag_wilcox_some_up <- rownames(marker_set_vlpag_wilcox_some_up[marker_set_vlpag_wilcox_some_up$FDR<fdr_threshold, ])

# Check results were appended correctly
names(metadata(PAG_sceset_qc_norm_filt_corr_clust_de))
```
...
```{r}
# Take a look at the results from one fo the comparisons
head(marker_set_vgat_vlpag_wilcox_all_up[marker_set_vgat_vlpag_wilcox_all_up$FDR<0.05,])
```

In the case of VGAT neurons, even when seting `pval.type=some` we find very little or no markers in dm/dl/lpag neurons. We nonetheless keep the few we obtained:
```{r}
# Set false discovery rate threshold
fdr_threshold <- 0.05 # try 0.1, 0.05, 0.01 depending on how stringent you want your results to be
...
## Results from `pval = all`
# DE genes in dmPAG cells
metadata(PAG_sceset_qc_norm_filt_corr_clust_de)$marker_set_vgat_dmpag_wilcox_all_up <- rownames(marker_set_vgat_dmpag_wilcox_all_up[marker_set_vgat_dmpag_wilcox_all_up$FDR<fdr_threshold, ])

# DE genes in dlPAG cells
metadata(PAG_sceset_qc_norm_filt_corr_clust_de)$marker_set_vgat_dlpag_wilcox_all_up <- rownames(marker_set_vgat_dlpag_wilcox_all_up[marker_set_vgat_dlpag_wilcox_all_up$FDR<fdr_threshold, ])

# DE genes in lPAG cells
metadata(PAG_sceset_qc_norm_filt_corr_clust_de)$marker_set_vgat_lpag_wilcox_all_up <- rownames(marker_set_vgat_lpag_wilcox_all_up[marker_set_vgat_lpag_wilcox_all_up$FDR<fdr_threshold, ])

# DE genes in vlPAG cells
metadata(PAG_sceset_qc_norm_filt_corr_clust_de)$marker_set_vgat_vlpag_wilcox_all_up <- rownames(marker_set_vgat_vlpag_wilcox_all_up[marker_set_vgat_vlpag_wilcox_all_up$FDR<fdr_threshold, ])

## Results from `pval = some`
# DE genes in dmPAG cells
metadata(PAG_sceset_qc_norm_filt_corr_clust_de)$marker_set_vgat_dmpag_wilcox_some_up <- rownames(marker_set_vgat_dmpag_wilcox_some_up[marker_set_vgat_dmpag_wilcox_some_up$FDR<fdr_threshold, ])

# DE genes in dlPAG cells
metadata(PAG_sceset_qc_norm_filt_corr_clust_de)$marker_set_vgat_dlpag_wilcox_some_up <- rownames(marker_set_vgat_dlpag_wilcox_some_up[marker_set_vgat_dlpag_wilcox_some_up$FDR<fdr_threshold, ])
...
We next repeat the same for VGluT2 neurons. To make sure we don't carry any variables from previous runs, we reset all the parameters to their default values:
```{r}
library(scran)
# Parameters defaults:
groups <- NULL # clustering results or group for each cell.
...
Even though we are still looking only at the differences between PAG subdivisions within VGluT2+ neurons, in this case we seem to have slightly more potential markers between dmPAG and dlPAG neurons, and nearly none for lPAG and vlPAG. Whereas for VGAT neurons it was the vlPAG that seemed more different than the other three, in the case of VGluT2 neurons it seems to be dm/dlPAG that have more markers than l/vlPAG.
```{r}
# Take a look at the results from one fo the comparisons
head(marker_set_vglut2_dlpag_wilcox_all_up[marker_set_vglut2_dlpag_wilcox_all_up$FDR<0.05,])
```

Again, we store the genes that pass the false discovery rate threshold in the `SingleCellExperiment` object.
```{r}
# Set false discovery rate threshold
fdr_threshold <- 0.05 # try 0.1, 0.05, 0.01 depending on how stringent you want your results to be
...
## Results from `pval = all`
# DE genes in dmPAG cells
metadata(PAG_sceset_qc_norm_filt_corr_clust_de)$marker_set_vglut2_dmpag_wilcox_all_up <- rownames(marker_set_vglut2_dmpag_wilcox_all_up[marker_set_vglut2_dmpag_wilcox_all_up$FDR<fdr_threshold, ])

# DE genes in dlPAG cells
metadata(PAG_sceset_qc_norm_filt_corr_clust_de)$marker_set_vglut2_dlpag_wilcox_all_up <- rownames(marker_set_vglut2_dlpag_wilcox_all_up[marker_set_vglut2_dlpag_wilcox_all_up$FDR<fdr_threshold, ])

# DE genes in lPAG cells
metadata(PAG_sceset_qc_norm_filt_corr_clust_de)$marker_set_vglut2_lpag_wilcox_all_up <- rownames(marker_set_vglut2_lpag_wilcox_all_up[marker_set_vglut2_lpag_wilcox_all_up$FDR<fdr_threshold, ])

# DE genes in vlPAG cells
metadata(PAG_sceset_qc_norm_filt_corr_clust_de)$marker_set_vglut2_vlpag_wilcox_all_up <- rownames(marker_set_vglut2_vlpag_wilcox_all_up[marker_set_vglut2_vlpag_wilcox_all_up$FDR<fdr_threshold, ])

## Results from `pval = some`
# DE genes in dmPAG cells
metadata(PAG_sceset_qc_norm_filt_corr_clust_de)$marker_set_vglut2_dmpag_wilcox_some_up <- rownames(marker_set_vglut2_dmpag_wilcox_some_up[marker_set_vglut2_dmpag_wilcox_some_up$FDR<fdr_threshold, ])

# DE genes in dlPAG cells
metadata(PAG_sceset_qc_norm_filt_corr_clust_de)$marker_set_vglut2_dlpag_wilcox_some_up <- rownames(marker_set_vglut2_dlpag_wilcox_some_up[marker_set_vglut2_dlpag_wilcox_some_up$FDR<fdr_threshold, ])

# DE genes in lPAG cells
metadata(PAG_sceset_qc_norm_filt_corr_clust_de)$marker_set_vglut2_lpag_wilcox_some_up <- rownames(marker_set_vglut2_lpag_wilcox_some_up[marker_set_vglut2_lpag_wilcox_some_up$FDR<fdr_threshold, ])

# DE genes in vlPAG cells
metadata(PAG_sceset_qc_norm_filt_corr_clust_de)$marker_set_vglut2_vlpag_wilcox_some_up <- rownames(marker_set_vglut2_vlpag_wilcox_some_up[marker_set_vglut2_vlpag_wilcox_some_up$FDR<fdr_threshold, ])
...
For the `cluster.ID` comparisons, the number of groups will depend on the parameters we chose. For the graph-based clustering results, we will run the pairwise comparisons both for a low value of `k` (e.g. `k=5`, which resulted in 11 clusters) and for a higher value of `k` (e.g. `k = 13`, which resulted in 8 clusters). For the SC3 clustering results, we will run the comparisons for the case of `k=4`, as it cleanly splits dm/dl/lPAG and vlPAG cells for both inhibitory and excitatory cells. Given that we are still mainly interested in the genes specific to each group, we will set `pval.type=all`. Again, after seeing `all` results in very few potential markers, we could add `some` to the mix. We will also set `direction = "up"` to focus on genes that are more highly expressed in each group. We will again use the Wilcoxon rank sum test and will block for `mouse.sex` to account for any sex specific differences that we are not interested in.

### 7.4.1 | Marker genes for SNN clusters (k = 5)
We can start by refreshing how the clustering results looked like when using `k = 5`:
```{r}
UMAP_SNN_JaccardLouvain_k5
```
  ...
  for (comparison_choice in desired_comparisons){
    ...
    for (direction_choice in desired_directions){
      ...
      for (chosen_group in range_of_groups) {
        ...
        # Export the results to a .csv file:
        write.csv(temp_marker_set[temp_marker_set$FDR<0.1,], file = str_c(path_for_figures, "SNN_clusters/", "PAG_DE_results_", 
                                                                         results_name, marker_set, ".csv"), quote = FALSE, row.names = TRUE)
        # Populate the summary data.frame:        
...
```{r}
# Take a look at the results from one fo the comparisons
head(marker_set_SNN_k5_1_wilcox_some_up[marker_set_SNN_k5_1_wilcox_some_up$FDR<0.05,])[1:2]
```

As it happened in the case of `celltype_PAGarea`, we observe that using the `pval.type=all` finds little or no marker genes that are specific for each cluster. We do have a few when using `pval.type=some`, so we will store the results from both approaches in the `metadata` slot.
```{r}
# Set false discovery rate threshold
fdr_threshold <- 0.05 # try 0.1, 0.05, 0.01 depending on how stringent you want your results to be

## Results from `pval = all`
# DE genes in cluster 1
metadata(PAG_sceset_qc_norm_filt_corr_clust_de)$marker_set_SNN_k5_1_wilcox_all_up <- rownames(marker_set_SNN_k5_1_wilcox_all_up[marker_set_SNN_k5_1_wilcox_all_up$FDR<fdr_threshold, ])
# DE genes in cluster 2
metadata(PAG_sceset_qc_norm_filt_corr_clust_de)$marker_set_SNN_k5_2_wilcox_all_up <- rownames(marker_set_SNN_k5_2_wilcox_all_up[marker_set_SNN_k5_2_wilcox_all_up$FDR<fdr_threshold, ])
# DE genes in cluster 3
metadata(PAG_sceset_qc_norm_filt_corr_clust_de)$marker_set_SNN_k5_3_wilcox_all_up <- rownames(marker_set_SNN_k5_3_wilcox_all_up[marker_set_SNN_k5_3_wilcox_all_up$FDR<fdr_threshold, ])
# DE genes in cluster 4
metadata(PAG_sceset_qc_norm_filt_corr_clust_de)$marker_set_SNN_k5_4_wilcox_all_up <- rownames(marker_set_SNN_k5_4_wilcox_all_up[marker_set_SNN_k5_4_wilcox_all_up$FDR<fdr_threshold, ])
# DE genes in cluster 5
metadata(PAG_sceset_qc_norm_filt_corr_clust_de)$marker_set_SNN_k5_5_wilcox_all_up <- rownames(marker_set_SNN_k5_5_wilcox_all_up[marker_set_SNN_k5_5_wilcox_all_up$FDR<fdr_threshold, ])
# DE genes in cluster 6
metadata(PAG_sceset_qc_norm_filt_corr_clust_de)$marker_set_SNN_k5_6_wilcox_all_up <- rownames(marker_set_SNN_k5_6_wilcox_all_up[marker_set_SNN_k5_6_wilcox_all_up$FDR<fdr_threshold, ])
# DE genes in cluster 7
metadata(PAG_sceset_qc_norm_filt_corr_clust_de)$marker_set_SNN_k5_7_wilcox_all_up <- rownames(marker_set_SNN_k5_7_wilcox_all_up[marker_set_SNN_k5_7_wilcox_all_up$FDR<fdr_threshold, ])
# DE genes in cluster 8
metadata(PAG_sceset_qc_norm_filt_corr_clust_de)$marker_set_SNN_k5_8_wilcox_all_up <- rownames(marker_set_SNN_k5_8_wilcox_all_up[marker_set_SNN_k5_8_wilcox_all_up$FDR<fdr_threshold, ])
# DE genes in cluster 9
metadata(PAG_sceset_qc_norm_filt_corr_clust_de)$marker_set_SNN_k5_9_wilcox_all_up <- rownames(marker_set_SNN_k5_9_wilcox_all_up[marker_set_SNN_k5_9_wilcox_all_up$FDR<fdr_threshold, ])
# DE genes in cluster 11 (we removed cluster 10)
metadata(PAG_sceset_qc_norm_filt_corr_clust_de)$marker_set_SNN_k5_10_wilcox_all_up <- rownames(marker_set_SNN_k5_10_wilcox_all_up[marker_set_SNN_k5_10_wilcox_all_up$FDR<fdr_threshold, ])

## Results from `pval = some`
# DE genes in cluster 1
metadata(PAG_sceset_qc_norm_filt_corr_clust_de)$marker_set_SNN_k5_1_wilcox_some_up <- rownames(marker_set_SNN_k5_1_wilcox_some_up[marker_set_SNN_k5_1_wilcox_some_up$FDR<fdr_threshold, ])
# DE genes in cluster 2
metadata(PAG_sceset_qc_norm_filt_corr_clust_de)$marker_set_SNN_k5_2_wilcox_some_up <- rownames(marker_set_SNN_k5_2_wilcox_some_up[marker_set_SNN_k5_2_wilcox_some_up$FDR<fdr_threshold, ])
# DE genes in cluster 3
metadata(PAG_sceset_qc_norm_filt_corr_clust_de)$marker_set_SNN_k5_3_wilcox_some_up <- rownames(marker_set_SNN_k5_3_wilcox_some_up[marker_set_SNN_k5_3_wilcox_some_up$FDR<fdr_threshold, ])
# DE genes in cluster 4
metadata(PAG_sceset_qc_norm_filt_corr_clust_de)$marker_set_SNN_k5_4_wilcox_some_up <- rownames(marker_set_SNN_k5_4_wilcox_some_up[marker_set_SNN_k5_4_wilcox_some_up$FDR<fdr_threshold, ])
# DE genes in cluster 5
metadata(PAG_sceset_qc_norm_filt_corr_clust_de)$marker_set_SNN_k5_5_wilcox_some_up <- rownames(marker_set_SNN_k5_5_wilcox_some_up[marker_set_SNN_k5_5_wilcox_some_up$FDR<fdr_threshold, ])
# DE genes in cluster 6
metadata(PAG_sceset_qc_norm_filt_corr_clust_de)$marker_set_SNN_k5_6_wilcox_some_up <- rownames(marker_set_SNN_k5_6_wilcox_some_up[marker_set_SNN_k5_6_wilcox_some_up$FDR<fdr_threshold, ])
# DE genes in cluster 7
metadata(PAG_sceset_qc_norm_filt_corr_clust_de)$marker_set_SNN_k5_7_wilcox_some_up <- rownames(marker_set_SNN_k5_7_wilcox_some_up[marker_set_SNN_k5_7_wilcox_some_up$FDR<fdr_threshold, ])
# DE genes in cluster 8
metadata(PAG_sceset_qc_norm_filt_corr_clust_de)$marker_set_SNN_k5_8_wilcox_some_up <- rownames(marker_set_SNN_k5_8_wilcox_some_up[marker_set_SNN_k5_8_wilcox_some_up$FDR<fdr_threshold, ])
# DE genes in cluster 9
metadata(PAG_sceset_qc_norm_filt_corr_clust_de)$marker_set_SNN_k5_9_wilcox_some_up <- rownames(marker_set_SNN_k5_9_wilcox_some_up[marker_set_SNN_k5_9_wilcox_some_up$FDR<fdr_threshold, ])
# DE genes in cluster 11 (we removed cluster 10)
metadata(PAG_sceset_qc_norm_filt_corr_clust_de)$marker_set_SNN_k5_10_wilcox_some_up <- rownames(marker_set_SNN_k5_10_wilcox_some_up[marker_set_SNN_k5_10_wilcox_some_up$FDR<fdr_threshold, ])
...
We repeat the analysis for the results obtained using `k = 13`:
```{r}
UMAP_SNN_JaccardLouvain_k13
```
...
```{r}
# Take a look at the results from one fo the comparisons
head(marker_set_SNN_k13_3_wilcox_all_up[marker_set_SNN_k13_3_wilcox_all_up$FDR<0.05,])[1:2]
```

Once again, some clusters are left without specific markers when using `pval.type=all`, so we will store the results from both approaches into the `metadata` slot.
```{r}
# Set false discovery rate threshold
fdr_threshold <- 0.05 # try 0.1, 0.05, 0.01 depending on how stringent you want your results to be
...
## Results from `pval = all`
# DE genes in cluster 1
metadata(PAG_sceset_qc_norm_filt_corr_clust_de)$marker_set_SNN_k13_1_wilcox_all_up <- rownames(marker_set_SNN_k13_1_wilcox_all_up[marker_set_SNN_k13_1_wilcox_all_up$FDR<fdr_threshold, ])
# DE genes in cluster 2
metadata(PAG_sceset_qc_norm_filt_corr_clust_de)$marker_set_SNN_k13_2_wilcox_all_up <- rownames(marker_set_SNN_k13_2_wilcox_all_up[marker_set_SNN_k13_2_wilcox_all_up$FDR<fdr_threshold, ])
# DE genes in cluster 3
metadata(PAG_sceset_qc_norm_filt_corr_clust_de)$marker_set_SNN_k13_3_wilcox_all_up <- rownames(marker_set_SNN_k13_3_wilcox_all_up[marker_set_SNN_k13_3_wilcox_all_up$FDR<fdr_threshold, ])
# DE genes in cluster 4
metadata(PAG_sceset_qc_norm_filt_corr_clust_de)$marker_set_SNN_k13_4_wilcox_all_up <- rownames(marker_set_SNN_k13_4_wilcox_all_up[marker_set_SNN_k13_4_wilcox_all_up$FDR<fdr_threshold, ])
# DE genes in cluster 5
metadata(PAG_sceset_qc_norm_filt_corr_clust_de)$marker_set_SNN_k13_5_wilcox_all_up <- rownames(marker_set_SNN_k13_5_wilcox_all_up[marker_set_SNN_k13_5_wilcox_all_up$FDR<fdr_threshold, ])
# DE genes in cluster 6
metadata(PAG_sceset_qc_norm_filt_corr_clust_de)$marker_set_SNN_k13_6_wilcox_all_up <- rownames(marker_set_SNN_k13_6_wilcox_all_up[marker_set_SNN_k13_6_wilcox_all_up$FDR<fdr_threshold, ])
# DE genes in cluster 8 (we removed cluster 7)
metadata(PAG_sceset_qc_norm_filt_corr_clust_de)$marker_set_SNN_k13_7_wilcox_all_up <- rownames(marker_set_SNN_k13_7_wilcox_all_up[marker_set_SNN_k13_7_wilcox_all_up$FDR<fdr_threshold, ])

## Results from `pval = some`
# DE genes in cluster 1
metadata(PAG_sceset_qc_norm_filt_corr_clust_de)$marker_set_SNN_k13_1_wilcox_some_up <- rownames(marker_set_SNN_k13_1_wilcox_some_up[marker_set_SNN_k13_1_wilcox_some_up$FDR<fdr_threshold, ])
# DE genes in cluster 2
metadata(PAG_sceset_qc_norm_filt_corr_clust_de)$marker_set_SNN_k13_2_wilcox_some_up <- rownames(marker_set_SNN_k13_2_wilcox_some_up[marker_set_SNN_k13_2_wilcox_some_up$FDR<fdr_threshold, ])
# DE genes in cluster 3
metadata(PAG_sceset_qc_norm_filt_corr_clust_de)$marker_set_SNN_k13_3_wilcox_some_up <- rownames(marker_set_SNN_k13_3_wilcox_some_up[marker_set_SNN_k13_3_wilcox_some_up$FDR<fdr_threshold, ])
# DE genes in cluster 4
metadata(PAG_sceset_qc_norm_filt_corr_clust_de)$marker_set_SNN_k13_4_wilcox_some_up <- rownames(marker_set_SNN_k13_4_wilcox_some_up[marker_set_SNN_k13_4_wilcox_some_up$FDR<fdr_threshold, ])
# DE genes in cluster 5
metadata(PAG_sceset_qc_norm_filt_corr_clust_de)$marker_set_SNN_k13_5_wilcox_some_up <- rownames(marker_set_SNN_k13_5_wilcox_some_up[marker_set_SNN_k13_5_wilcox_some_up$FDR<fdr_threshold, ])
# DE genes in cluster 6
metadata(PAG_sceset_qc_norm_filt_corr_clust_de)$marker_set_SNN_k13_6_wilcox_some_up <- rownames(marker_set_SNN_k13_6_wilcox_some_up[marker_set_SNN_k13_6_wilcox_some_up$FDR<fdr_threshold, ])
# DE genes in cluster 8 (we removed cluster 7)
metadata(PAG_sceset_qc_norm_filt_corr_clust_de)$marker_set_SNN_k13_7_wilcox_some_up <- rownames(marker_set_SNN_k13_7_wilcox_some_up[marker_set_SNN_k13_7_wilcox_some_up$FDR<fdr_threshold, ])

# Check results were appended correctly
names(metadata(PAG_sceset_qc_norm_filt_corr_clust_de))
```

### 7.4.3 | Marker genes for SC3 clusters (k = 4)
Finally, we will run the `findMarkers()` function on the results obtained by clustering with SC3 and `k=4`, as it is the clustering approach that better captures the two main clusters for each excitatory and inhibitory cells. This would be the closest to comparing dm/dl versus l/vl without using the actual metadata, and hopefully yields more markers than those obtained with the other approaches. We can first refresh the clustering results using `k = 4`:
```{r}
UMAP_SC3_k4
```
...
```{r}
# Set the groups to be compared (cluster IDs or metadata slot of interest):
groups <- PAG_sceset_qc_norm_filt_corr_clust_de$sc3_4_clusters

# Set the string to be added to the filename when saving the results:
results_name <- "SC3_clusters_k4_" # one of "SNN_clusters_cv2_jaccard_k5_", "cell_type_", "PAG_subdivision", "celltype_PAGarea_", "celltype_PAGAPaxis_", "VGAT_VGluT2_expression"
...
# Initialise an empty `data.frame` to populate with a summary of marker genes:
SC3_clusters_k4_marker_set_summary <- data.frame()
...
for (test_choice in desired_tests){
  ...
  for (comparison_choice in desired_comparisons){
    ...
    for (direction_choice in desired_directions){
      ...
      # Save results:
      saveRDS(marker_genes, file = str_c(path_for_figures, "SC3_clusters/", results_name, 
                                         "marker_genes_", test, "_", comparisons, "_", direction, ".rds"))
      ...
      for (chosen_group in range_of_groups) {
        # Compose name of variable where results will be assigned to:
        marker_set <- str_c("marker_set_SC3_k4_", levels(groups)[chosen_group], "_", test, "_", comparisons, "_", direction)
        ...
        # Export the results to a .csv file:
        write.csv(temp_marker_set[temp_marker_set$FDR<0.1,], file = str_c(path_for_figures, "SC3_clusters/", "PAG_DE_results_", 
                                                                         results_name, marker_set, ".csv"), quote = FALSE, row.names = TRUE)
        # Populate the summary data.frame:        
        SC3_clusters_k4_marker_set_summary = rbind(SC3_clusters_k4_marker_set_summary,
                                                   data.frame(marker_set = marker_set,
                                                              group = levels(groups)[chosen_group],
                                                              test = test,
                                                              comparisons = comparisons,
...
# Check out the summary: 
SC3_clusters_k4_marker_set_summary
```

Indeed, this time we get a few more markers with the `pval.type=all`, suggesting that we may have been overclustering with the other approaches.
```{r}
# Take a look at the results from one fo the comparisons
head(marker_set_SC3_k4_1_wilcox_all_up[marker_set_SC3_k4_1_wilcox_all_up$FDR<0.05,]) # VGluT2 cluster (dm/dl)
head(marker_set_SC3_k4_4_wilcox_all_up[marker_set_SC3_k4_4_wilcox_all_up$FDR<0.05,]) # VGluT2 cluster (l/vl)

head(marker_set_SC3_k4_2_wilcox_all_up[marker_set_SC3_k4_2_wilcox_all_up$FDR<0.05,]) # VGAT cluster (dm/dl)
head(marker_set_SC3_k4_3_wilcox_all_up[marker_set_SC3_k4_3_wilcox_all_up$FDR<0.05,]) # VGAT cluster (l/vl)
```

We can quickly examine whether there are any interesting genes
```{r}
# Set the FDR threshold you want to look at.
fdr_threshold <- 0.05 # try 0.1, 0.05, 0.01 depending on how stringent you want your results to be

VGluT2_dmdl_cluster_de_genes <- rownames(marker_set_SC3_k4_1_wilcox_all_up[marker_set_SC3_k4_1_wilcox_all_up$FDR<fdr_threshold, ])
VGluT2_lvl_cluster_de_genes <- rownames(marker_set_SC3_k4_4_wilcox_all_up[marker_set_SC3_k4_4_wilcox_all_up$FDR<fdr_threshold, ])
VGAT_dmdl_cluster_de_genes <- rownames(marker_set_SC3_k4_2_wilcox_all_up[marker_set_SC3_k4_2_wilcox_all_up$FDR<fdr_threshold, ])
VGAT_lvl_cluster_de_genes <- rownames(marker_set_SC3_k4_3_wilcox_all_up[marker_set_SC3_k4_3_wilcox_all_up$FDR<fdr_threshold, ])

cat("--- Ion Channels ---\n")
cat("--- VGluT2 cells (dm/dl cluster)\n")
VGluT2_dmdl_cluster_de_genes[VGluT2_dmdl_cluster_de_genes %in% metadata(PAG_sceset_qc_norm_filt_corr_clust_de)$genes.ionchannels]
cat("--- VGluT2 cells (l/vl cluster)\n")
VGluT2_lvl_cluster_de_genes[VGluT2_lvl_cluster_de_genes %in% metadata(PAG_sceset_qc_norm_filt_corr_clust_de)$genes.ionchannels]
cat("--- VGAT cells (dm/dl cluster)\n")
VGAT_dmdl_cluster_de_genes[VGAT_dmdl_cluster_de_genes %in% metadata(PAG_sceset_qc_norm_filt_corr_clust_de)$genes.ionchannels]
cat("--- VGAT cells (l/vl cluster)\n")
VGAT_lvl_cluster_de_genes[VGAT_lvl_cluster_de_genes %in% metadata(PAG_sceset_qc_norm_filt_corr_clust_de)$genes.ionchannels]

cat("\n\n--- Neuromodulators and Neuropeptides ---\n")
cat("--- VGluT2 cells (dm/dl cluster)\n")
VGluT2_dmdl_cluster_de_genes[VGluT2_dmdl_cluster_de_genes %in% metadata(PAG_sceset_qc_norm_filt_corr_clust_de)$genes.NeuromodulatorsPeptides]
cat("--- VGluT2 cells (l/vl cluster)\n")
VGluT2_lvl_cluster_de_genes[VGluT2_lvl_cluster_de_genes %in% metadata(PAG_sceset_qc_norm_filt_corr_clust_de)$genes.NeuromodulatorsPeptides]
cat("--- VGAT cells (dm/dl cluster)\n")
VGAT_dmdl_cluster_de_genes[VGAT_dmdl_cluster_de_genes %in% metadata(PAG_sceset_qc_norm_filt_corr_clust_de)$genes.NeuromodulatorsPeptides]
cat("--- VGAT cells (l/vl cluster)\n")
VGAT_lvl_cluster_de_genes[VGAT_lvl_cluster_de_genes %in% metadata(PAG_sceset_qc_norm_filt_corr_clust_de)$genes.NeuromodulatorsPeptides]

cat("\n\n--- Monoamines ---\n")
cat("--- VGluT2 cells (dm/dl cluster)\n")
VGluT2_dmdl_cluster_de_genes[VGluT2_dmdl_cluster_de_genes %in% metadata(PAG_sceset_qc_norm_filt_corr_clust_de)$genes.Monoamines]
cat("--- VGluT2 cells (l/vl cluster)\n")
VGluT2_lvl_cluster_de_genes[VGluT2_lvl_cluster_de_genes %in% metadata(PAG_sceset_qc_norm_filt_corr_clust_de)$genes.Monoamines]
cat("--- VGAT cells (dm/dl cluster)\n")
VGAT_dmdl_cluster_de_genes[VGAT_dmdl_cluster_de_genes %in% metadata(PAG_sceset_qc_norm_filt_corr_clust_de)$genes.Monoamines]
cat("--- VGAT cells (l/vl cluster)\n")
VGAT_lvl_cluster_de_genes[VGAT_lvl_cluster_de_genes %in% metadata(PAG_sceset_qc_norm_filt_corr_clust_de)$genes.Monoamines]

cat("\n\n--- Transporters ---\n")
cat("--- VGluT2 cells (dm/dl cluster)\n")
VGluT2_dmdl_cluster_de_genes[VGluT2_dmdl_cluster_de_genes %in% metadata(PAG_sceset_qc_norm_filt_corr_clust_de)$genes.Transporters]
cat("--- VGluT2 cells (l/vl cluster)\n")
VGluT2_lvl_cluster_de_genes[VGluT2_lvl_cluster_de_genes %in% metadata(PAG_sceset_qc_norm_filt_corr_clust_de)$genes.Transporters]
cat("--- VGAT cells (dm/dl cluster)\n")
VGAT_dmdl_cluster_de_genes[VGAT_dmdl_cluster_de_genes %in% metadata(PAG_sceset_qc_norm_filt_corr_clust_de)$genes.Transporters]
cat("--- VGAT cells (l/vl cluster)\n")
VGAT_lvl_cluster_de_genes[VGAT_lvl_cluster_de_genes %in% metadata(PAG_sceset_qc_norm_filt_corr_clust_de)$genes.Transporters]

cat("\n\n--- Transcription Factors ---\n")
cat("--- VGluT2 cells (dm/dl cluster)\n")
VGluT2_dmdl_cluster_de_genes[VGluT2_dmdl_cluster_de_genes %in% metadata(PAG_sceset_qc_norm_filt_corr_clust_de)$genes.Transcriptionfactors]
cat("--- VGluT2 cells (l/vl cluster)\n")
VGluT2_lvl_cluster_de_genes[VGluT2_lvl_cluster_de_genes %in% metadata(PAG_sceset_qc_norm_filt_corr_clust_de)$genes.Transcriptionfactors]
cat("--- VGAT cells (dm/dl cluster)\n")
VGAT_dmdl_cluster_de_genes[VGAT_dmdl_cluster_de_genes %in% metadata(PAG_sceset_qc_norm_filt_corr_clust_de)$genes.Transcriptionfactors]
cat("--- VGAT cells (l/vl cluster)\n")
VGAT_lvl_cluster_de_genes[VGAT_lvl_cluster_de_genes %in% metadata(PAG_sceset_qc_norm_filt_corr_clust_de)$genes.Transcriptionfactors]
```

We store the results in the `metadata`:
```{r}
# Set false discovery rate threshold
fdr_threshold <- 0.05 # try 0.1, 0.05, 0.01 depending on how stringent you want your results to be
...
## Results from `pval = all`
# DE genes in cluster 1
metadata(PAG_sceset_qc_norm_filt_corr_clust_de)$marker_set_SC3_k4_1_wilcox_all_up <- rownames(marker_set_SC3_k4_1_wilcox_all_up[marker_set_SC3_k4_1_wilcox_all_up$FDR<fdr_threshold, ])
# DE genes in cluster 2
metadata(PAG_sceset_qc_norm_filt_corr_clust_de)$marker_set_SC3_k4_2_wilcox_all_up <- rownames(marker_set_SC3_k4_2_wilcox_all_up[marker_set_SC3_k4_2_wilcox_all_up$FDR<fdr_threshold, ])
# DE genes in cluster 3
metadata(PAG_sceset_qc_norm_filt_corr_clust_de)$marker_set_SC3_k4_3_wilcox_all_up <- rownames(marker_set_SC3_k4_3_wilcox_all_up[marker_set_SC3_k4_3_wilcox_all_up$FDR<fdr_threshold, ])
# DE genes in cluster 4
metadata(PAG_sceset_qc_norm_filt_corr_clust_de)$marker_set_SC3_k44_wilcox_all_up <- rownames(marker_set_SC3_k4_4_wilcox_all_up[marker_set_SC3_k4_4_wilcox_all_up$FDR<fdr_threshold, ])

# Check results were appended correctly
names(metadata(PAG_sceset_qc_norm_filt_corr_clust_de))
```

## Step 7.5 | Save the SingleCellExperiment object
After extracting and saving the differential expression results obtained with `findMarkers()`, we save the `SingleCellExperiment` object so we can use it for data visualisation.
```{r}
