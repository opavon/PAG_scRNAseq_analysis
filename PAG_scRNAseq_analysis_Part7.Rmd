...
subtitle: "Part VII: gene set enrichment analysis"
author:
...
## Step 11.1 | EGSEA: Easy and efficient ensemble gene set testing
...
### 11.1.1 | Exploring, selecting and indexing gene set collections
...
### 11.1.3 | Ensemble testing with EGSEA
...
### 11.1.4 | Reporting EGSEA results
...
### 11.1.5 | Visualizing results at the gene set level
...
### 11.1.6 | Visualizing results at the experiment level
...
### 11.1.7 | Creating an HTML report of the results
...
### 11.3.1 | Running bigSCale2
```{r}
data(PAG_sceset_qc)
```

bigSCale2 can be run with one single command, and we can set `speed.preset="slow"` for mazimun accuracy. Once we've run this, the results are stored in the `SingleCellExperiment` object.
```{r}
PAG_sceset_qc = bigscale(PAG_sceset_qc, speed.preset='fast', clustering='recursive') 
...
### 11.3.2 | Visualising the results
...
```{r}
viewGeneViolin(PAG_sceset_qc, 'Aqp4')
```
...
```
If you want to color the cell according to some custom annotation you can pass a `factor` variable in place of a gene name. If you want to visualize a UMAP plot, first compute the UMAP data with `PAG_sceset_qc = storeUMAP(PAG_sceset_qc)` and then `viewReduced(PAG_sceset_qc, method = 'UMAP')`.
...
### 11.3.3 | Gene Regulatory Networks
...
results.vgat = compute.network(expr.data = PAG_sceset_qc$VGAT, gene.names = gene.names, clustering = 'recursive')
results.vglut2 = compute.network(expr.data = PAG_sceset_qc$VGluT2, gene.names = gene.names, clustering = 'recursive')
```
