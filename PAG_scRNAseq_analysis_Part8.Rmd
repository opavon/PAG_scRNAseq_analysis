...
```{r}
PAG_seurat <- subset(PAG_seurat, subset = nFeature_RNA > 200 & nFeature_RNA < 2500 & percent.mt < 5)
```
...
```{r}
PAG_seurat <- RunUMAP(PAG_seurat, dims = 1:10)
DimPlot(PAG_seurat, reduction = "umap")
```
...
cluster5.markers <- FindMarkers(PAG_seurat, ident.1 = 5, ident.2 = c(0, 3), min.pct = 0.25)
...
```{r}
VlnPlot(PAG_seurat, features = c("MS4A1", "CD79A"))

# you can plot raw counts as well
VlnPlot(PAG_seurat, features = c("NKG7", "PF4"), slot = "counts", log = TRUE)
