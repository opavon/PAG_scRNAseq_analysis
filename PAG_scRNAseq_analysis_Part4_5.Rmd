...
for(n in reducedDimNames(PAG_sceset_qc_norm_filt)[2:length(reducedDimNames(PAG_sceset_qc_norm_filt))]) {
...
dim(reducedDim(PAG_sceset_qc_norm_filt, "PCA_denoise")) # Cells are rows, PCs are columns
...
for(n in reducedDimNames(PAG_sceset_qc_norm_filt)[2:length(reducedDimNames(PAG_sceset_qc_norm_filt))]) {
  PAG_sceset_qc_norm_filt <- denoisePCA(PAG_sceset_qc_norm_filt,
                                        ...
                                        name = "PCA_denoise")
  print(ncol(reducedDim(PAG_sceset_qc_norm_filt, "PCA_denoise")))
}
...
plotReducedDim(PAG_sceset_qc_norm_filt,
               dimred = "PCA_HVG_var_logcounts",
               ncomponents = 4,
               colour_by = "PAGarea_celltype") # "batch.processing"
```
...
```{r}
set.seed(1991)
range_of_perplexities = list(5, 10, 20, 50, 80)
for(n in range_of_perplexities) {
    print(
        plotTSNE(PAG_sceset_qc_norm_filt,
                 rerun = TRUE, 
                 ncomponents = 2, 
                 colour_by = "PAG.arearegistration", # "batch.processing"
                 shape_by = "cell.type",
                 size_by = "detected",
                 run_args = list(feature_set = metadata(PAG_sceset_qc_norm_filt)$hvg_var_out_no_spikes_block_filt, exprs_values = "corrected", 
                                 use_dimred = "PCA_HVG_var", n_dimred = 17, perplexity = n, theta = 0.5),
                 name = paste0("tSNE_", n)
...
Once we have the right settings, we can use `runTSNE()` to store the t-SNE results inside our `SingleCellExperiment` object. This avoids repeating the calculations whenever we want to create a new plot with `plotReducedDim()`. 
```{r}
set.seed(1991)
PAG_sceset_qc_norm_filt <- runTSNE(PAG_sceset_qc_norm_filt,
                                   dimred="PCA_HVG_var_corrected", n_dimred = 17,
                                   feature_set = metadata(PAG_sceset_qc_norm_filt)$hvg_var_out_no_spikes_block_filt, 
...
PAG_sceset_qc_norm_filt <- runUMAP(PAG_sceset_qc_norm_filt, 
                                        dimred = "PCA_HVG_var_logcounts",
                                        n_dimred = 17, # Choose the number of dimensions tu use from the specificed dimred
                                        feature_set = metadata(PAG_sceset_qc_norm_filt)$hvg_var_out_no_spikes_filt, 
                                        exprs_values = "logcounts",
                                        n_neighbors = 15,
                                        metric = "euclidean",
                                        learning_rate = 1,
                                        name = "UMAP_var_logcounts")

plotReducedDim(PAG_sceset_qc_norm_filt, 
               dimred = "UMAP_var_logcounts", 
               colour_by = "PAG.arearegistration",
               shape_by = "cell.type") # You can also choose any other metadata (e.g. "batch.processing"), a specific gene (e.g. "Gad1"), or shape_by = "cell.type"

PAG_sceset_qc_norm_filt <- runUMAP(PAG_sceset_qc_norm_filt, 
                                        dimred = "PCA_HVG_cv2_logcounts",
                                        n_dimred = 17, # Choose the number of dimensions tu use from the specificed dimred
                                        feature_set = metadata(PAG_sceset_qc_norm_filt)$hvg_cv2_out_no_spikes_filt, 
                                        exprs_values = "logcounts",
                                        n_neighbors = 15,
                                        metric = "euclidean",
                                        learning_rate = 1,
                                        name = "UMAP_cv2_logcounts")

plotReducedDim(PAG_sceset_qc_norm_filt, 
               dimred = "UMAP_cv2_logcounts", 
               colour_by = "PAG.arearegistration",
               shape_by = "cell.type") # You can also choose any other metadata (e.g. "batch.processing"), a specific gene (e.g. "Gad1"), or shape_by = "cell.type"
```
