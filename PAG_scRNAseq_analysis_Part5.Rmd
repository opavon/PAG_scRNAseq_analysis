...
### 7.1.1 | Prepare SingleCellExperiment object for SC3
...
```{r}
PAG_sceset_qc_norm_filt <- sc3_prepare(PAG_sceset_qc_norm_filt,
...
str(metadata(PAG_sceset_qc_norm_filt)$sc3)
```
...
#rowData(PAG_sceset_qc_norm_filt)$sc3_gene_filter <- rownames(PAG_sceset_qc_norm_filt) %in% metadata(PAG_sceset_qc_norm_filt)$genes.ionchannels

# Use Neuropeptides and Neuromodulators Genes:
#rowData(PAG_sceset_qc_norm_filt)$sc3_gene_filter <- rownames(PAG_sceset_qc_norm_filt) %in% metadata(PAG_sceset_qc_norm_filt)$genes.NeuromodulatorsPeptides
```
...
### 7.1.3 | Calculate distances and transformations
...
```{r}
PAG_sceset_qc_norm_filt <- sc3_calc_dists(PAG_sceset_qc_norm_filt)
names(metadata(PAG_sceset_qc_norm_filt)$sc3$distances)
```
...
```{r}
PAG_sceset_qc_norm_filt <- sc3_calc_transfs(PAG_sceset_qc_norm_filt)
names(metadata(PAG_sceset_qc_norm_filt)$sc3$transformations)
metadata(PAG_sceset_qc_norm_filt)$sc3$distances
```
...
```{r}
PAG_sceset_qc_norm_filt <- sc3_kmeans(PAG_sceset_qc_norm_filt,
                                      ks = 2:10)
names(metadata(PAG_sceset_qc_norm_filt)$sc3$kmeans)
```

### 7.1.5 | Calculate consensus
...
```{r}
PAG_sceset_qc_norm_filt <- sc3_calc_consens(PAG_sceset_qc_norm_filt)
names(metadata(PAG_sceset_qc_norm_filt)$sc3$consensus)
names(metadata(PAG_sceset_qc_norm_filt)$sc3$consensus$`1`)
metadata(PAG_sceset_qc_norm_filt)$sc3$kmeans
```

### 7.1.6 | Calculate biology
...
```{r}
PAG_sceset_qc_norm_filt <- sc3_calc_biology(PAG_sceset_qc_norm_filt, 
                                            ks = 2:10)
```

### 7.1.7 | Running SC3 with the wrapper function [OPTIONAL ALTERNATIVE]
...
## STEP 7.2 | clusterExperiment [TO TRY]
## STEP 7.3 | bigSCale2
...
```
## STEP 7.4 | Hierarchical clustering on Euclidean distances
...
## STEP 7.5 | Identifying marker genes between subpopulations
