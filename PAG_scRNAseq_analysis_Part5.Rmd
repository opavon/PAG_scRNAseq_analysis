...
plotExplanatoryVariables(PAG_sceset_qc_norm_filt,
                         ...
                         theme_size = 11
                         ) + ggtitle("Counts log-normalized with Scran")
...
PCA_HVG_var_counts <- plotReducedDim(PAG_sceset_qc_norm_filt,
                                     ...
                                     size_by = "detected"
                                     ) + ggtitle("PCA using HVG (var) and raw counts")
...
PCA_HVG_var_logcounts_raw <- plotReducedDim(PAG_sceset_qc_norm_filt,
                                            ...
                                            shape_by = "cell.type", # "mouse.id"
                                            size_by = "detected"
                                            ) + ggtitle("PCA using HVG (var) and log2 raw counts") 
...
PCA_HVG_var_logcounts <- plotReducedDim(PAG_sceset_qc_norm_filt,
                                        ...
                                        shape_by = "cell.type", # "mouse.id"
                                        size_by = "detected"
                                        ) + ggtitle("PCA using HVG (var) and log-normalized counts") 
...
library(patchwork)
batch_correction_PCA_var_composed <- PCA_HVG_var_counts + PCA_HVG_var_counts + PCA_HVG_var_logcounts + PCA_HVG_var_corrected +
  plot_annotation(title = "Comparison of effects of normalization and batch correction with PCA (var)", tag_levels = "A", 
...
PCA_HVG_cv2_counts <- plotReducedDim(PAG_sceset_qc_norm_filt,
                                     ...
                                     shape_by = "cell.type", # "mouse.id"
                                     size_by = "detected"
                                     ) + ggtitle("PCA using HVG (cv2) and raw counts")
...
PCA_HVG_cv2_logcounts_raw <- plotReducedDim(PAG_sceset_qc_norm_filt,
                                            ...
                                            shape_by = "cell.type", # "mouse.id"
                                            size_by = "detected"
                                            ) + ggtitle("PCA using HVG (cv2) and log2 raw counts") 
...
PCA_HVG_cv2_logcounts <- plotReducedDim(PAG_sceset_qc_norm_filt,
                                        ...
                                        shape_by = "cell.type", # "mouse.id"
                                        size_by = "detected"
                                        ) + ggtitle("PCA using HVG (cv2) and log-normalized counts") 
...
batch_correction_PCA_cv2_composed <- PCA_HVG_cv2_counts + PCA_HVG_cv2_logcounts_raw + PCA_HVG_cv2_logcounts + PCA_HVG_cv2_corrected +
  plot_annotation(title = "Comparison of effects of normalization and batch correction with PCA (cv2)", tag_levels = "A", 
                  theme = theme(plot.title = element_text(size = 18), plot.tag = element_text(size = 14))) + 
  plot_layout(guides = "collect")
...
explanatory_variables_composed <- explanatory_variables_counts + explanatory_variables_logcounts_raw + explanatory_variables_logcounts + explanatory_variables_corrected +
  plot_annotation(title = "Explanatory variables after normalization and batch correction", tag_levels = "A", 
                  theme = theme(plot.title = element_text(size = 18), plot.tag = element_text(size = 14)))
...
plot(attr(chosen_slot, "percentVar"),
     ...
     sub = sprintf("Calculated Elbow: %s PCs", chosen_elbow),
     col = tmpCol, type = "p", pch = 19, cex = 1.25, las = 1)
title(main = "Scree plot for PCA from HVG (var) using log-normalized counts", adj = 0)
abline(v = chosen_elbow, col = "red", lty = "dashed", lwd = 2)
abline(v = 25, col = "gray80", lty = "dotted", lwd = 2)
dev.off()
...
plot(attr(chosen_slot, "percentVar"),
     ...
     sub = sprintf("Calculated Elbow: %s PCs", chosen_elbow),
     col = tmpCol, type = "p", pch = 19, cex = 1.25, las = 1)
title(main = "Scree plot for PCA from HVG (var) after batch correction", adj = 0)
abline(v = chosen_elbow, col = "red", lty = "dashed", lwd = 2)
abline(v = 25, col = "gray80", lty = "dotted", lwd = 2)
dev.off()
...
plot(attr(chosen_slot, "percentVar"),
     ...
     sub = sprintf("Calculated Elbow: %s PCs", chosen_elbow),
     col = tmpCol, type = "p", pch = 19, cex = 1.25, las = 1)
title(main = "Scree plot for PCA from HVG (cv2) using log-normalized counts", adj = 0)
abline(v = chosen_elbow, col = "red", lty = "dashed", lwd = 2)
abline(v = 25, col = "gray80", lty = "dotted", lwd = 2)
dev.off()
...
plot(attr(chosen_slot, "percentVar"),
     ...
     sub = sprintf("Calculated Elbow: %s PCs", chosen_elbow),
     col = tmpCol, type = "p", pch = 19, cex = 1.25, las = 1)
title(main = "Scree plot for PCA from HVG (cv2) after batch correction", adj = 0)
abline(v = chosen_elbow, col = "red", lty = "dashed", lwd = 2)
abline(v = 25, col = "gray80", lty = "dotted", lwd = 2)
dev.off()
```
...
plot(choices$n.pcs, choices$n.clusters, 
     xlab = "PC #", ylab = "Number of clusters", sub = sprintf("Calculated PCs to keep: %s PCs", metadata(choices)$chosen),
     col = tmpCol, type = "p", pch = 19, cex = 1.25, las = 1)
title(main = "Chosen PCs for PCA from HVG (var) using log-normalized counts", adj = 0)
abline(a = 1, b = 1, col = "red")
abline(v = metadata(choices)$chosen, col = "grey80", lty = 2, lwd = 2)
abline(v = 25, col = "aquamarine3", lty = "dotted", lwd = 2)
dev.off()
...
plot(choices$n.pcs, choices$n.clusters, 
     xlab = "PC #", ylab = "Number of clusters", sub = sprintf("Calculated PCs to keep: %s PCs", metadata(choices)$chosen),
     col = tmpCol, type = "p", pch = 19, cex = 1.25, las = 1)
title(main = "Chosen PCs for PCA from HVG (cv2) using log-normalized counts", adj = 0)
abline(a = 1, b = 1, col = "red")
abline(v = metadata(choices)$chosen, col = "grey80", lty = 2, lwd = 2)
abline(v = 25, col = "aquamarine3", lty = "dotted", lwd = 2)
dev.off()
...
After exploring the different options, we will try to make a decision on how many PCs to keep from now on. Looking at the results from the `corrected` assay we can see that the elbow method suggests keeping 2 PCs when we use HVGs from Variance modelling and 50 when using HVGs from CV2 modelling. However, by looking at the plots we would say that perhaps 30 PCs would be a value after which adding more PCs doesn't seem to make much difference on the variance explained (i.e. the scree plot becomes almost flat), whereas 10 or 15 would be a perhaps more appropriate elbow. If we look at the population structure approach, the suggested values are 12 and 14, respectively, although anything between 20 and 30 seems to output similar results. We will try to settle for a middle point in an attempt to achieve a balance between the extremes: taking more than 30 PCs makes little sense as we are probably mainly adding noise by then, whereas keeping less than 15 PCs would probably discard some potentially interesting biological information. In an attempt to retain a bit more of information, we will keep the first 25 PCs. 
...
```{r}
# Prepare PCA plots
PCA_HVG_var_corrected_celltype <- plotReducedDim(PAG_sceset_qc_norm_filt, dimred = "PCA_HVG_var_corrected",
                                                 by_exprs_values = "corrected", colour_by = "cell.type") + ggtitle("HVG (var)")
PCA_HVG_var_corrected_PAGarea <- plotReducedDim(PAG_sceset_qc_norm_filt, dimred = "PCA_HVG_var_corrected",
                                                by_exprs_values = "corrected", colour_by = "PAG.area") + ggtitle("HVG (var)")
PCA_HVG_var_corrected_celltype_PAGarea <- plotReducedDim(PAG_sceset_qc_norm_filt, dimred = "PCA_HVG_var_corrected",
                                                         by_exprs_values = "corrected", colour_by = "celltype_PAGarea") + ggtitle("HVG (var)")
PCA_HVG_var_corrected_cd68 <- plotReducedDim(PAG_sceset_qc_norm_filt, dimred = "PCA_HVG_var_corrected",
                                             by_exprs_values = "corrected", colour_by = "Cd68") + ggtitle("HVG (var)")

PCA_HVG_cv2_corrected_celltype <- plotReducedDim(PAG_sceset_qc_norm_filt, dimred = "PCA_HVG_cv2_corrected",
                                                 colour_by = "cell.type") + ggtitle("HVG (cv2)")
PCA_HVG_cv2_corrected_PAGarea <- plotReducedDim(PAG_sceset_qc_norm_filt, dimred = "PCA_HVG_cv2_corrected",
                                                colour_by = "PAG.area") + ggtitle("HVG (cv2)")
PCA_HVG_cv2_corrected_celltype_PAGarea <- plotReducedDim(PAG_sceset_qc_norm_filt, dimred = "PCA_HVG_cv2_corrected",
                                                         colour_by = "celltype_PAGarea") + ggtitle("HVG (cv2)")
PCA_HVG_cv2_corrected_cd68 <- plotReducedDim(PAG_sceset_qc_norm_filt, dimred = "PCA_HVG_cv2_corrected",
                                             by_exprs_values = "corrected", colour_by = "Cd68") + ggtitle("HVG (cv2)")
...
PCA_HVG_corrected_composed <- ((PCA_HVG_var_corrected_celltype + PCA_HVG_cv2_corrected_celltype) / 
  ...
  plot_annotation(title = "PCA using batch corrected log-normalized counts", tag_levels = "A", 
                  theme = theme(plot.title = element_text(size = 18), plot.tag = element_text(size = 14)))
...
tSNE_HVG_var_corrected_5 <- plotReducedDim(PAG_sceset_qc_norm_filt, dimred = "tSNE_var_corrected_perplexity_5",
                                           colour_by = "PAG.area", shape_by = "cell.type") + labs(title = "Perplexity = 5", x = "t-SNE 1", y = "t-SNE 2")

tSNE_HVG_var_corrected_10 <- plotReducedDim(PAG_sceset_qc_norm_filt, dimred = "tSNE_var_corrected_perplexity_10",
                                            colour_by = "PAG.area", shape_by = "cell.type") + labs(title = "Perplexity = 10", x = "t-SNE 1", y = "t-SNE 2")

tSNE_HVG_var_corrected_15 <- plotReducedDim(PAG_sceset_qc_norm_filt, dimred = "tSNE_var_corrected_perplexity_15",
                                            colour_by = "PAG.area", shape_by = "cell.type") + labs(title = "Perplexity = 15", x = "t-SNE 1", y = "t-SNE 2")

tSNE_HVG_var_corrected_20 <- plotReducedDim(PAG_sceset_qc_norm_filt, dimred = "tSNE_var_corrected_perplexity_20",
                                            colour_by = "PAG.area", shape_by = "cell.type") + labs(title = "Perplexity = 20", x = "t-SNE 1", y = "t-SNE 2")

tSNE_HVG_var_corrected_30 <- plotReducedDim(PAG_sceset_qc_norm_filt, dimred = "tSNE_var_corrected_perplexity_30",
                                            colour_by = "PAG.area", shape_by = "cell.type") + labs(title = "Perplexity = 30", x = "t-SNE 1", y = "t-SNE 2")

tSNE_HVG_var_corrected_40 <- plotReducedDim(PAG_sceset_qc_norm_filt, dimred = "tSNE_var_corrected_perplexity_40",
                                            colour_by = "PAG.area", shape_by = "cell.type") + labs(title = "Perplexity = 40", x = "t-SNE 1", y = "t-SNE 2")

tSNE_HVG_var_corrected_50 <- plotReducedDim(PAG_sceset_qc_norm_filt, dimred = "tSNE_var_corrected_perplexity_50",
                                            colour_by = "PAG.area", shape_by = "cell.type") + labs(title = "Perplexity = 50", x = "t-SNE 1", y = "t-SNE 2")
  
tSNE_HVG_var_corrected_75 <- plotReducedDim(PAG_sceset_qc_norm_filt, dimred = "tSNE_var_corrected_perplexity_75",
                                            colour_by = "PAG.area", shape_by = "cell.type") + labs(title = "Perplexity = 75", x = "t-SNE 1", y = "t-SNE 2")
...
tSNE_HVG_var_corrected <- (tSNE_HVG_var_corrected_5 + tSNE_HVG_var_corrected_10 + tSNE_HVG_var_corrected_15 + tSNE_HVG_var_corrected_20 +
  ...
  plot_annotation(title = "tSNE plots at different perplexities (HVG - var)", tag_levels = "A", 
                  theme = theme(plot.title = element_text(size = 18), plot.tag = element_text(size = 14))) + 
  plot_layout(ncol = 2, guides = "collect")

tSNE_HVG_var_corrected

ggsave(filename = str_c(date, "_tSNE_HVG_var_corrected_composed.pdf"),
...
tSNE_HVG_cv2_corrected_5 <- plotReducedDim(PAG_sceset_qc_norm_filt, dimred = "tSNE_cv2_corrected_perplexity_5",
                                           colour_by = "PAG.area", shape_by = "cell.type") + labs(title = "Perplexity = 5", x = "t-SNE 1", y = "t-SNE 2")

tSNE_HVG_cv2_corrected_10 <- plotReducedDim(PAG_sceset_qc_norm_filt, dimred = "tSNE_cv2_corrected_perplexity_10",
                                            colour_by = "PAG.area", shape_by = "cell.type") + labs(title = "Perplexity = 10", x = "t-SNE 1", y = "t-SNE 2")

tSNE_HVG_cv2_corrected_15 <- plotReducedDim(PAG_sceset_qc_norm_filt, dimred = "tSNE_cv2_corrected_perplexity_15",
                                            colour_by = "PAG.area", shape_by = "cell.type") + labs(title = "Perplexity = 15", x = "t-SNE 1", y = "t-SNE 2")

tSNE_HVG_cv2_corrected_20 <- plotReducedDim(PAG_sceset_qc_norm_filt, dimred = "tSNE_cv2_corrected_perplexity_20",
                                            colour_by = "PAG.area", shape_by = "cell.type") + labs(title = "Perplexity = 20", x = "t-SNE 1", y = "t-SNE 2")

tSNE_HVG_cv2_corrected_30 <- plotReducedDim(PAG_sceset_qc_norm_filt, dimred = "tSNE_cv2_corrected_perplexity_30",
                                            colour_by = "PAG.area", shape_by = "cell.type") + labs(title = "Perplexity = 30", x = "t-SNE 1", y = "t-SNE 2")

tSNE_HVG_cv2_corrected_40 <- plotReducedDim(PAG_sceset_qc_norm_filt, dimred = "tSNE_cv2_corrected_perplexity_40",
                                           colour_by = "PAG.area", shape_by = "cell.type") + labs(title = "Perplexity = 40", x = "t-SNE 1", y = "t-SNE 2")

tSNE_HVG_cv2_corrected_50 <- plotReducedDim(PAG_sceset_qc_norm_filt, dimred = "tSNE_cv2_corrected_perplexity_50",
                                            colour_by = "PAG.area", shape_by = "cell.type") + labs(title = "Perplexity = 50", x = "t-SNE 1", y = "t-SNE 2")

tSNE_HVG_cv2_corrected_75 <- plotReducedDim(PAG_sceset_qc_norm_filt, dimred = "tSNE_cv2_corrected_perplexity_75",
                                            colour_by = "PAG.area", shape_by = "cell.type") + labs(title = "Perplexity = 75", x = "t-SNE 1", y = "t-SNE 2")
...
ggsave(filename = str_c(date, "_tSNE_HVG_cv2_corrected_composed.pdf"),
...
#### n_neighbors = 5
UMAP_var_corrected_5_neighbors_0.01_min_dist <- plotReducedDim(PAG_sceset_qc_norm_filt, dimred = "UMAP_var_corrected_5_neighbors_0.01_min_dist",
                                                               colour_by = "PAG.area", shape_by = "cell.type"
                                                               ) + labs(title = "n_neighbours = 5 | min_dist = 0.01", x = "UMAP 1", y = "UMAP 2")
UMAP_var_corrected_5_neighbors_0.05_min_dist <- plotReducedDim(PAG_sceset_qc_norm_filt, dimred = "UMAP_var_corrected_5_neighbors_0.05_min_dist",
                                                               colour_by = "PAG.area",shape_by = "cell.type"
                                                               ) + labs(title = "n_neighbours = 5 | min_dist = 0.05", x = "UMAP 1", y = "UMAP 2")
UMAP_var_corrected_5_neighbors_0.1_min_dist <- plotReducedDim(PAG_sceset_qc_norm_filt, dimred = "UMAP_var_corrected_5_neighbors_0.1_min_dist",
                                                              colour_by = "PAG.area", shape_by = "cell.type"
                                                              ) + labs(title = "n_neighbours = 5 | min_dist = 0.1", x = "UMAP 1", y = "UMAP 2")
UMAP_var_corrected_5_neighbors_0.2_min_dist <- plotReducedDim(PAG_sceset_qc_norm_filt, dimred = "UMAP_var_corrected_5_neighbors_0.2_min_dist",
                                                              colour_by = "PAG.area", shape_by = "cell.type"
                                                              ) + labs(title = "n_neighbours = 5 | min_dist = 0.2", x = "UMAP 1", y = "UMAP 2")

#### n_neighbors = 10
UMAP_var_corrected_10_neighbors_0.01_min_dist <- plotReducedDim(PAG_sceset_qc_norm_filt, dimred = "UMAP_var_corrected_10_neighbors_0.01_min_dist",
                                                                colour_by = "PAG.area", shape_by = "cell.type"
                                                                ) + labs(title = "n_neighbours = 10 | min_dist = 0.01", x = "UMAP 1", y = "UMAP 2")
UMAP_var_corrected_10_neighbors_0.05_min_dist <- plotReducedDim(PAG_sceset_qc_norm_filt, dimred = "UMAP_var_corrected_10_neighbors_0.05_min_dist",
                                                                colour_by = "PAG.area", shape_by = "cell.type"
                                                                ) + labs(title = "n_neighbours = 10 | min_dist = 0.05", x = "UMAP 1", y = "UMAP 2")
UMAP_var_corrected_10_neighbors_0.1_min_dist <- plotReducedDim(PAG_sceset_qc_norm_filt, dimred = "UMAP_var_corrected_10_neighbors_0.1_min_dist",
                                                               colour_by = "PAG.area", shape_by = "cell.type"
                                                               ) + labs(title = "n_neighbours = 10 | min_dist = 0.1", x = "UMAP 1", y = "UMAP 2")
UMAP_var_corrected_10_neighbors_0.2_min_dist <- plotReducedDim(PAG_sceset_qc_norm_filt, dimred = "UMAP_var_corrected_10_neighbors_0.2_min_dist",
                                                               colour_by = "PAG.area", shape_by = "cell.type"
                                                               ) + labs(title = "n_neighbours = 10 | min_dist = 0.2", x = "UMAP 1", y = "UMAP 2")

#### n_neighbors = 15
UMAP_var_corrected_15_neighbors_0.01_min_dist <- plotReducedDim(PAG_sceset_qc_norm_filt, dimred = "UMAP_var_corrected_15_neighbors_0.01_min_dist",
                                                                colour_by = "PAG.area", shape_by = "cell.type"
                                                                ) + labs(title = "n_neighbours = 15 | min_dist = 0.01", x = "UMAP 1", y = "UMAP 2")
UMAP_var_corrected_15_neighbors_0.05_min_dist <- plotReducedDim(PAG_sceset_qc_norm_filt, dimred = "UMAP_var_corrected_15_neighbors_0.05_min_dist",
                                                                colour_by = "PAG.area", shape_by = "cell.type"
                                                                ) + labs(title = "n_neighbours = 15 | min_dist = 0.05", x = "UMAP 1", y = "UMAP 2")
UMAP_var_corrected_15_neighbors_0.1_min_dist <- plotReducedDim(PAG_sceset_qc_norm_filt, dimred = "UMAP_var_corrected_15_neighbors_0.1_min_dist",
                                                               colour_by = "PAG.area", shape_by = "cell.type"
                                                               ) + labs(title = "n_neighbours = 15 | min_dist = 0.1", x = "UMAP 1", y = "UMAP 2")
UMAP_var_corrected_15_neighbors_0.2_min_dist <- plotReducedDim(PAG_sceset_qc_norm_filt, dimred = "UMAP_var_corrected_15_neighbors_0.2_min_dist",
                                                               colour_by = "PAG.area", shape_by = "cell.type"
                                                               ) + labs(title = "n_neighbours = 15 | min_dist = 0.2", x = "UMAP 1", y = "UMAP 2")

#### n_neighbors = 20
UMAP_var_corrected_20_neighbors_0.01_min_dist <- plotReducedDim(PAG_sceset_qc_norm_filt, dimred = "UMAP_var_corrected_20_neighbors_0.01_min_dist",
                                                                colour_by = "PAG.area", shape_by = "cell.type"
                                                                ) + labs(title = "n_neighbours = 20 | min_dist = 0.01", x = "UMAP 1", y = "UMAP 2")
UMAP_var_corrected_20_neighbors_0.05_min_dist <- plotReducedDim(PAG_sceset_qc_norm_filt, dimred = "UMAP_var_corrected_20_neighbors_0.05_min_dist",
                                                                colour_by = "PAG.area", shape_by = "cell.type"
                                                                ) + labs(title = "n_neighbours = 20 | min_dist = 0.05", x = "UMAP 1", y = "UMAP 2")
UMAP_var_corrected_20_neighbors_0.1_min_dist <- plotReducedDim(PAG_sceset_qc_norm_filt, dimred = "UMAP_var_corrected_20_neighbors_0.1_min_dist",
                                                               colour_by = "PAG.area", shape_by = "cell.type"
                                                               ) + labs(title = "n_neighbours = 20 | min_dist = 0.1", x = "UMAP 1", y = "UMAP 2")
UMAP_var_corrected_20_neighbors_0.2_min_dist <- plotReducedDim(PAG_sceset_qc_norm_filt, dimred = "UMAP_var_corrected_20_neighbors_0.2_min_dist",
                                                               colour_by = "PAG.area", shape_by = "cell.type"
                                                               ) + labs(title = "n_neighbours = 20 | min_dist = 0.2", x = "UMAP 1", y = "UMAP 2")
...
UMAP_HVG_var_corrected_neighbors_5 <- (UMAP_var_corrected_5_neighbors_0.01_min_dist + UMAP_var_corrected_5_neighbors_0.05_min_dist +
  ...
  plot_annotation(title = "UMAP plots for 5 neighbours at different min_dist (HVG - var)", tag_levels = "A", 
                  theme = theme(plot.title = element_text(size = 18), plot.tag = element_text(size = 14))) + plot_layout(ncol = 2, guides = "collect")
...
# Save them
ggsave(filename = str_c(date, "_UMAP_HVG_var_corrected_neighbors_5_composed.pdf"),
...
ggsave(filename = str_c(date, "_UMAP_HVG_var_corrected_neighbors_10_composed.pdf"),
...
ggsave(filename = str_c(date, "_UMAP_HVG_var_corrected_neighbors_15_composed.pdf"),
...
ggsave(filename = str_c(date, "_UMAP_HVG_var_corrected_neighbors_20_composed.pdf"),
...
#### n_neighbors = 5
UMAP_cv2_corrected_5_neighbors_0.01_min_dist <- plotReducedDim(PAG_sceset_qc_norm_filt, dimred = "UMAP_cv2_corrected_5_neighbors_0.01_min_dist",
                                                               colour_by = "PAG.area", shape_by = "cell.type"
                                                               ) + labs(title = "n_neighbours = 5 | min_dist = 0.01", x = "UMAP 1", y = "UMAP 2")
UMAP_cv2_corrected_5_neighbors_0.05_min_dist <- plotReducedDim(PAG_sceset_qc_norm_filt, dimred = "UMAP_cv2_corrected_5_neighbors_0.05_min_dist",
                                                               colour_by = "PAG.area", shape_by = "cell.type"
                                                               ) + labs(title = "n_neighbours = 5 | min_dist = 0.05", x = "UMAP 1", y = "UMAP 2")
UMAP_cv2_corrected_5_neighbors_0.1_min_dist <- plotReducedDim(PAG_sceset_qc_norm_filt, dimred = "UMAP_cv2_corrected_5_neighbors_0.1_min_dist",
                                                              colour_by = "PAG.area", shape_by = "cell.type"
                                                              ) + labs(title = "n_neighbours = 5 | min_dist = 0.1", x = "UMAP 1", y = "UMAP 2")
UMAP_cv2_corrected_5_neighbors_0.2_min_dist <- plotReducedDim(PAG_sceset_qc_norm_filt, dimred = "UMAP_cv2_corrected_5_neighbors_0.2_min_dist",
                                                              colour_by = "PAG.area", shape_by = "cell.type"
                                                              ) + labs(title = "n_neighbours = 5 | min_dist = 0.2", x = "UMAP 1", y = "UMAP 2")

#### n_neighbors = 10
UMAP_cv2_corrected_10_neighbors_0.01_min_dist <- plotReducedDim(PAG_sceset_qc_norm_filt, dimred = "UMAP_cv2_corrected_10_neighbors_0.01_min_dist",
                                                                colour_by = "PAG.area", shape_by = "cell.type"
                                                                ) + labs(title = "n_neighbours = 10 | min_dist = 0.01", x = "UMAP 1", y = "UMAP 2")
UMAP_cv2_corrected_10_neighbors_0.05_min_dist <- plotReducedDim(PAG_sceset_qc_norm_filt, dimred = "UMAP_cv2_corrected_10_neighbors_0.05_min_dist",
                                                                colour_by = "PAG.area", shape_by = "cell.type"
                                                                ) + labs(title = "n_neighbours = 10 | min_dist = 0.05", x = "UMAP 1", y = "UMAP 2")
UMAP_cv2_corrected_10_neighbors_0.1_min_dist <- plotReducedDim(PAG_sceset_qc_norm_filt, dimred = "UMAP_cv2_corrected_10_neighbors_0.1_min_dist",
                                                               colour_by = "PAG.area", shape_by = "cell.type"
                                                               ) + labs(title = "n_neighbours = 10 | min_dist = 0.1", x = "UMAP 1", y = "UMAP 2")
UMAP_cv2_corrected_10_neighbors_0.2_min_dist <- plotReducedDim(PAG_sceset_qc_norm_filt, dimred = "UMAP_cv2_corrected_10_neighbors_0.2_min_dist",
                                                               colour_by = "PAG.area", shape_by = "cell.type"
                                                               ) + labs(title = "n_neighbours = 10 | min_dist = 0.2", x = "UMAP 1", y = "UMAP 2")

#### n_neighbors = 15
UMAP_cv2_corrected_15_neighbors_0.01_min_dist <- plotReducedDim(PAG_sceset_qc_norm_filt, dimred = "UMAP_cv2_corrected_15_neighbors_0.01_min_dist",
                                                                colour_by = "PAG.area", shape_by = "cell.type"
                                                                ) + labs(title = "n_neighbours = 15 | min_dist = 0.01", x = "UMAP 1", y = "UMAP 2")
UMAP_cv2_corrected_15_neighbors_0.05_min_dist <- plotReducedDim(PAG_sceset_qc_norm_filt, dimred = "UMAP_cv2_corrected_15_neighbors_0.05_min_dist",
                                                                colour_by = "PAG.area", shape_by = "cell.type"
                                                                ) + labs(title = "n_neighbours = 15 | min_dist = 0.05", x = "UMAP 1", y = "UMAP 2")
UMAP_cv2_corrected_15_neighbors_0.1_min_dist <- plotReducedDim(PAG_sceset_qc_norm_filt, dimred = "UMAP_cv2_corrected_15_neighbors_0.1_min_dist",
                                                               colour_by = "PAG.area", shape_by = "cell.type"
                                                               ) + labs(title = "n_neighbours = 15 | min_dist = 0.1", x = "UMAP 1", y = "UMAP 2")
UMAP_cv2_corrected_15_neighbors_0.2_min_dist <- plotReducedDim(PAG_sceset_qc_norm_filt, dimred = "UMAP_cv2_corrected_15_neighbors_0.2_min_dist",
                                                               colour_by = "PAG.area", shape_by = "cell.type"
                                                               ) + labs(title = "n_neighbours = 15 | min_dist = 0.2", x = "UMAP 1", y = "UMAP 2")

#### n_neighbors = 20 
UMAP_cv2_corrected_20_neighbors_0.01_min_dist <- plotReducedDim(PAG_sceset_qc_norm_filt, dimred = "UMAP_cv2_corrected_20_neighbors_0.01_min_dist",
                                                                colour_by = "PAG.area", shape_by = "cell.type"
                                                                ) + labs(title = "n_neighbours = 20 | min_dist = 0.01", x = "UMAP 1", y = "UMAP 2")
UMAP_cv2_corrected_20_neighbors_0.05_min_dist <- plotReducedDim(PAG_sceset_qc_norm_filt, dimred = "UMAP_cv2_corrected_20_neighbors_0.05_min_dist",
                                                                colour_by = "PAG.area", shape_by = "cell.type"
                                                                ) + labs(title = "n_neighbours = 20 | min_dist = 0.05", x = "UMAP 1", y = "UMAP 2")
UMAP_cv2_corrected_20_neighbors_0.1_min_dist <- plotReducedDim(PAG_sceset_qc_norm_filt, dimred = "UMAP_cv2_corrected_20_neighbors_0.1_min_dist",
                                                               colour_by = "PAG.area", shape_by = "cell.type"
                                                               ) + labs(title = "n_neighbours = 20 | min_dist = 0.1", x = "UMAP 1", y = "UMAP 2")
UMAP_cv2_corrected_20_neighbors_0.2_min_dist <- plotReducedDim(PAG_sceset_qc_norm_filt, dimred = "UMAP_cv2_corrected_20_neighbors_0.2_min_dist",
                                                               colour_by = "PAG.area", shape_by = "cell.type"
                                                               ) + labs(title = "n_neighbours = 20 | min_dist = 0.2", x = "UMAP 1", y = "UMAP 2")
...
# Save them
ggsave(filename = str_c(date, "_UMAP_HVG_cv2_corrected_neighbors_5_composed.pdf"),
...
ggsave(filename = str_c(date, "_UMAP_HVG_cv2_corrected_neighbors_10_composed.pdf"),
...
ggsave(filename = str_c(date, "_UMAP_HVG_cv2_corrected_neighbors_15_composed.pdf"),
...
ggsave(filename = str_c(date, "_UMAP_HVG_cv2_corrected_neighbors_20_composed.pdf"),
...
chosen_tSNE_HVG_var_corrected_15 <- plotReducedDim(PAG_sceset_qc_norm_filt, dimred = "tSNE_var_corrected_perplexity_15",
                                                   colour_by = "PAG.area", shape_by = "cell.type", point_alpha = 0.6, point_size = 2
                                                   ) + labs(title = "HVG (var) - Perplexity = 15", x = "t-SNE 1", y = "t-SNE 2")

chosen_tSNE_HVG_cv2_corrected_15 <- plotReducedDim(PAG_sceset_qc_norm_filt, dimred = "tSNE_cv2_corrected_perplexity_15",
                                                   colour_by = "PAG.area", shape_by = "cell.type", point_alpha = 0.6, point_size = 2
...
chosen_tSNE_HVG_var_corrected_15_cd68 <- plotReducedDim(PAG_sceset_qc_norm_filt, dimred = "tSNE_var_corrected_perplexity_15",
                                                   colour_by = "Cd68", shape_by = "cell.type", point_alpha = 0.6, point_size = 2
                                                   ) + labs(title = "HVG (var) - Perplexity = 15", x = "t-SNE 1", y = "t-SNE 2")

chosen_tSNE_HVG_cv2_corrected_15_cd68 <- plotReducedDim(PAG_sceset_qc_norm_filt, dimred = "tSNE_cv2_corrected_perplexity_15",
                                                   colour_by = "Cd68", shape_by = "cell.type", point_alpha = 0.6, point_size = 2
                                                   ) + labs(title = "HVG (cv2) - Perplexity = 15", x = "t-SNE 1", y = "t-SNE 2")

# Plot the selected UMAP
chosen_UMAP_var_corrected_10_0.01 <- plotReducedDim(PAG_sceset_qc_norm_filt, dimred = "UMAP_var_corrected_10_neighbors_0.01_min_dist",
                                                    colour_by = "PAG.area", shape_by = "cell.type", point_alpha = 0.6, point_size = 2
                                                    ) + labs(title = "HVG (var) - n_neighbours = 10 | min_dist = 0.01", x = "UMAP 1", y = "UMAP 2")

chosen_UMAP_cv2_corrected_10_0.05 <- plotReducedDim(PAG_sceset_qc_norm_filt, dimred = "UMAP_cv2_corrected_10_neighbors_0.05_min_dist",
                                                    colour_by = "PAG.area", shape_by = "cell.type", point_alpha = 0.6, point_size = 2
                                                    ) + labs(title = "HVG (cv2) - n_neighbours = 10 | min_dist = 0.05", x = "UMAP 1", y = "UMAP 2")

chosen_UMAP_cv2_corrected_15_0.01 <- plotReducedDim(PAG_sceset_qc_norm_filt, dimred = "UMAP_cv2_corrected_15_neighbors_0.01_min_dist",
                                                    colour_by = "PAG.area", shape_by = "cell.type", point_alpha = 0.6, point_size = 2
                                                    ) + labs(title = "HVG (cv2) - n_neighbours = 15 | min_dist = 0.01", x = "UMAP 1", y = "UMAP 2")
...
chosen_UMAP_var_corrected_10_0.01_cd68 <- plotReducedDim(PAG_sceset_qc_norm_filt, dimred = "UMAP_var_corrected_10_neighbors_0.01_min_dist",
                                                         colour_by = "Cd68", shape_by = "cell.type", point_alpha = 0.6, point_size = 2
                                                         ) + labs(title = "HVG (var) - n_neighbours = 10 | min_dist = 0.01", x = "UMAP 1", y = "UMAP 2")

chosen_UMAP_cv2_corrected_10_0.05_cd68 <- plotReducedDim(PAG_sceset_qc_norm_filt, dimred = "UMAP_cv2_corrected_10_neighbors_0.05_min_dist",
                                                         colour_by = "Cd68", shape_by = "cell.type", point_alpha = 0.6, point_size = 2
                                                         ) + labs(title = "HVG (cv2) - n_neighbours = 10 | min_dist = 0.05", x = "UMAP 1", y = "UMAP 2")

chosen_UMAP_cv2_corrected_15_0.01_cd68 <- plotReducedDim(PAG_sceset_qc_norm_filt, dimred = "UMAP_cv2_corrected_15_neighbors_0.01_min_dist",
                                                         colour_by = "Cd68", shape_by = "cell.type", point_alpha = 0.6, point_size = 2
                                                         ) + labs(title = "HVG (cv2) - n_neighbours = 15 | min_dist = 0.01", x = "UMAP 1", y = "UMAP 2")
...
chosen_UMAP_composed <- (chosen_UMAP_var_corrected_10_0.01 + chosen_UMAP_cv2_corrected_10_0.05 + chosen_UMAP_cv2_corrected_15_0.01 +
                           chosen_UMAP_var_corrected_10_0.01_cd68 + chosen_UMAP_cv2_corrected_10_0.05_cd68 + chosen_UMAP_cv2_corrected_15_0.01_cd68) +
  plot_annotation(title = "Selected UMAP plots", tag_levels = "A", 
                  theme = theme(plot.title = element_text(size = 18), plot.tag = element_text(size = 14))) + plot_layout(ncol = 3, guides = "collect")
...
chosen_UMAP_cv2_corrected_10_0.05 <- plotReducedDim(PAG_sceset_qc_norm_filt, dimred = "UMAP_cv2_corrected_10_neighbors_0.05_min_dist",
                                                    colour_by = "PAG.area", shape_by = "cell.type", point_alpha = 0.6, point_size = 2
                                                    ) + labs(title = "UMAP HVG (cv2) | n_neighbours = 10 | min_dist = 0.05", x = "UMAP 1", y = "UMAP 2")

# Separating by `cell.type`:
chosen_UMAP_cv2_facet_celltype <- plotReducedDim(PAG_sceset_qc_norm_filt, dimred = "UMAP_cv2_corrected_10_neighbors_0.05_min_dist",
                                                 colour_by = "PAG.area", point_alpha = 0.6, point_size = 2
                                                 ) + labs(title = "Facets by cell type and coloured by PAG subdivision", x = "UMAP 1", y = "UMAP 2"
                                                          ) + facet_wrap(~PAG_sceset_qc_norm_filt$cell.type)

# Separating by `cell.type` and coloring by VGAT or VGlUT2 expression:
chosen_UMAP_cv2_facet_celltype_VGAT <- plotReducedDim(PAG_sceset_qc_norm_filt, dimred = "UMAP_cv2_corrected_10_neighbors_0.05_min_dist",
                                                      colour_by = "log2_total_counts_VGAT",  shape_by = "cell.type", point_alpha = 0.6, point_size = 2
                                                      ) + labs(title = "Facets by cell type and coloured by VGAT expression", x = "UMAP 1", y = "UMAP 2"
                                                               ) + facet_wrap(~PAG_sceset_qc_norm_filt$cell.type)

chosen_UMAP_cv2_facet_celltype_VGluT2 <- plotReducedDim(PAG_sceset_qc_norm_filt, dimred = "UMAP_cv2_corrected_10_neighbors_0.05_min_dist",
                                                        colour_by = "log2_total_counts_VGluT2",  shape_by = "cell.type", point_alpha = 0.6, point_size = 2
                                                        ) + labs(title = "Facets by cell type and coloured by VGluT2 expression", x = "UMAP 1", y = "UMAP 2"
                                                                 ) + facet_wrap(~PAG_sceset_qc_norm_filt$cell.type)

# Separating by `cell.type` and coloring by whether a cell expresses VGAT or VGluT2 only, or a combination of both:
chosen_UMAP_cv2_facet_celltype_VGATVGluT2 <- plotReducedDim(PAG_sceset_qc_norm_filt, dimred = "UMAP_cv2_corrected_10_neighbors_0.05_min_dist",
                                                            colour_by = "VGAT_VGluT2_expression",  shape_by = "cell.type", point_alpha = 0.6, point_size = 2
                                                            ) + labs(title = "Facets by cell type and coloured by VGAT-VGluT2 profile", x = "UMAP 1", y = "UMAP 2"
                                                                     ) + facet_wrap(~PAG_sceset_qc_norm_filt$cell.type)

# Separating by `PAG.area`:
chosen_UMAP_cv2_facet_PAGarea <- plotReducedDim(PAG_sceset_qc_norm_filt, dimred = "UMAP_cv2_corrected_10_neighbors_0.05_min_dist",
                                                colour_by = "cell.type", point_alpha = 0.6, point_size = 2
                                                ) + labs(title = "Facets by PAG subdivision and coloured by cell type", x = "UMAP 1", y = "UMAP 2"
                                                         ) + facet_wrap(~PAG_sceset_qc_norm_filt$PAG.area, dir = "v")

# Separating by whether a cell expresses VGAT or VGluT2 only, or a combination of both:
chosen_UMAP_cv2_facet_VGATVGluT2 <- plotReducedDim(PAG_sceset_qc_norm_filt, dimred = "UMAP_cv2_corrected_10_neighbors_0.05_min_dist",
                                                   colour_by = "PAG.area",  shape_by = "cell.type", point_alpha = 0.6, point_size = 2
                                                   ) + labs(title = "Facets by VGAT-VGluT2 profile and coloured by PAG subdivsion", x = "UMAP 1", y = "UMAP 2"
                                                            ) + facet_wrap(~PAG_sceset_qc_norm_filt$VGAT_VGluT2_expression, nrow = 2, drop = FALSE)
...
chosen_UMAP_facet_celltype_composed <- (chosen_UMAP_cv2_facet_celltype / chosen_UMAP_cv2_facet_celltype_VGATVGluT2 /
  ...
  plot_annotation(title = NULL, tag_levels = "A", theme = theme(plot.title = element_text(size = 18), plot.tag = element_text(size = 14)))

# Plot them
chosen_UMAP_cv2_corrected_10_0.05
chosen_UMAP_cv2_facet_PAGarea
chosen_UMAP_facet_celltype_composed
chosen_UMAP_cv2_facet_VGATVGluT2

# Save them
ggsave(filename = str_c(date, "_chosen_UMAP_cv2_corrected_10_0.05.pdf"),
       plot = chosen_UMAP_cv2_corrected_10_0.05,
       device = "pdf", # or one of "eps", "ps", "tex" (pictex), "pdf", "jpeg", "tiff", "png", "bmp", "svg" or "wmf" (windows only).
       path = path_for_figures,
       width = 12, height = 9, units = "in", dpi = 300,
       family = "Arial", bg = "transparent"
...
table(PAG_sceset_qc_norm_filt$VGAT_VGluT2_expression)
which(PAG_sceset_qc_norm_filt$VGAT_VGluT2_expression == "VGAT_Cre_VGluT2_only")
PAG_sceset_qc_norm_filt$mouse.id[(PAG_sceset_qc_norm_filt$VGAT_VGluT2_expression == "VGAT_Cre_VGluT2_only")]
PAG_sceset_qc_norm_filt$batch.processing[(PAG_sceset_qc_norm_filt$VGAT_VGluT2_expression == "VGAT_Cre_VGluT2_only")]
```
