...
```{r}
# How many predicted genes to we have in the HVGs?
is_predicted <- grepl("^Gm", rownames(PAG_sceset_qc_norm_filt))
rowData(PAG_sceset_qc_norm_filt)$predicted_genes <- is_predicted
sum(is_predicted %in% metadata(PAG_sceset_qc_norm_filt)$hvg_out_no_spikes_filt)
```
...
We continue using the `PAG_sceset_qc_norm_filt` after normalization. We should thus have a `corrected` slot in `assays`:
```{r}
set.seed(1991)
library(SC3)
library(scater)
...
PAG_sceset_qc_norm_filt <- readRDS("PAG_sceset_qc_norm_filt.rds") # Contains filtered cells and genes, and normalized data
...
### 7.1.1 | Estimate k and prepare SingleCellExperiment object for SC3
```{r}
...
# Estimate K
PAG_sceset_qc_norm_filt <- sc3_estimate_k(PAG_sceset_qc_norm_filt)
metadata(PAG_sceset_qc_norm_filt)$sc3$k_estimation
```

SC3 also requires a `feature_symbol` column of the `rowData` slot of the input `SingleCellExperiment` object to contain preferable feature names (genes/transcript) which will be used in the future visualisations. In addition, SC3 applies a gene filter to remove genes very lowly expressed in all samples and also the ones that are expressed everywhere, so that these genes do not contribute to the clustering (see `$sc3_gene_filter`). However, we have already selected highly variable genes, which we could use for it. We can manually define genes that we want to use for clustering by setting the `sc3_gene_filter` column of the `rowData` slot:
```{r}
rowData(PAG_sceset_qc_norm_filt)$feature_symbol <- rownames(PAG_sceset_qc_norm_filt)
rowData(PAG_sceset_qc_norm_filt)$sc3_gene_filter <- rownames(PAG_sceset_qc_norm_filt) %in% metadata(PAG_sceset_qc_norm_filt)$hvg_out_no_spikes_filt
# WE CURRENTLY HAVE MANY PREDICTED GENES AMONG HVG, FILTER THOSE OUT BEFORE HVG DETECTION
```

### 7.1.2 | Running SC3 with the wrapper function
We can also run SC3 with the wrapper function and ask it to calculate biological properties of the clusters as well. We try a range of k and explore the different clustering solutions with the interactive `Shiny` session. We can check the stability of the different clusters and play with the thresholds for DE and Marker genes. 

SC3 is based on k-means clustering, so we need to define the ks at the beginning. It is best to set a range of ks around the estimated value to try and compare different solutions.
```{r}
start_time_sc3 <- Sys.time() # Takes around 35 min
PAG_sceset_qc_norm_filt <- sc3(PAG_sceset_qc_norm_filt,
                               ks = 2:10,
                               gene_filter = FALSE,
                               biology = TRUE, 
                               n_cores = 2,
                               rand_seed = 1991)
end_time_sc3 <- Sys.time()
end_time_sc3 - start_time_sc3
```

### 7.1.3 | Running SC3 step by step

### 7.1.4 | Explore the results
To run the interactive `Shiny` session, we can do the following: 
```{r}
sc3_interactive(PAG_sceset_qc_norm_filt)
```
...
```{r}
# Consensus Matrix
sc3_plot_consensus(PAG_sceset_qc_norm_filt, 
                   k = 4, 
                   show_pdata = "cell.type")

# DE Genes
sc3_plot_de_genes(PAG_sceset_qc_norm_filt, 
                 k = 4,
                 p.val = 0.001,
                 show_pdata = "cell.type")

# Marker Genes
sc3_plot_markers(PAG_sceset_qc_norm_filt, 
                 k = 4,
                 auroc = 0.8,
                 p.val = 0.01,
                 show_pdata = "cell.type")
...
```{r}
plotPCA(PAG_sceset_qc_norm_filt, colour_by = "sc3_8_clusters")
```
...
```{r}
sc3_plot_silhouette(PAG_sceset_qc_norm_filt,
                    k = 8)
```
...
```{r}
sc3_plot_cluster_stability(PAG_sceset_qc_norm_filt, 
                           k = 8)
```
...
```{r}
sc3_plot_expression(PAG_sceset_qc_norm_filt, 
                    k = 8,
                    show_pdata = c("cell.type",
                                  "PAG.areacollection",
                                  "sc3_8_clusters"
...
### 7.1.5 | Export the results
SC3 allows us to export the results into an Excel file:
```{r}
sc3_export_results_xls(PAG_sceset_qc_norm_filt, filename = "PAG_clustering_SC3")
```
