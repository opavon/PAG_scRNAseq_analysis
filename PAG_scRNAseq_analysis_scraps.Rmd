...
## Step 7.10 | SNN-Cliq (graph-based clustering) [not installed, python based]
...
## STEP X.1 | scRNA-seq analysis with Seurat (for large datasets) 
Seurat (https://satijalab.org/seurat/) was developed with 10x in mind and is not very good with small datasets (smaller than 1000). For small datasets a good alternative would be SC3.
```{r}
#Restart R session before running Seurat
library(SingleCellExperiment)
library(Seurat)
library(mclust)
library(dplyr)
...
### Build Seurat object
Seurat uses its own object class, so we need to create a new object with the raw (non-normalised) data.
```{r}
PAG_seurat_set <- CreateSeuratObject(
  raw.data = (counts = as.matrix(PAG_data)), 
  min.cells = 5, #keep genes expressed in more than 5 cells
  min.genes = 6000 #keep cells with at least 6000 detected genes
)
```
...
```{r}
# We don't have UMIs in our data, so the nUMI slot is equivalent to the total counts per cell
VlnPlot(
    object = PAG_seurat_set, 
    features.plot = c("nGene", "nUMI"), #expressed genes, total counts
    nCol = 2
)
```
...
GenePlot(
    object = PAG_seurat_set, 
    gene1 = "nUMI", #total counts
    gene2 = "nGene" #expressed genes
)
```

We can exclude cells with a clear outlier number of read counts
```{r}
PAG_seurat_set <- FilterCells(
    object = PAG_seurat_set, 
    subset.names = c("nUMI"),
    low.thresholds = c(5e5)
)
```
...
```{r}
PAG_seurat_set <- FindVariableGenes(
    object = PAG_seurat_set,
...
length(x = PAG_seurat_set@var.genes)
```
...
```{r}
PAG_seurat_set <- RunPCA(
    object = PAG_seurat_set, 
    pc.genes = PAG_seurat_set@var.genes, 
    do.print = TRUE, 
    pcs.print = 1:5, 
    genes.print = 5
)
...
```{r}
PrintPCA(object = PAG_seurat_set, pcs.print = 1:5, genes.print = 5, use.full = FALSE)
VizPCA(object = PAG_seurat_set, pcs.use = 1:5)
PCAPlot(object = PAG_seurat_set, dim.1 = 1, dim.2 = 2)
```
...
```{r}
JackStrawPlot(object = PAG_seurat_set, PCs = 1:11)
```
...
```{r}
PAG_seurat_set <- RunTSNE(
    object = PAG_seurat_set,
    dims.use = 1:8,
    do.fast = TRUE
)
TSNEPlot(object = PAG_seurat_set)
```
...
```{r}
markers2 <- FindMarkers(PAG_seurat_set, 2)
VlnPlot(object = PAG_seurat_set, features.plot = rownames(markers2)[1:20])
FeaturePlot(
    PAG_seurat_set, 
...
## STEP X.2 | Explore and compare dimensionality reduction approaches with Sleepwalk
...
## STEP X.3 | Comparing/Combining scRNASeq datasets (scmap)
...
## STEP X.4 | Pseudotime Analysis (for development trajectories)
...
## STEP X.5 | Imputation (after QC but prior to downstream analysis)
...
## STEP X.6 | Search scRNA-seq data (scfind)
...
### Load gene lists
```{r}
# Load the csv file containing several lists of interesting genes
PAG_scRNAseq_gene_lists <- data.frame(read.table("PAG_scRNAseq_gene_lists.csv", sep=",", header=TRUE))
```

### Plot Ion Channels
```{r}
# Get the ion channel gene names from the appropriate column
PAG_ionchannels <- PAG_scRNAseq_gene_lists$genes.ionchannels
#length(ionchannels)
#typeof(ionchannels)

# Filter any genes not present in our dataset by comparing the gene names with the rownames of our expression matrix (which contains gene names)
PAG_ionchannels <- PAG_ionchannels[(PAG_ionchannels %in% rownames(PAG_data))]
#length(PAG_ionchannels)
#typeof(PAG_ionchannels)

# Make violing plots
plotExpression(PAG_sceset, PAG_ionchannels[1:20], x = "PAG.areacollection", colour_by = "cell.type", exprs_values = "counts", ncol = 5, show_median = TRUE, log=TRUE)
# Use PAG_ionchannels[1:20] to select a subset of ion channels
# Alternate between x = "PAG.areacollection", colour_by = "cell.type"
# or x = "cell.type", colour_by = "PAG.areacollection"
# Use ncol = 10 to speficy the number of columns the plot will be divided into
```

### Plot Potassium Channels
```{r}
PAG_potassium <- PAG_scRNAseq_gene_lists$genes.icPotassium
PAG_potassium <- PAG_potassium[(PAG_potassium %in% rownames(PAG_data))]
plotExpression(PAG_sceset, PAG_potassium, x = "cell.type", colour_by = "PAG.areacollection", exprs_values = "counts", ncol = 10, show_median = TRUE, log=TRUE)
plotExpression(PAG_sceset, PAG_potassium, x = "PAG.areacollection", colour_by = "cell.type", exprs_values = "counts", ncol = 5, show_median = TRUE, log=TRUE)

plotHeatmap(PAG_sceset, PAG_potassium, columns = 1:48, exprs_values = "logcounts_raw", center = FALSE, cluster_rows = FALSE, cluster_cols = FALSE, zlim = c(0,10), color = colorRampPalette(c("white", "#009CB5"))(100), border_color=NA, fontsize=12, cellwidth = 15, cellheight = 25, show_colnames=FALSE)
plotHeatmap(PAG_sceset, PAG_potassium, columns = 49:96, exprs_values = "logcounts_raw", center = FALSE, cluster_rows = FALSE, cluster_cols = FALSE, zlim = c(0,10), color = colorRampPalette(c("white", "#ff777c"))(100), border_color=NA, fontsize=12, cellwidth = 15, cellheight = 25, show_colnames=FALSE)

```

### Plot Calcium Channels
```{r}
PAG_calcium <- PAG_scRNAseq_gene_lists$genes.icCalcium
PAG_calcium <- PAG_calcium[(PAG_calcium %in% rownames(PAG_data))]
plotExpression(PAG_sceset, PAG_calcium, x = "cell.type", colour_by = "PAG.areacollection", exprs_values = "counts", ncol = 5, show_median = TRUE, log=TRUE)
plotExpression(PAG_sceset, PAG_calcium, x = "PAG.areacollection", colour_by = "cell.type", exprs_values = "counts", ncol = 5, show_median = TRUE, log=TRUE)

plotHeatmap(PAG_sceset, PAG_calcium, columns = 1:48, exprs_values = "logcounts_raw", center = FALSE, cluster_rows = FALSE, cluster_cols = FALSE, zlim = c(0,10), color = colorRampPalette(c("white", "#009CB5"))(100), border_color=NA, fontsize=12, cellwidth = 15, cellheight = 25, show_colnames=FALSE)
plotHeatmap(PAG_sceset, PAG_calcium, columns = 49:96, exprs_values = "logcounts_raw", center = FALSE, cluster_rows = FALSE, cluster_cols = FALSE, zlim = c(0,10), color = colorRampPalette(c("white", "#ff777c"))(100), border_color=NA, fontsize=12, cellwidth = 15, cellheight = 25, show_colnames=FALSE)

```

### Plot Sodium Channels
```{r}
PAG_sodium <- PAG_scRNAseq_gene_lists$genes.icSodium
PAG_sodium <- PAG_sodium[(PAG_sodium %in% rownames(PAG_data))]
plotExpression(PAG_sceset, PAG_sodium, x = "cell.type", colour_by = "PAG.areacollection", exprs_values = "counts", ncol = 5, show_median = TRUE, log=TRUE)
plotExpression(PAG_sceset, PAG_sodium, x = "PAG.areacollection", colour_by = "cell.type", exprs_values = "counts", ncol = 5, show_median = TRUE, log=TRUE)

plotHeatmap(PAG_sceset, PAG_sodium, columns = 1:48, exprs_values = "logcounts_raw", center = FALSE, cluster_rows = FALSE, cluster_cols = FALSE, zlim = c(0,10), color = colorRampPalette(c("white", "#009CB5"))(100), border_color=NA, fontsize=12, cellwidth = 15, cellheight = 25, show_colnames=FALSE)
plotHeatmap(PAG_sceset, PAG_sodium, columns = 49:96, exprs_values = "logcounts_raw", center = FALSE, cluster_rows = FALSE, cluster_cols = FALSE, zlim = c(0,10), color = colorRampPalette(c("white", "#ff777c"))(100), border_color=NA, fontsize=12, cellwidth = 15, cellheight = 25, show_colnames=FALSE)

```

### Plot Chloride Channels
```{r}
PAG_chloride <- PAG_scRNAseq_gene_lists$genes.icChloride
PAG_chloride <- PAG_chloride[(PAG_chloride %in% rownames(PAG_data))]
plotExpression(PAG_sceset, PAG_chloride, x = "cell.type", colour_by = "PAG.areacollection", exprs_values = "counts", ncol = 5, show_median = TRUE, log=TRUE)
plotExpression(PAG_sceset, PAG_chloride, x = "PAG.areacollection", colour_by = "cell.type", exprs_values = "counts", ncol = 5, show_median = TRUE, log=TRUE)

plotHeatmap(PAG_sceset, PAG_chloride, columns = 1:48, exprs_values = "logcounts_raw", center = FALSE, cluster_rows = FALSE, cluster_cols = FALSE, zlim = c(0,10), color = colorRampPalette(c("white", "#009CB5"))(100), border_color=NA, fontsize=12, cellwidth = 15, cellheight = 25, show_colnames=FALSE)
plotHeatmap(PAG_sceset, PAG_chloride, columns = 49:96, exprs_values = "logcounts_raw", center = FALSE, cluster_rows = FALSE, cluster_cols = FALSE, zlim = c(0,10), color = colorRampPalette(c("white", "#ff777c"))(100), border_color=NA, fontsize=12, cellwidth = 15, cellheight = 25, show_colnames=FALSE)

```

### Plot Asic Channels
```{r}
PAG_asic <- PAG_scRNAseq_gene_lists$genes.icAsic
PAG_asic <- PAG_asic[(PAG_asic %in% rownames(PAG_data))]
plotExpression(PAG_sceset, PAG_asic, x = "cell.type", colour_by = "PAG.areacollection", exprs_values = "counts", ncol = 5, show_median = TRUE, log=TRUE)
plotExpression(PAG_sceset, PAG_asic, x = "PAG.areacollection", colour_by = "cell.type", exprs_values = "counts", ncol = 5, show_median = TRUE, log=TRUE)

plotHeatmap(PAG_sceset, PAG_asic, columns = 1:48, exprs_values = "logcounts_raw", center = FALSE, cluster_rows = FALSE, cluster_cols = FALSE, zlim = c(0,10), color = colorRampPalette(c("white", "#009CB5"))(100), border_color=NA, fontsize=12, cellwidth = 15, cellheight = 25, show_colnames=FALSE)
plotHeatmap(PAG_sceset, PAG_asic, columns = 49:96, exprs_values = "logcounts_raw", center = FALSE, cluster_rows = FALSE, cluster_cols = FALSE, zlim = c(0,10), color = colorRampPalette(c("white", "#ff777c"))(100), border_color=NA, fontsize=12, cellwidth = 15, cellheight = 25, show_colnames=FALSE)

```

### Plot Cyclic Nucleotide Channels
```{r}
PAG_cyclic_nucleotides <- PAG_scRNAseq_gene_lists$genes.icCyclicnucleotides
PAG_cyclic_nucleotides <- PAG_cyclic_nucleotides[(PAG_cyclic_nucleotides %in% rownames(PAG_data))]
plotExpression(PAG_sceset, PAG_cyclic_nucleotides, x = "cell.type", colour_by = "PAG.areacollection", exprs_values = "counts", ncol = 5, show_median = TRUE, log=TRUE)
plotExpression(PAG_sceset, PAG_cyclic_nucleotides, x = "PAG.areacollection", colour_by = "cell.type", exprs_values = "counts", ncol = 5, show_median = TRUE, log=TRUE)

plotHeatmap(PAG_sceset, PAG_cyclic_nucleotides, columns = 1:48, exprs_values = "logcounts_raw", center = FALSE, cluster_rows = FALSE, cluster_cols = FALSE, zlim = c(0,10), color = colorRampPalette(c("white", "#009CB5"))(100), border_color=NA, fontsize=12, cellwidth = 15, cellheight = 25, show_colnames=FALSE)
plotHeatmap(PAG_sceset, PAG_cyclic_nucleotides, columns = 49:96, exprs_values = "logcounts_raw", center = FALSE, cluster_rows = FALSE, cluster_cols = FALSE, zlim = c(0,10), color = colorRampPalette(c("white", "#ff777c"))(100), border_color=NA, fontsize=12, cellwidth = 15, cellheight = 25, show_colnames=FALSE)

```

### Plot TRP Channels
```{r}
PAG_trp <- PAG_scRNAseq_gene_lists$genes.icTRP
PAG_trp <- PAG_trp[(PAG_trp %in% rownames(PAG_data))]
plotExpression(PAG_sceset, PAG_trp, x = "cell.type", colour_by = "PAG.areacollection", exprs_values = "counts", ncol = 5, show_median = TRUE, log=TRUE)
plotExpression(PAG_sceset, PAG_trp, x = "PAG.areacollection", colour_by = "cell.type", exprs_values = "counts", ncol = 5, show_median = TRUE, log=TRUE)

plotHeatmap(PAG_sceset, PAG_trp, columns = 1:48, exprs_values = "logcounts_raw", center = FALSE, cluster_rows = FALSE, cluster_cols = FALSE, zlim = c(0,10), color = colorRampPalette(c("white", "#009CB5"))(100), border_color=NA, fontsize=12, cellwidth = 15, cellheight = 25, show_colnames=FALSE)
plotHeatmap(PAG_sceset, PAG_trp, columns = 49:96, exprs_values = "logcounts_raw", center = FALSE, cluster_rows = FALSE, cluster_cols = FALSE, zlim = c(0,10), color = colorRampPalette(c("white", "#ff777c"))(100), border_color=NA, fontsize=12, cellwidth = 15, cellheight = 25, show_colnames=FALSE)

```

### Plot Connexins Channels
```{r}
PAG_connexins <- PAG_scRNAseq_gene_lists$genes.icConnexins
PAG_connexins <- PAG_connexins[(PAG_connexins %in% rownames(PAG_data))]
plotExpression(PAG_sceset, PAG_connexins, x = "cell.type", colour_by = "PAG.areacollection", exprs_values = "counts", ncol = 5, show_median = TRUE, log=TRUE)
plotExpression(PAG_sceset, PAG_connexins, x = "PAG.areacollection", colour_by = "cell.type", exprs_values = "counts", ncol = 5, show_median = TRUE, log=TRUE)

plotHeatmap(PAG_sceset, PAG_connexins, columns = 1:48, exprs_values = "logcounts_raw", center = FALSE, cluster_rows = FALSE, cluster_cols = FALSE, zlim = c(0,10), color = colorRampPalette(c("white", "#009CB5"))(100), border_color=NA, fontsize=12, cellwidth = 15, cellheight = 25, show_colnames=FALSE)
plotHeatmap(PAG_sceset, PAG_connexins, columns = 49:96, exprs_values = "logcounts_raw", center = FALSE, cluster_rows = FALSE, cluster_cols = FALSE, zlim = c(0,10), color = colorRampPalette(c("white", "#ff777c"))(100), border_color=NA, fontsize=12, cellwidth = 15, cellheight = 25, show_colnames=FALSE)

```

### Plot Purinergic Channels
```{r}
PAG_purinergic <- PAG_scRNAseq_gene_lists$genes.icP2X
PAG_purinergic <- PAG_purinergic[(PAG_purinergic %in% rownames(PAG_data))]
plotExpression(PAG_sceset, PAG_purinergic, x = "cell.type", colour_by = "PAG.areacollection", exprs_values = "counts", ncol = 5, show_median = TRUE, log=TRUE)
plotExpression(PAG_sceset, PAG_purinergic, x = "PAG.areacollection", colour_by = "cell.type", exprs_values = "counts", ncol = 5, show_median = TRUE, log=TRUE)

plotHeatmap(PAG_sceset, PAG_purinergic, columns = 1:48, exprs_values = "logcounts_raw", center = FALSE, cluster_rows = FALSE, cluster_cols = FALSE, zlim = c(0,10), color = colorRampPalette(c("white", "#009CB5"))(100), border_color=NA, fontsize=12, cellwidth = 15, cellheight = 25, show_colnames=FALSE)
plotHeatmap(PAG_sceset, PAG_purinergic, columns = 49:96, exprs_values = "logcounts_raw", center = FALSE, cluster_rows = FALSE, cluster_cols = FALSE, zlim = c(0,10), color = colorRampPalette(c("white", "#ff777c"))(100), border_color=NA, fontsize=12, cellwidth = 15, cellheight = 25, show_colnames=FALSE)

```

### Plot Aquaporins Channels
```{r}
PAG_aquaporins <- PAG_scRNAseq_gene_lists$genes.icAquaporins
PAG_aquaporins <- PAG_aquaporins[(PAG_aquaporins %in% rownames(PAG_data))]
plotExpression(PAG_sceset, PAG_aquaporins, x = "cell.type", colour_by = "PAG.areacollection", exprs_values = "counts", ncol = 5, show_median = TRUE, log=TRUE)
plotExpression(PAG_sceset, PAG_aquaporins, x = "PAG.areacollection", colour_by = "cell.type", exprs_values = "counts", ncol = 5, show_median = TRUE, log=TRUE)

plotHeatmap(PAG_sceset, PAG_aquaporins, columns = 1:48, exprs_values = "logcounts_raw", center = FALSE, cluster_rows = FALSE, cluster_cols = FALSE, zlim = c(0,10), color = colorRampPalette(c("white", "#009CB5"))(100), border_color=NA, fontsize=12, cellwidth = 15, cellheight = 25, show_colnames=FALSE)
plotHeatmap(PAG_sceset, PAG_aquaporins, columns = 49:96, exprs_values = "logcounts_raw", center = FALSE, cluster_rows = FALSE, cluster_cols = FALSE, zlim = c(0,10), color = colorRampPalette(c("white", "#ff777c"))(100), border_color=NA, fontsize=12, cellwidth = 15, cellheight = 25, show_colnames=FALSE)

```

### Plot Other Channels
```{r}
PAG_otherchannels <- PAG_scRNAseq_gene_lists$genes.icOther
PAG_otherchannels <- PAG_otherchannels[(PAG_otherchannels %in% rownames(PAG_data))]
plotExpression(PAG_sceset, PAG_otherchannels, x = "cell.type", colour_by = "PAG.areacollection", exprs_values = "counts", ncol = 5, show_median = TRUE, log=TRUE)
plotExpression(PAG_sceset, PAG_otherchannels, x = "PAG.areacollection", colour_by = "cell.type", exprs_values = "counts", ncol = 5, show_median = TRUE, log=TRUE)

plotHeatmap(PAG_sceset, PAG_otherchannels, columns = 1:48, exprs_values = "logcounts_raw", center = FALSE, cluster_rows = FALSE, cluster_cols = FALSE, zlim = c(0,10), color = colorRampPalette(c("white", "#009CB5"))(100), border_color=NA, fontsize=12, cellwidth = 15, cellheight = 25, show_colnames=FALSE)
plotHeatmap(PAG_sceset, PAG_otherchannels, columns = 49:96, exprs_values = "logcounts_raw", center = FALSE, cluster_rows = FALSE, cluster_cols = FALSE, zlim = c(0,10), color = colorRampPalette(c("white", "#ff777c"))(100), border_color=NA, fontsize=12, cellwidth = 15, cellheight = 25, show_colnames=FALSE)
...
### Plot DE genes from Miguel Branco
```{r}
...
### Plot Data Club Neuromodulators
```{r}
DataClub_neuromodulators <- PAG_scRNAseq_gene_lists$genes.nmDataClub
DataClub_neuromodulators <- DataClub_neuromodulators[(DataClub_neuromodulators %in% rownames(PAG_data))]
plotExpression(PAG_sceset, DataClub_neuromodulators, x = "cell.type", colour_by = "PAG.areacollection", exprs_values = "counts", ncol = 10, show_median = TRUE, log=TRUE)
plotExpression(PAG_sceset, DataClub_neuromodulators, x = "PAG.areacollection", colour_by = "cell.type", exprs_values = "counts", ncol = 10, show_median = TRUE, log=TRUE)

plotHeatmap(PAG_sceset, DataClub_neuromodulators, columns = 1:48, exprs_values = "logcounts_raw", center = FALSE, cluster_rows = FALSE, cluster_cols = FALSE, zlim = c(0,10), color = colorRampPalette(c("white", "#009CB5"))(100), border_color=NA, fontsize=12, cellwidth = 15, cellheight = 25, show_colnames=FALSE)
plotHeatmap(PAG_sceset, DataClub_neuromodulators, columns = 49:96, exprs_values = "logcounts_raw", center = FALSE, cluster_rows = FALSE, cluster_cols = FALSE, zlim = c(0,10), color = colorRampPalette(c("white", "#ff777c"))(100), border_color=NA, fontsize=12, cellwidth = 15, cellheight = 25, show_colnames=FALSE)

```

### Plot Data Club Neuropeptides
```{r} 
...
### Plot Selected Genes
```{r}
PAG_selection <- PAG_scRNAseq_gene_lists$genes.OriolDataClub
PAG_selection <- PAG_selection[(PAG_selection %in% rownames(PAG_data))]

# VGAT and VGlut
plotExpression(PAG_sceset, PAG_selection[1:8], x = "cell.type", colour_by = "PAG.areacollection", exprs_values = "counts", ncol = 4, show_median = TRUE, log=TRUE, theme_size = 12)

# Opiods and Nociceptins
plotExpression(PAG_sceset, PAG_selection[9:20], x = "cell.type", colour_by = "PAG.areacollection", exprs_values = "counts", ncol = 4, show_median = TRUE, log=TRUE)

# Neuropeptides and Neuromodulators
plotExpression(PAG_sceset, PAG_selection[21:35], x = "cell.type", colour_by = "PAG.areacollection", exprs_values = "counts", ncol = 4, show_median = TRUE, log=TRUE)
plotExpression(PAG_sceset, PAG_selection[35:40], x = "PAG.areacollection", colour_by = "cell.type", exprs_values = "counts", ncol = 6, show_median = TRUE, log=TRUE)

# Ion Channels
plotExpression(PAG_sceset, PAG_selection[39:45], x = "cell.type", colour_by = "PAG.areacollection", exprs_values = "counts", ncol = 6, show_median = TRUE, log=TRUE)
```

## Trying Heatmap plots
```{r}
# cluster_rows = FALSE, cluster_cols = FALSE
# Use PAG_ionchannels[1:20] to select a subset of ion channels
# Use columns = 1:48 to plot VGluT2 and columns 49:96 to plot VGAT
# colour_columns_by = "PAG.areacollection" or "cell.type"
