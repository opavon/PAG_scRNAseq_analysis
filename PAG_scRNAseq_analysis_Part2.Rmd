...
An easy way to check everything is as it should be is to look whether each cell expresses only the fluorophor it should according to the transgenic line it was acquired from. A simple scatterplot of log-transformed counts of EYFP and tdTomato should be enough.
```{r}
...
plot(PAG_sceset[, PAG_sceset$cell.fluorophor=="EYFP"]$total_counts_EYFP,
     PAG_sceset[, PAG_sceset$cell.fluorophor=="EYFP"]$total_counts_tdTomato,
     xlim = c(0, max(PAG_sceset$total_counts_EYFP, PAG_sceset$total_counts_tdTomato)),
     ylim = c(0, max(PAG_sceset$total_counts_EYFP, PAG_sceset$total_counts_tdTomato)),
     xlab = "EYFP (counts)", ylab = "tdTomato (counts)",
     type = "p", pch = 19, cex = 1.25, 
     col = rgb(255, 200, 0, 255/2, maxColorValue = 255))

# Add the equivalent plot for tdTomato cells
points(PAG_sceset[, PAG_sceset$cell.fluorophor=="tdTomato"]$total_counts_EYFP,
       PAG_sceset[, PAG_sceset$cell.fluorophor=="tdTomato"]$total_counts_tdTomato,
       type = "p", pch = 19, cex = 1,
       col = rgb(255, 0, 0, 255/6, maxColorValue = 255))
```

### 2.3.4 | VGAT/VGluT2 expression
We isolated cells from transgenic animals expressing either EYFP or tdTomato as a reporter under VGAT or VGluT2. Do they only express one or the other, or do some cells express both? If so, in which proportions? The coexpression of VGAT and VGluT2 has been reported previously, so it shouldnt't surprise us if this ocurs. However, if we find cells that only express the opposite transporter as they should (e.g. if a VGAT-Cre cell only expresses VGluT2) we should consider excluding it (although again, it might just be dropout events). 
```{r}
...
plot(PAG_sceset[, PAG_sceset$cell.type=="VGAT"]$total_counts_VGAT,
     PAG_sceset[, PAG_sceset$cell.type=="VGAT"]$total_counts_VGluT2,
     xlim = c(0, max(PAG_sceset$total_counts_VGAT)),
     ylim = c(0, max(PAG_sceset$total_counts_VGluT2)),
     xlab = "VGAT (counts)", ylab = "VGluT2 (counts)",
     type = "p", pch = 19, cex = 1.25, 
     col = rgb(255, 119, 124, 255/2, maxColorValue = 255))

# Add the equivalent plot for VGluT2 cells
points(PAG_sceset[, PAG_sceset$cell.type=="VGluT2"]$total_counts_VGAT,
       PAG_sceset[, PAG_sceset$cell.type=="VGluT2"]$total_counts_VGluT2,
       type = "p", pch = 19, cex = 1,
       col = rgb(0, 156, 181, 255/6, maxColorValue = 255))
```
...
# Less than 35% of counts in control features
...
# Custom filters: identify the cells that have no expression of the expected fluorophor and express the wrong one
filter_by_wrong_flurorophor_EYFP_cells <- PAG_sceset$cell.fluorophor=="EYFP" & PAG_sceset$log2_total_counts_tdTomato > 0
table(filter_by_wrong_flurorophor_EYFP_cells)

filter_by_wrong_flurorophor_tdTomato_cells <- PAG_sceset$cell.fluorophor=="tdTomato" & PAG_sceset$log2_total_counts_EYFP > 0
table(filter_by_wrong_flurorophor_tdTomato_cells)

# Custom filters: also label the cells not expressing any fluorophor, although they are likely to be dropout events and don't really tell us much
cells_nofluorophor <- PAG_sceset$log2_total_counts_EYFP == 0 & PAG_sceset$log2_total_counts_tdTomato == 0
table(cells_nofluorophor)

# Custom filters: from the VGAT cells not expressing VGAT, mark the ones expressing VGluT2
filter_by_wrong_transporter_VGAT_cells <- PAG_sceset$cell.type=="VGAT" & PAG_sceset$log2_total_counts_VGAT == 0 & PAG_sceset$log2_total_counts_VGluT2 > 0
table(filter_by_wrong_transporter_VGAT_cells)

# Custom filters: from the VGluT2 cells not expressing VGluT2, mark the ones expressing VGAT
filter_by_wrong_transporter_VGluT2_cells <- PAG_sceset$cell.type=="VGluT2" & PAG_sceset$log2_total_counts_VGAT > 0 & PAG_sceset$log2_total_counts_VGluT2 == 0
table(filter_by_wrong_transporter_VGluT2_cells)

# Custom filters: mark any cells with no expression of either VGAT nor VGluT2 (again, these are likely to be dropouts)
cells_noVGAT_noVGluT2 <- PAG_sceset$log2_total_counts_VGAT == 0 & PAG_sceset$log2_total_counts_VGluT2 == 0
table(cells_noVGAT_noVGluT2)
```
...
PAG_sceset$manual_filters <- !(
    ...
    filter_by_wrong_flurorophor_EYFP_cells |
    # Remove tdTomato cells expressing EYFP
    filter_by_wrong_flurorophor_tdTomato_cells |   
    # Remove VGAT cells expressing only VGluT2
    filter_by_wrong_transporter_VGAT_cells |    
    # Remove VGluT2 cells expressing only VGAT
    filter_by_wrong_transporter_VGluT2_cells
    )

data.frame(ByLibSize=sum(filter_by_total_counts_manual), 
           ...
           ByPatchseqRecorded=sum(filter_patchseq_cells),
           ByWrongFlurorophorEYFP=sum(filter_by_wrong_flurorophor_EYFP_cells),
           ByWrongFlurorophortdTomato=sum(filter_by_wrong_flurorophor_tdTomato_cells),
           ByWrongTransporterVGAT=sum(filter_by_wrong_transporter_VGAT_cells),
           ByWrongTransporterVGluT2=sum(filter_by_wrong_transporter_VGluT2_cells),
...
```{r}
plyr::ldply(list(ByLibSize=colnames(PAG_sceset[, filter_by_total_counts_manual]),
                 ByExprFeatures=colnames(PAG_sceset[, filter_by_expr_features_manual]),
                 ByERCC=colnames(PAG_sceset[, filter_by_pct_ERCC_manual]),
                 ByMitochondrial=colnames(PAG_sceset[, filter_by_pct_Mitochondrial_manual]),
                 ByRibosomal=colnames(PAG_sceset[, filter_by_pct_Ribosomal_manual]),
                 ByTSOconcatamer=colnames(PAG_sceset[, filter_by_pct_TSO_concatamer_manual]),
                 ByCtrlFeatures=colnames(PAG_sceset[, filter_by_pct_counts_feature_control_manual]),
                 ByAqueduct=colnames(PAG_sceset[, filter_AQ_cells]),
                 ByPatchseqRecorded=colnames(PAG_sceset[, filter_patchseq_cells]),
                 ByWrongFlurorophorEYFP=colnames(PAG_sceset[, filter_by_wrong_flurorophor_EYFP_cells]),
                 ByWrongFlurorophortdTomato=colnames(PAG_sceset[, filter_by_wrong_flurorophor_tdTomato_cells]),
                 ByWrongTransporterVGAT=colnames(PAG_sceset[, filter_by_wrong_transporter_VGAT_cells]),
                 ByWrongTransporterVGluT2=colnames(PAG_sceset[, filter_by_wrong_transporter_VGluT2_cells])
                 ), 
            rbind)
```
...
```{r}
# Total counts
filter_by_total_counts_MADs <- isOutlier(PAG_sceset$total_counts, nmads = 5, type = "lower", log = TRUE, batch = NULL)
table(filter_by_total_counts_MADs)

# Expressed features
filter_by_expr_features_MADs <- isOutlier(PAG_sceset$total_features_by_counts, nmads = 5, type = "lower", log = TRUE, batch = NULL)
...
```{r}
# ERCCs
filter_by_pct_ERCC_MADs <- isOutlier(PAG_sceset$pct_counts_ERCC, nmads = 5, type = "higher", batch = NULL)
table(filter_by_pct_ERCC_MADs)

# Mitochondrial genes
filter_by_pct_Mitochondrial_MADs <- isOutlier(PAG_sceset$pct_counts_Mitochondrial, nmads = 5, type = "higher", batch = NULL)
table(filter_by_pct_Mitochondrial_MADs)

# Ribosomal genes
filter_by_pct_Ribosomal_MADs <- isOutlier(PAG_sceset$pct_counts_Ribosomal, nmads = 5, type = "higher", batch = NULL)
table(filter_by_pct_Ribosomal_MADs)

# TSO concatamers
filter_by_pct_TSO_concatamer_MADs <- isOutlier(PAG_sceset$pct_counts_TSO_concatamers, nmads = 5, type = "higher", batch = NULL)
table(filter_by_pct_TSO_concatamer_MADs)

# Control features
filter_by_pct_counts_feature_control_MADs <- isOutlier(PAG_sceset$pct_counts_feature_control, nmads = 5, type = "higher", batch = NULL)
...
data.frame(ByLibSize=sum(filter_by_total_counts_MADs), 
