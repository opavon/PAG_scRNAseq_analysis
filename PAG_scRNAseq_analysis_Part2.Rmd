...
# EYFP, tdTomato, Cre, and TSO concatemers
is_tdTomato <- which(rowData(PAG_sceset)$Chromosome == "A") # tdTomato fluorophore
# Alternatively, if using gene_name use <- grepl("^tdTomato", rownames(PAG_sceset))
is_EYFP <- which(rowData(PAG_sceset)$Chromosome == "B") # EYFP fluorophore
# Alternatively, if using gene_name use <- grepl("^EYFP", rownames(PAG_sceset))
is_Cre <- which(rowData(PAG_sceset)$Chromosome == "C") # Cre transgene to express fluorophores
# Alternatively, if using gene_name use <- grepl("^Cre", rownames(PAG_sceset))
is_SmartSeqTSO <- which(rowData(PAG_sceset)$Chromosome == "D") # TSO concatemers
...
PAG_sceset <- addPerCellQC(PAG_sceset,
                           subsets = list(#ERCC = isSpike(PAG_sceset, "ERCC"), # Deprecated
                                          ...
                                          EYFP = is_EYFP,
                                          Cre = is_Cre,
                                          TSO_concatemers = is_SmartSeqTSO,
                                          VGAT = is_VGAT,
                                          VGluT2 = is_VGluT2
                                          )
                           )
...
```{r}
# Transform units to millions for a nicer plot:
PAG_sceset$total_million <- PAG_sceset$total/1e6

# Plot the histograms of the library sizes and number of detected genes:
par(mfrow = c(1,2)) # Draw figures in a 1 by 2 array.

hist(PAG_sceset$total_million,
     breaks = 50,
     xlab = "Reads per sample (millions)", main = "Library size", col = "grey80", ylab = "Number of cells")
abline(v = c(0.4, 4.6), col = "blue", lwd = 2, lty = 2) # less than 400K reads and more than 4.6M

hist(PAG_sceset$detected, 
     breaks = 50,
     xlab = "Number of detected genes", main = "Detected genes", col = "grey80", ylab = "Number of cells")
abline(v = c(5000, 13000), col = "blue", lwd = 2, lty = 2) # exclude less than 5,000 features and more than 13K (potential doublets?)
```

Let's quickly take a look at the percentages of cells above a certain cutoff for total number of reads, uniquely mapping reads, and % uniquely mapping reads.
```{r}
# Total sequenced reads
summary(PAG_sceset$reads.total/1e6)

# Uniquely mapping reads
summary(PAG_sceset$reads.unique/1e6)

# % uniquely mapping reads
summary(PAG_sceset$reads.percentunique)
sum(PAG_sceset$reads.percentunique > 70) # number of cells with at least 70% of reads uniquely mapping to the genome
sum(PAG_sceset$reads.percentunique > 70) / ncol(PAG_sceset) * 100 # % of cells with at least 70% of reads uniquely mapping to the genome

# Total reads mapped to annotated genes
summary(PAG_sceset$total/1e6)

# % of total reads mapped to annotated genes
summary((PAG_sceset$total/1e6) / (PAG_sceset$reads.total/1e6) * 100)
summary((PAG_sceset$total/1e6) / (PAG_sceset$reads.unique/1e6) * 100)
...
### 2.3.2 | ERCCs, Mitochondrial and Ribosomal RNAs, and TSO concatemers
...
_Using ERCC for normalisation vs using them for QC_: ERCCs contain 96 different sequences at increasing concentrations. If you use a very diluted volume (1:5,000,000) you will only get the ones at a higher concentration, so it will not be useful for normalisation, but it will still be useful for QC. If you want to use them for normalisation, you would need roughly 50% of your reads to be from ERCCs (which might be a waste of reads if you are not benchmarking different methods). We will use them for QC only, in which case you would want about 5-10% of the reads going to the spike-ins. If this is not the case, it suggests that you need to alter the dilution.
```{r}
...
hist(PAG_sceset$altexps_ERCC_percent,
     breaks = 50,
     xlab = "ERCC proportion (%)", ylab = "Number of cells", main = "ERCCs", col = "grey80")
abline(v = 1, col = "blue", lwd = 2, lty = 2)

# Mitochondrial genes
hist(PAG_sceset$subsets_Mitochondrial_percent,
     breaks = 50,
     xlab = "Mitochondrial proportion (%)", ylab = "Number of cells", main = "Mitochondrial genes", col = "grey80")
abline(v = 15, col = "blue", lwd = 2, lty = 2)

# Ribosomal genes
hist(PAG_sceset$subsets_Ribosomal_percent,
     breaks = 50,
     xlab = "Ribosomal proportion (%)", ylab = "Number of cells", main = "Ribosomal genes", col = "grey80")
abline(v = 20, col = "blue", lwd = 2, lty = 2)

# TSO concatemers
hist(PAG_sceset$subsets_TSO_concatemers_percent,
     breaks = 50,
     xlab = "TSO concatemers proportion (%)", ylab = "Number of cells", main = "TSO concatemers", col = "grey80")
...
summary(PAG_sceset$subsets_TSO_concatemers_percent)
```
...
plotColData(PAG_sceset,
            x = "detected",
            y = "subsets_TSO_concatemers_percent",
            colour_by = "PAG.area",
            other_fields = "cell.type") +
  facet_wrap(~cell.type) + 
  xlab("Number of detected genes") +
  ylab("TSO concatemers proportion (%)") +
  stat_smooth(method = "lm", se = FALSE, size = 0.5, fullrange = TRUE)
```
...
hist(PAG_sceset$subsets_EYFP_sum,
     breaks = 50,
     xlab = "EYFP (counts)", ylab = "Number of cells", main = "", col = "grey80")

# tdTomato
hist(PAG_sceset$subsets_tdTomato_sum,
     breaks = 50,
     xlab = "tdTomato (counts)", ylab = "Number of cells", main = "", col = "grey80")

# Cre
hist(PAG_sceset$subsets_Cre_sum,
     breaks = 50,
     xlab = "Cre (counts)", ylab = "Number of cells", main = "", col = "grey80")
```
...
theme_args_violin_byfluorophore <- theme(plot.title = element_text(size = 12, face = "bold"), 
                                         axis.title = element_text(size = 11, face = "plain"),
                                         axis.text = element_text(size = 10, face = "plain"), 
                                         legend.text = element_text(size = 9, face = "plain"), 
                                         strip.text = element_text(size = 10, face = "plain"))

## Violin plots from EYFP animals by Mouse ID
# Using counts
violin_fluorophores_EYFPbymouse_counts <- plotExpression(PAG_sceset[, PAG_sceset$cell.fluorophore == "EYFP"], c("EYFP", "tdTomato", "Cre"), x = "mouse.id",
                                                         exprs_values = "counts",
                                                         colour_by = "batch.processing", ncol = 3, theme_size = 11, point_alpha = 0.8) +
  ggtitle("Transgene expression in cells from Cre::EYFP animals (raw counts)") + xlab("Mouse ID") + ylab("Expression (raw counts)") + 
  theme_args_violin_bymouse

violin_fluorophores_EYFPbymouse_counts
saveRDS(violin_fluorophores_EYFPbymouse_counts, file = str_c(path_for_figures, "violin_fluorophores_EYFPbymouse_counts.rds"))

ggsave(filename = str_c(date, "_violin_fluorophores_EYFPbymouse_counts.pdf"),
       plot = violin_fluorophores_EYFPbymouse_counts,
...
violin_fluorophores_EYFPbymouse_logcounts_raw <- plotExpression(PAG_sceset[, PAG_sceset$cell.fluorophore == "EYFP"], c("EYFP", "tdTomato", "Cre"), x = "mouse.id",
                                                                ...
                                                                colour_by = "batch.processing", ncol = 3, theme_size = 11, point_alpha = 0.8) +
  ggtitle("Transgene expression in cells from Cre::EYFP animals (log2 raw counts)") + xlab("Mouse ID") + ylab("Expression (log2 raw counts)") + 
  theme_args_violin_bymouse

violin_fluorophores_EYFPbymouse_logcounts_raw
saveRDS(violin_fluorophores_EYFPbymouse_logcounts_raw, file = str_c(path_for_figures, "violin_fluorophores_EYFPbymouse_logcounts_raw.rds"))

ggsave(filename = str_c(date, "_violin_fluorophores_EYFPbymouse_logcounts_raw.pdf"),
       plot = violin_fluorophores_EYFPbymouse_logcounts_raw,
       device = "pdf", # or one of "eps", "ps", "tex" (pictex), "pdf", "jpeg", "tiff", "png", "bmp", "svg" or "wmf" (windows only).
       path = path_for_figures,
       width = 8, height = 4, units = "in", dpi = 300,
       family = "Arial", bg = "transparent"
...
## Violin plots from tdTomato animals by Mouse ID
# Using counts
violin_fluorophores_tdTomatobymouse_counts <- plotExpression(PAG_sceset[, PAG_sceset$cell.fluorophore == "tdTomato"], c("EYFP", "tdTomato", "Cre"), x = "mouse.id",
                                                             exprs_values = "counts",
                                                             colour_by = "batch.processing", ncol = 3, theme_size = 11, point_alpha = 0.8) +
  ggtitle("Transgene expression in cells from Cre::tdTomato animals (raw counts)") + xlab("Mouse ID") + ylab("Expression (raw counts)") + 
  theme_args_violin_bymouse

violin_fluorophores_tdTomatobymouse_counts
saveRDS(violin_fluorophores_tdTomatobymouse_counts, file = str_c(path_for_figures, "violin_fluorophores_tdTomatobymouse_counts.rds"))

ggsave(filename = str_c(date, "_violin_fluorophores_tdTomatobymouse_counts.pdf"),
       plot = violin_fluorophores_tdTomatobymouse_counts,
       device = "pdf", # or one of "eps", "ps", "tex" (pictex), "pdf", "jpeg", "tiff", "png", "bmp", "svg" or "wmf" (windows only).
       path = path_for_figures,
       width = 8, height = 4, units = "in", dpi = 300,
       family = "Arial", bg = "transparent"
...
# Using logcounts_raw
violin_fluorophores_tdTomatobymouse_logcounts_raw <- plotExpression(PAG_sceset[, PAG_sceset$cell.fluorophore == "tdTomato"], c("EYFP", "tdTomato", "Cre"), x = "mouse.id",
                                                                    exprs_values = "logcounts_raw",
                                                                    colour_by = "batch.processing", ncol = 3, theme_size = 11, point_alpha = 0.8) +
  ggtitle("Transgene expression in cells from Cre::tdTomato animals (log2 raw counts)") + xlab("Mouse ID") + ylab("Expression (log2 raw counts)") + 
  theme_args_violin_bymouse

violin_fluorophores_tdTomatobymouse_logcounts_raw
saveRDS(violin_fluorophores_tdTomatobymouse_logcounts_raw, file = str_c(path_for_figures, "violin_fluorophores_tdTomatobymouse_logcounts_raw.rds"))

ggsave(filename = str_c(date, "_violin_fluorophores_tdTomatobymouse_logcounts_raw.pdf"),
       plot = violin_fluorophores_tdTomatobymouse_logcounts_raw,
       device = "pdf", # or one of "eps", "ps", "tex" (pictex), "pdf", "jpeg", "tiff", "png", "bmp", "svg" or "wmf" (windows only).
       path = path_for_figures,
       width = 8, height = 4, units = "in", dpi = 300,
       family = "Arial", bg = "transparent"
...
## Violin plots by fluorophore expressed
# Using counts
violin_fluorophores_celltype_counts <- plotExpression(PAG_sceset, c("EYFP", "tdTomato"), x = "cell.fluorophore",
  ...
  ggtitle("Fluorophore expression levels (raw counts)") + xlab("Fluorophore") + ylab("Expression (raw counts)") +
  theme_args_violin_byfluorophore

violin_fluorophores_celltype_counts
saveRDS(violin_fluorophores_celltype_counts, file = str_c(path_for_figures, "violin_fluorophores_celltype_counts.rds"))

ggsave(filename = str_c(date, "_violin_fluorophores_celltype_counts.pdf"),
       plot = violin_fluorophores_celltype_counts,
       device = "pdf", # or one of "eps", "ps", "tex" (pictex), "pdf", "jpeg", "tiff", "png", "bmp", "svg" or "wmf" (windows only).
       path = path_for_figures,
       width = 8, height = 4, units = "in", dpi = 300,
       family = "Arial", bg = "transparent"
...
# Using logcounts_raw
violin_fluorophores_celltype_logcounts_raw <- plotExpression(PAG_sceset, c("EYFP", "tdTomato"), x = "cell.fluorophore",
                                                             exprs_values = "logcounts_raw", 
                                                             colour_by = "mouse.id", ncol = 2, theme_size = 11, point_alpha = 0.8) +
  ggtitle("Fluorophore expression levels (log2 raw counts)") + xlab("Fluorophore") + ylab("Expression (log2 raw counts)") +
  theme_args_violin_byfluorophore

violin_fluorophores_celltype_logcounts_raw
saveRDS(violin_fluorophores_celltype_logcounts_raw, file = str_c(path_for_figures, "violin_fluorophores_celltype_logcounts_raw.rds"))

ggsave(filename = str_c(date, "_violin_fluorophores_celltype_logcounts_raw.pdf"),
       plot = violin_fluorophores_celltype_logcounts_raw,
       device = "pdf", # or one of "eps", "ps", "tex" (pictex), "pdf", "jpeg", "tiff", "png", "bmp", "svg" or "wmf" (windows only).
       path = path_for_figures,
       width = 8, height = 4, units = "in", dpi = 300,
       family = "Arial", bg = "transparent"
...
plotExpression(PAG_sceset, "tdTomato", x = "EYFP", 
               exprs_values = "counts",
               colour_by = "cell.fluorophore", 
               xlab = "EYFP (raw counts)") + ylab("tdTomato (raw counts)")

plotExpression(PAG_sceset, "tdTomato", x = "EYFP", 
               exprs_values = "logcounts_raw",
               colour_by = "cell.fluorophore", 
               xlab = "EYFP (log2 raw counts)") + ylab("tdTomato (log2 raw counts)")

plotColData(PAG_sceset,
            x = "detected",
            y = "subsets_EYFP_sum",
            colour_by = "mouse.id",
            other_fields = "cell.fluorophore") +
  facet_wrap(~cell.fluorophore) + 
  xlab("Number of detected genes") +
  ylab("EYFP (counts)")
...
An easy way to check everything is as it should be is to look at whether each cell expresses only the fluorophore it should according to the transgenic line it was acquired from. A simple scatterplot of log-transformed counts of EYFP and tdTomato should be enough to reveal this.
```{r}
...
plot(PAG_sceset[, PAG_sceset$cell.fluorophore == "EYFP"]$subsets_EYFP_sum,
     PAG_sceset[, PAG_sceset$cell.fluorophore == "EYFP"]$subsets_tdTomato_sum,
     xlim = c(0, max(PAG_sceset$subsets_EYFP_sum)),
     ylim = c(0, max(PAG_sceset$subsets_tdTomato_sum)),
     main = "Scatter plot of fluorophore expression (raw counts)", font.main = 2, cex.main = 1.1,
     xlab = "EYFP (counts)", ylab = "tdTomato (counts)", font.lab = 1, cex.lab = 1,
     type = "p", pch = 19, cex = 1.25, las = 1, # plot axis labels always horizontal (defaults to 0: always parallel to the axis)
     col = rgb(255, 200, 0, 255/2, maxColorValue = 255))
# Add the equivalent plot for tdTomato cells
points(PAG_sceset[, PAG_sceset$cell.fluorophore == "tdTomato"]$subsets_EYFP_sum,
       PAG_sceset[, PAG_sceset$cell.fluorophore == "tdTomato"]$subsets_tdTomato_sum,
       type = "p", pch = 19, cex = 1, las = 1,
       col = rgb(255, 0, 0, 255/6, maxColorValue = 255))
legend("topright", col = c(rgb(255, 200, 0, 255/2, maxColorValue = 255), rgb(255, 0, 0, 255/6, maxColorValue = 255)), pch = 19, cex = 0.8,  bty = "n",
       legend = c("Cre::EYFP cells", "Cre::tdTomato cells"))

# In EYFP cells, plot the EYFP log2 total counts against the tdTomato counts
plot(PAG_sceset[, PAG_sceset$cell.fluorophore == "EYFP"]$log2_total_counts_EYFP,
     PAG_sceset[, PAG_sceset$cell.fluorophore == "EYFP"]$log2_total_counts_tdTomato,
     xlim = c(0, max(PAG_sceset$log2_total_counts_EYFP)),
     ylim = c(0, max(PAG_sceset$log2_total_counts_tdTomato)),
     main = "Scatter plot of fluorophore expression (log2 raw counts)", font.main = 2, cex.main = 1.1,
     xlab = "EYFP (log2 raw counts)", ylab = "tdTomato (log2 raw counts)", font.lab = 1, cex.lab = 1,
     type = "p", pch = 19, cex = 1.25, las = 1, # plot axis labels always horizontal (defaults to 0: always parallel to the axis)
     col = rgb(255, 200, 0, 255/2, maxColorValue = 255))
# Add the equivalent plot for tdTomato cells
points(PAG_sceset[, PAG_sceset$cell.fluorophore == "tdTomato"]$log2_total_counts_EYFP,
       PAG_sceset[, PAG_sceset$cell.fluorophore == "tdTomato"]$log2_total_counts_tdTomato,
       type = "p", pch = 19, cex = 1, las = 1,
       col = rgb(255, 0, 0, 255/6, maxColorValue = 255))
legend("topright", col = c(rgb(255, 200, 0, 255/2, maxColorValue = 255), rgb(255, 0, 0, 255/6, maxColorValue = 255)), pch = 19, cex = 0.8,  bty = "n",
       legend = c("Cre::EYFP cells", "Cre::tdTomato cells"))
...
### Alternative way of doing this (without having to transform counts and assign them to a new slot in colData)
## Scatter plot with counts
# In EYFP cells, plot the EYFP log2 total counts against the tdTomato counts
plot(assay(PAG_sceset[rownames(PAG_sceset)=="EYFP", PAG_sceset$cell.fluorophore == "EYFP"], "counts"),
     assay(PAG_sceset[rownames(PAG_sceset)=="tdTomato", PAG_sceset$cell.fluorophore == "EYFP"], "counts"),
     xlim = c(0, max(assay(PAG_sceset[rownames(PAG_sceset)=="EYFP", ], "counts"))),
     ylim = c(0, max(assay(PAG_sceset[rownames(PAG_sceset)=="tdTomato", ], "counts"))),
     main = "Scatter plot of fluorophore expression (raw counts)", font.main = 2, cex.main = 1.1,
     xlab = "EYFP (raw counts)", ylab = "tdTomato (raw counts)", font.lab = 1, cex.lab = 1,
     type = "p", pch = 19, cex = 1.25, las = 1, # plot axis labels always horizontal (defaults to 0: always parallel to the axis)
     col = rgb(255, 200, 0, 255/2, maxColorValue = 255))
# Add the equivalent plot for tdTomato cells
points(assay(PAG_sceset[rownames(PAG_sceset)=="EYFP", PAG_sceset$cell.fluorophore == "tdTomato"], "counts"),
       assay(PAG_sceset[rownames(PAG_sceset)=="tdTomato", PAG_sceset$cell.fluorophore == "tdTomato"], "counts"),
       type = "p", pch = 19, cex = 1, las = 1,
       col = rgb(255, 0, 0, 255/6, maxColorValue = 255))
legend("topright", col = c(rgb(255, 200, 0, 255/2, maxColorValue = 255), rgb(255, 0, 0, 255/6, maxColorValue = 255)), pch = 19, cex = 0.8,  bty = "n",
       legend = c("Cre::EYFP cells", "Cre::tdTomato cells"))
...
# Save the plot
pdf(file = str_c(path_for_figures, date, "_scatter_fluorophore_expression_counts.pdf"),
...
# In EYFP cells, plot the EYFP log2 total counts against the tdTomato counts
plot(assay(PAG_sceset[rownames(PAG_sceset)=="EYFP", PAG_sceset$cell.fluorophore == "EYFP"], "logcounts_raw"),
     assay(PAG_sceset[rownames(PAG_sceset)=="tdTomato", PAG_sceset$cell.fluorophore == "EYFP"], "logcounts_raw"),
     xlim = c(0, max(assay(PAG_sceset[rownames(PAG_sceset)=="EYFP", ], "logcounts_raw"))),
     ylim = c(0, max(assay(PAG_sceset[rownames(PAG_sceset)=="tdTomato", ], "logcounts_raw"))),
     main = "Scatter plot of fluorophore expression (log2 raw counts)", font.main = 2, cex.main = 1.1,
     xlab = "EYFP (log2 raw counts)", ylab = "tdTomato (log2 raw counts)", font.lab = 1, cex.lab = 1,
     type = "p", pch = 19, cex = 1.25, las = 1, # plot axis labels always horizontal (defaults to 0: always parallel to the axis)
     col = rgb(255, 200, 0, 255/2, maxColorValue = 255))
# Add the equivalent plot for tdTomato cells
points(assay(PAG_sceset[rownames(PAG_sceset)=="EYFP", PAG_sceset$cell.fluorophore == "tdTomato"], "logcounts_raw"),
       assay(PAG_sceset[rownames(PAG_sceset)=="tdTomato", PAG_sceset$cell.fluorophore == "tdTomato"], "logcounts_raw"),
       type = "p", pch = 19, cex = 1, las = 1,
       col = rgb(255, 0, 0, 255/6, maxColorValue = 255))
legend("topright", col = c(rgb(255, 200, 0, 255/2, maxColorValue = 255), rgb(255, 0, 0, 255/6, maxColorValue = 255)), pch = 19, cex = 0.8,  bty = "n",
       legend = c("Cre::EYFP cells", "Cre::tdTomato cells"))
...
# Save the plot
pdf(file = str_c(path_for_figures, date, "_scatter_fluorophore_expression_logcounts_raw.pdf"),
    width = 5, height = 5, pointsize = 11,
    family = "Arial", bg = "transparent")
plot(assay(PAG_sceset[rownames(PAG_sceset)=="EYFP", PAG_sceset$cell.fluorophore == "EYFP"], "logcounts_raw"),
     assay(PAG_sceset[rownames(PAG_sceset)=="tdTomato", PAG_sceset$cell.fluorophore == "EYFP"], "logcounts_raw"),
...
As we can see, we have a minority of cells that express both fluorophores when they should only express one. The safest is to exclude those, as there may have been contamination at one of the processing steps.

### 2.3.4 | VGAT/VGluT2 expression
We isolated cells from transgenic animals expressing either EYFP or tdTomato as a reporter under VGAT or VGluT2 promoters. However, we don't really know whether neurons in the midbrain area we are investigating express one or the other, or whether they express both? If the latter, in which proportions? We repeat the same approach we used for the fluorophores to inspect VGAT and VGluT2 expression.
```{r}
...
hist(PAG_sceset$subsets_VGAT_sum,
     breaks = 50,
     xlab = "VGAT (counts)", ylab = "Number of cells", main = "", col = "grey80")

# VGluT2
hist(PAG_sceset$subsets_VGluT2_sum,
     breaks = 50,
     xlab = "VGluT2 (counts)", ylab = "Number of cells", main = "", col = "grey80")
```
...
# Using counts
violin_VGATVGluT2_VGATbymouse_counts <- plotExpression(PAG_sceset[, PAG_sceset$cell.type == "VGAT"], c("Slc32a1", "Slc17a6"), x = "mouse.id",
                                                       exprs_values = "counts",
                                                       colour_by = "batch.processing", ncol = 2, theme_size = 11, point_alpha = 0.8) +
  ggtitle("Transporter expression levels in cells from VGAT::Cre animals (raw counts)") + xlab("Mouse ID") + ylab("Expression (raw counts)") + 
  theme_args_violin_bymouse

violin_VGATVGluT2_VGATbymouse_counts
saveRDS(violin_VGATVGluT2_VGATbymouse_counts, file = str_c(path_for_figures, "violin_VGATVGluT2_VGATbymouse_counts.rds"))

ggsave(filename = str_c(date, "_violin_VGATVGluT2_VGATbymouse_counts.pdf"),
       plot = violin_VGATVGluT2_VGATbymouse_counts,
       device = "pdf", # or one of "eps", "ps", "tex" (pictex), "pdf", "jpeg", "tiff", "png", "bmp", "svg" or "wmf" (windows only).
       path = path_for_figures,
       width = 8, height = 4, units = "in", dpi = 300,
       family = "Arial", bg = "transparent"
...
# Using logcounts_raw
violin_VGATVGluT2_VGATbymouse_logcounts_raw <- plotExpression(PAG_sceset[, PAG_sceset$cell.type == "VGAT"], c("Slc32a1", "Slc17a6"), x = "mouse.id",
                                                              exprs_values = "logcounts_raw",
                                                              colour_by = "batch.processing", ncol = 2, theme_size = 11, point_alpha = 0.8) +
  ggtitle("Transporter expression levels in cells from VGAT::Cre animals (log2 raw counts)") + xlab("Mouse ID") + ylab("Expression (log2 raw counts)") + 
  theme_args_violin_bymouse

violin_VGATVGluT2_VGATbymouse_logcounts_raw
saveRDS(violin_VGATVGluT2_VGATbymouse_logcounts_raw, file = str_c(path_for_figures, "violin_VGATVGluT2_VGATbymouse_logcounts_raw.rds"))

ggsave(filename = str_c(date, "_violin_VGATVGluT2_VGATbymouse_logcounts_raw.pdf"),
       plot = violin_VGATVGluT2_VGATbymouse_logcounts_raw,
       device = "pdf", # or one of "eps", "ps", "tex" (pictex), "pdf", "jpeg", "tiff", "png", "bmp", "svg" or "wmf" (windows only).
       path = path_for_figures,
       width = 8, height = 4, units = "in", dpi = 300,
       family = "Arial", bg = "transparent"
...
# Using counts
violin_VGATVGluT2_VGluT2bymouse_counts <- plotExpression(PAG_sceset[, PAG_sceset$cell.type == "VGluT2"], c("Slc32a1", "Slc17a6"), x = "mouse.id",
                                                         exprs_values = "counts",
                                                         colour_by = "batch.processing", ncol = 2, theme_size = 11, point_alpha = 0.8) +
  ggtitle("Transporter expression levels in cells from VGluT2::Cre animals (raw counts)") + xlab("Mouse ID") + ylab("Expression (raw counts)") + 
  theme_args_violin_bymouse

violin_VGATVGluT2_VGluT2bymouse_counts
saveRDS(violin_VGATVGluT2_VGluT2bymouse_counts, file = str_c(path_for_figures, "violin_VGATVGluT2_VGluT2bymouse_counts.rds"))

ggsave(filename = str_c(date, "_violin_VGATVGluT2_VGluT2bymouse_counts.pdf"),
       plot = violin_VGATVGluT2_VGluT2bymouse_counts,
       device = "pdf", # or one of "eps", "ps", "tex" (pictex), "pdf", "jpeg", "tiff", "png", "bmp", "svg" or "wmf" (windows only).
       path = path_for_figures,
       width = 8, height = 4, units = "in", dpi = 300,
       family = "Arial", bg = "transparent"
...
# Using logcounts_raw
violin_VGATVGluT2_VGluT2bymouse_logcounts_raw <- plotExpression(PAG_sceset[, PAG_sceset$cell.type == "VGluT2"], c("Slc32a1", "Slc17a6"), x = "mouse.id",
                                                                exprs_values = "logcounts_raw",
                                                                colour_by = "batch.processing", ncol = 2, theme_size = 11, point_alpha = 0.8) +
  ggtitle("Transporter expression levels in cells from VGluT2::Cre animals (log2 raw counts)") + xlab("Mouse ID") + ylab("Expression (log2 raw counts)") + 
  theme_args_violin_bymouse

violin_VGATVGluT2_VGluT2bymouse_logcounts_raw
saveRDS(violin_VGATVGluT2_VGluT2bymouse_logcounts_raw, file = str_c(path_for_figures, "violin_VGATVGluT2_VGluT2bymouse_logcounts_raw.rds"))

ggsave(filename = str_c(date, "_violin_VGATVGluT2_VGluT2bymouse_logcounts_raw.pdf"),
       plot = violin_VGATVGluT2_VGluT2bymouse_logcounts_raw,
       device = "pdf", # or one of "eps", "ps", "tex" (pictex), "pdf", "jpeg", "tiff", "png", "bmp", "svg" or "wmf" (windows only).
       path = path_for_figures,
       width = 8, height = 4, units = "in", dpi = 300,
       family = "Arial", bg = "transparent"
...
# Using counts
violin_VGATVGluT2_celltype_counts <- plotExpression(PAG_sceset, c("Slc32a1", "Slc17a6"), x = "cell.type",
                                                    exprs_values = "counts", 
                                                    colour_by = "mouse.id", ncol = 2, theme_size = 11, point_alpha = 0.8) +
  ggtitle("Transporter expression levels (raw counts)") + xlab("Cell type") + ylab("Expression (raw counts)") +
  theme_args_violin_bytype

violin_VGATVGluT2_celltype_counts
saveRDS(violin_VGATVGluT2_celltype_counts, file = str_c(path_for_figures, "violin_VGATVGluT2_celltype_counts.rds"))

ggsave(filename = str_c(date, "_violin_VGATVGluT2_celltype_counts.pdf"),
       plot = violin_VGATVGluT2_celltype_counts,
       device = "pdf", # or one of "eps", "ps", "tex" (pictex), "pdf", "jpeg", "tiff", "png", "bmp", "svg" or "wmf" (windows only).
       path = path_for_figures,
       width = 8, height = 4, units = "in", dpi = 300,
       family = "Arial", bg = "transparent"
...
# Using logcounts_raw
violin_VGATVGluT2_celltype_logcounts_raw <- plotExpression(PAG_sceset, c("Slc32a1", "Slc17a6"), x = "cell.type",
                                                           exprs_values = "logcounts_raw", 
                                                           colour_by = "mouse.id", ncol = 2, theme_size = 11, point_alpha = 0.8) +
  ggtitle("Transporter expression levels (log2 raw counts)") + xlab("Cell type") + ylab("Expression (log2 raw counts)") +
  theme_args_violin_bytype

violin_VGATVGluT2_celltype_logcounts_raw
saveRDS(violin_VGATVGluT2_celltype_logcounts_raw, file = str_c(path_for_figures, "violin_VGATVGluT2_celltype_logcounts_raw.rds"))

ggsave(filename = str_c(date, "_violin_VGATVGluT2_celltype_logcounts_raw.pdf"),
       plot = violin_VGATVGluT2_celltype_logcounts_raw,
       device = "pdf", # or one of "eps", "ps", "tex" (pictex), "pdf", "jpeg", "tiff", "png", "bmp", "svg" or "wmf" (windows only).
       path = path_for_figures,
       width = 8, height = 4, units = "in", dpi = 300,
       family = "Arial", bg = "transparent"
...
plotExpression(PAG_sceset, "Slc32a1", x = "Slc17a6", 
               exprs_values = "counts", 
               colour_by = "cell.type",
               xlab = "VGluT2 (raw counts)") + ylab("VGAT (raw counts)")

plotExpression(PAG_sceset, "Slc32a1", x = "Slc17a6", 
               exprs_values = "logcounts_raw", 
               colour_by = "cell.type",
               xlab = "VGluT2 (log2 raw counts)") + ylab("VGAT (log2 raw counts)")
...
#      xlab = "VGAT (log2 raw counts)", ylab = "VGluT2 (log2 raw counts)",
#      type = "p", pch = 19, cex = 1.25, las = 1,
#      col = rgb(255, 119, 124, 255/2, maxColorValue = 255))
# # Add the equivalent plot for VGluT2 cells
...
### Alternative way of doing this (without having to transform counts and assign them to colData)
## Scatter plot with counts
# In VGAT cells, plot the VGAT log2 counts against the VGluT2 counts
plot(assay(PAG_sceset[rownames(PAG_sceset)=="Slc32a1", PAG_sceset$cell.type == "VGAT"], "counts"),
     assay(PAG_sceset[rownames(PAG_sceset)=="Slc17a6", PAG_sceset$cell.type == "VGAT"], "counts"),
     xlim = c(0, max(assay(PAG_sceset[rownames(PAG_sceset)=="Slc32a1", ], "counts"))),
     ylim = c(0, max(assay(PAG_sceset[rownames(PAG_sceset)=="Slc17a6", ], "counts"))),
     main = "Scatter plot of VGAT-VGluT2 expression (raw counts)", font.main = 2, cex.main = 1.1,
     xlab = "VGAT (raw counts)", ylab = "VGluT2 (raw counts)", font.lab = 1, cex.lab = 1,
     type = "p", pch = 19, cex = 1.25, las = 1, # plot axis labels always horizontal (defaults to 0: always parallel to the axis)
     col = rgb(255, 119, 124, 255/2, maxColorValue = 255))
# Add the equivalent plot for VGluT2 cells
points(assay(PAG_sceset[rownames(PAG_sceset)=="Slc32a1", PAG_sceset$cell.type == "VGluT2"], "counts"),
       assay(PAG_sceset[rownames(PAG_sceset)=="Slc17a6", PAG_sceset$cell.type == "VGluT2"], "counts"),
       type = "p", pch = 19, cex = 1, las = 1,
       col = rgb(0, 156, 181, 255/6, maxColorValue = 255))
...
# Save the plot
pdf(file = str_c(path_for_figures, date, "_scatter_VGATVGluT2_expression_counts.pdf"),
    width = 5, height = 5, pointsize = 11,
    family = "Arial", bg = "transparent")
plot(assay(PAG_sceset[rownames(PAG_sceset)=="Slc32a1", PAG_sceset$cell.type == "VGAT"], "counts"),
     assay(PAG_sceset[rownames(PAG_sceset)=="Slc17a6", PAG_sceset$cell.type == "VGAT"], "counts"),
...
## Scatter plot with logcounts_raw
# In VGAT cells, plot the VGAT log2 counts against the VGluT2 counts
plot(assay(PAG_sceset[rownames(PAG_sceset)=="Slc32a1", PAG_sceset$cell.type == "VGAT"], "logcounts_raw"),
     assay(PAG_sceset[rownames(PAG_sceset)=="Slc17a6", PAG_sceset$cell.type == "VGAT"], "logcounts_raw"),
     xlim = c(0, max(assay(PAG_sceset[rownames(PAG_sceset)=="Slc32a1", ], "logcounts_raw"))),
     ylim = c(0, max(assay(PAG_sceset[rownames(PAG_sceset)=="Slc17a6", ], "logcounts_raw"))),
     main = "Scatter plot of VGAT-VGluT2 expression (log2 raw counts)", font.main = 2, cex.main = 1.1,
     xlab = "VGAT (log2 raw counts)", ylab = "VGluT2 (log2 raw counts)", font.lab = 1, cex.lab = 1,
     type = "p", pch = 19, cex = 1.25, las = 1, # plot axis labels always horizontal (defaults to 0: always parallel to the axis)
     col = rgb(255, 119, 124, 255/2, maxColorValue = 255))
# Add the equivalent plot for VGluT2 cells
points(assay(PAG_sceset[rownames(PAG_sceset)=="Slc32a1", PAG_sceset$cell.type == "VGluT2"], "logcounts_raw"),
       assay(PAG_sceset[rownames(PAG_sceset)=="Slc17a6", PAG_sceset$cell.type == "VGluT2"], "logcounts_raw"),
       type = "p", pch = 19, cex = 1, las = 1,
       col = rgb(0, 156, 181, 255/6, maxColorValue = 255))
legend("topright", col = c(rgb(255, 119, 124, 255/2, maxColorValue = 255), rgb(0, 156, 181, 255/6, maxColorValue = 255)), pch = 19, cex = 0.8,  bty = "n",
       legend = c("VGAT::Cre cells", "VGluT2::Cre cells"))
...
# Save the plot
pdf(file = str_c(path_for_figures, date, "_scatter_VGATVGluT2_expression_logcounts_raw.pdf"),
    width = 5, height = 5, pointsize = 11,
    family = "Arial", bg = "transparent")
plot(assay(PAG_sceset[rownames(PAG_sceset)=="Slc32a1", PAG_sceset$cell.type == "VGAT"], "logcounts_raw"),
     assay(PAG_sceset[rownames(PAG_sceset)=="Slc17a6", PAG_sceset$cell.type == "VGAT"], "logcounts_raw"),
...
As we can see, a large proportion of cells express both VGAT and VGluT2. The coexpression of VGAT and VGluT2 has been reported previously in midbrain, so it shouldnt't be surprising. However, it may also come from our isolation method, as we may be aspirating synaptic terminals from neighbouring neurons, so we will not be using this metric to exclude cells. Similarly, if we find cells that only express the opposite transporter as they should (e.g. if a VGAT-Cre cell only expresses VGluT2) we should consider excluding them (although again, these might just be dropout events, so we can use combinatorial QC metrics to err on the safe side).
...
summary(PAG_sceset[,PAG_sceset$filter_by_total_counts_manual]$PAG.area)
...
summary(PAG_sceset[,PAG_sceset$filter_by_expr_features_manual]$PAG.area)
...
summary(PAG_sceset[,PAG_sceset$filter_by_percent_top_100_manual]$PAG.area)
...
summary(PAG_sceset[,PAG_sceset$filter_by_pct_ERCC_manual]$PAG.area)
...
summary(PAG_sceset[,PAG_sceset$filter_by_pct_Mitochondrial_manual]$PAG.area)
...
summary(PAG_sceset[,PAG_sceset$filter_by_pct_Ribosomal_VGAT_manual]$PAG.area)
...
summary(PAG_sceset[,PAG_sceset$filter_by_pct_Ribosomal_VGluT2_manual]$PAG.area)

PAG_sceset$filter_by_pct_Ribosomal_manual <- PAG_sceset$filter_by_pct_Ribosomal_VGAT_manual | PAG_sceset$filter_by_pct_Ribosomal_VGluT2_manual

# Not more than 5% of counts in TSO concatemers
PAG_sceset$filter_by_pct_TSO_concatemer_manual <- (PAG_sceset$subsets_TSO_concatemers_percent > 5)
table(PAG_sceset$filter_by_pct_TSO_concatemer_manual)
summary(PAG_sceset[,PAG_sceset$filter_by_pct_TSO_concatemer_manual]$PAG.area)
...
summary(PAG_sceset[,PAG_sceset$filter_AQ_cells]$PAG.area)

PAG_sceset$filter_patchseq_cells <- (PAG_sceset$patch.recorded == "yes")
table(PAG_sceset$filter_patchseq_cells)
summary(PAG_sceset[,PAG_sceset$filter_patchseq_cells]$PAG.area)

# Custom filters: identify the cells that express the wrong fluorophore
PAG_sceset$filter_by_wrong_fluorophore_EYFP_cells <- (PAG_sceset$cell.fluorophore == "EYFP" & PAG_sceset$subsets_tdTomato_sum > 0)
table(PAG_sceset$filter_by_wrong_fluorophore_EYFP_cells)
summary(PAG_sceset[,PAG_sceset$filter_by_wrong_fluorophore_EYFP_cells]$PAG.area)

PAG_sceset$filter_by_wrong_fluorophore_tdTomato_cells <- (PAG_sceset$cell.fluorophore == "tdTomato" & PAG_sceset$subsets_EYFP_sum > 0)
table(PAG_sceset$filter_by_wrong_fluorophore_tdTomato_cells)
summary(PAG_sceset[,PAG_sceset$filter_by_wrong_fluorophore_tdTomato_cells]$PAG.area)
...
#summary(PAG_sceset[,PAG_sceset$filter_by_wrong_Xist_in_males]$PAG.area)

# Custom filters: label the cells not expressing any fluorophore to use them in combination of other filters
PAG_sceset$cells_nofluorophore <- (PAG_sceset$subsets_EYFP_sum == 0 & PAG_sceset$subsets_tdTomato_sum == 0)
table(PAG_sceset$cells_nofluorophore)
summary(PAG_sceset[,PAG_sceset$cells_nofluorophore]$PAG.area)

# Custom filters: from the VGAT cells not expressing VGAT, mark the ones expressing VGluT2 and not expressing any fluorophore (they might be wrongly aspirated cells - e.g. a neighbouring non-fluorescent cell was aspirated instead of the fluorescent one)
PAG_sceset$filter_by_wrong_transporter_VGAT_cells <- (PAG_sceset$cell.type == "VGAT" & PAG_sceset$cells_nofluorophore == TRUE & PAG_sceset$subsets_VGAT_sum == 0 & PAG_sceset$subsets_VGluT2_sum > 0)
table(PAG_sceset$filter_by_wrong_transporter_VGAT_cells)
summary(PAG_sceset[,PAG_sceset$filter_by_wrong_transporter_VGAT_cells]$PAG.area)

# Custom filters: from the VGluT2 cells not expressing VGluT2, mark the ones expressing VGAT and not expressing any fluorophore (they might be wrongly aspirated cells - e.g. a neighbouring non-fluorescent cell was aspirated instead of the fluorescent one)
PAG_sceset$filter_by_wrong_transporter_VGluT2_cells <- (PAG_sceset$cell.type == "VGluT2" & PAG_sceset$cells_nofluorophore == TRUE & PAG_sceset$subsets_VGAT_sum > 0 & PAG_sceset$subsets_VGluT2_sum == 0)
table(PAG_sceset$filter_by_wrong_transporter_VGluT2_cells)
summary(PAG_sceset[,PAG_sceset$filter_by_wrong_transporter_VGluT2_cells]$PAG.area)

# Custom filters: mark any cells with no expression of either VGAT nor VGluT2 nor fluorophore (again, these are likely to be dropouts)
PAG_sceset$cells_noVGAT_noVGluT2 <- (PAG_sceset$subsets_VGAT_sum == 0 & PAG_sceset$subsets_VGluT2_sum == 0 & PAG_sceset$cells_nofluorophore == TRUE)
table(PAG_sceset$cells_noVGAT_noVGluT2)
summary(PAG_sceset[,PAG_sceset$cells_noVGAT_noVGluT2]$PAG.area)
```
...
table(PAG_sceset$VGAT_VGluT2_expression, PAG_sceset$PAG.area)
...
PAG_sceset$manual_filters <- !(
    ...
    # Remove cells with unusual proportion of reads in TSO concatemers
    PAG_sceset$filter_by_pct_TSO_concatemer_manual |
    ...
    PAG_sceset$filter_by_wrong_fluorophore_EYFP_cells |
    # Remove tdTomato cells expressing EYFP
    PAG_sceset$filter_by_wrong_fluorophore_tdTomato_cells |   
...
data.frame(ByLibSize = sum(PAG_sceset$filter_by_total_counts_manual), 
           ...
           ByTSOconcatemer = sum(PAG_sceset$filter_by_pct_TSO_concatemer_manual),
           ...
           ByWrongFluorophoreEYFP = sum(PAG_sceset$filter_by_wrong_fluorophore_EYFP_cells),
           ByWrongFluorophoretdTomato = sum(PAG_sceset$filter_by_wrong_fluorophore_tdTomato_cells),
...
table(PAG_sceset$manual_filters, PAG_sceset$PAG.area)
...
plyr::ldply(list(ByLibSize = colnames(PAG_sceset[, PAG_sceset$filter_by_total_counts_manual]),
                 ...
                 ByTSOconcatemer = colnames(PAG_sceset[, PAG_sceset$filter_by_pct_TSO_concatemer_manual]),
                 ...
                 ByWrongFluorophoreEYFP = colnames(PAG_sceset[, PAG_sceset$filter_by_wrong_fluorophore_EYFP_cells]),
                 ByWrongFluorophoretdTomato = colnames(PAG_sceset[, PAG_sceset$filter_by_wrong_fluorophore_tdTomato_cells]),
...
# TSO concatemers
PAG_sceset$filter_by_pct_TSO_concatemer_MADs <- isOutlier(PAG_sceset$subsets_TSO_concatemers_percent, nmads = 5, type = "higher", batch = NULL)
table(PAG_sceset$filter_by_pct_TSO_concatemer_MADs)
attr(PAG_sceset$filter_by_pct_TSO_concatemer_MADs, "thresholds")
```
...
PAG_sceset$MADs_filters <- !(
    ...
    # Remove cells with unusual proportion of reads in TSO concatemers
    PAG_sceset$filter_by_pct_TSO_concatemer_MADs
)
...
data.frame(ByLibSize=sum(PAG_sceset$filter_by_total_counts_MADs), 
           ...
           ByTSOconcatemer=sum(PAG_sceset$filter_by_pct_TSO_concatemer_MADs),
           Remaining=sum(PAG_sceset$MADs_filters)
           )

table(PAG_sceset$MADs_filters, PAG_sceset$PAG.area)
...
PAG_sceset <- runColDataPCA(PAG_sceset, outliers = TRUE,
                            variables = list("sum", "detected", "altexps_ERCC_percent", "subsets_Mitochondrial_percent", 
                                             "subsets_Ribosomal_percent", "subsets_TSO_concatemers_percent"),
                            name = "PCA_coldata")
plotReducedDim(PAG_sceset, dimred = "PCA_coldata", colour_by = "outlier", shape_by = "PAG.area")
table(PAG_sceset$outlier)

PAG_sceset$PCA_filters <- !PAG_sceset$outlier
table(PAG_sceset$PCA_filters, PAG_sceset$PAG.area)
```
...
vennDiagram(venn.diag,
            names = c("Manual", "MADs", "PCA based"),
            main = "Venn Diagram of excluded cells",
            circle.col = c("#E69F00", "#56B4E9", "#009E73"))

# Save the plot
ggsave(filename = str_c(date, "_venn_cell_QC_filtering_summary.pdf"),
       plot = vennDiagram(venn.diag,
                          names = c("Manual", "MADs", "PCA based"),
                          main = "Venn Diagram of excluded cells", 
                          cex = c(1.1, 1, 0.9),
                          circle.col = c("#E69F00", "#56B4E9", "#009E73")),
...
theme_args_violin_summary <- theme(plot.title = element_text(size = 11, face = "bold"), 
                                  axis.title = element_text(size = 11, face = "plain"),
                                  axis.text = element_text(size = 10, face = "plain"), 
                                  legend.text = element_text(size = 9, face = "plain"), 
                                  strip.text = element_text(size = 10, face = "plain"))

# Make individual violin plots, highlighting excluded cells (default colours c("#729ECE", "#FF9E4A"):
violin_cell_QC_library <- plotColData(PAG_sceset, x = "cell.type", y = "total_million",
                                      colour_by = "filter_by_total_counts_manual", theme_size = 11, point_alpha = 0.8) + 
  ggtitle("Library size") + xlab(NULL) + ylab("# of reads (millions)") + theme_args_violin_summary +
  scale_colour_manual(name = "Excluded", values = c("grey", "red"), aesthetics = c("colour", "fill"))
saveRDS(violin_cell_QC_library, file = str_c(path_for_figures, "violin_cell_QC_library.rds"))

violin_cell_QC_detected <- plotColData(PAG_sceset, x = "cell.type", y = "detected",
                                       colour_by = "filter_by_expr_features_manual", theme_size = 11, point_alpha = 0.8) + 
  ggtitle("Detected genes") + xlab(NULL) + ylab("# of genes detected") + theme_args_violin_summary +
  scale_colour_manual(name = "Excluded", values = c("grey", "red"), aesthetics = c("colour", "fill"))
saveRDS(violin_cell_QC_detected, file = str_c(path_for_figures, "violin_cell_QC_detected.rds"))

violin_cell_QC_mitochondrial <- plotColData(PAG_sceset, x = "cell.type", y = "subsets_Mitochondrial_percent",
                                            colour_by = "filter_by_pct_Mitochondrial_manual", theme_size = 11, point_alpha = 0.8) + 
  ggtitle("Mitochondrial genes") + xlab(NULL) + ylab("% of total reads") + theme_args_violin_summary +
  scale_colour_manual(name = "Excluded", values = c("grey", "red"), aesthetics = c("colour", "fill"))
saveRDS(violin_cell_QC_mitochondrial, file = str_c(path_for_figures, "violin_cell_QC_mitochondrial.rds"))

violin_cell_QC_ribosomal <- plotColData(PAG_sceset, x = "cell.type", y = "subsets_Ribosomal_percent",
                                        colour_by = "filter_by_pct_Ribosomal_manual", theme_size = 11, point_alpha = 0.8) +
  ggtitle("Ribosomal genes") + xlab(NULL) + ylab("% of total reads") + theme_args_violin_summary +
  scale_colour_manual(name = "Excluded", values = c("grey", "red"), aesthetics = c("colour", "fill"))
saveRDS(violin_cell_QC_ribosomal, file = str_c(path_for_figures, "violin_cell_QC_ribosomal.rds"))
...
violin_cell_QC_ERCC <- plotColData(PAG_sceset, x = "cell.type", y = "altexps_ERCC_percent",
  ...
  ggtitle("ERCC spike-ins") + xlab(NULL) + ylab("% of total reads") + theme_args_violin_summary +
  scale_colour_manual(name = "Excluded", values = c("grey", "red"), aesthetics = c("colour", "fill"))
saveRDS(violin_cell_QC_ERCC, file = str_c(path_for_figures, "violin_cell_QC_ERCC.rds"))

violin_cell_QC_TSO <- plotColData(PAG_sceset, x = "cell.type", y = "subsets_TSO_concatemers_percent",
                                  colour_by = "filter_by_pct_TSO_concatemer_manual", theme_size = 11, point_alpha = 0.8) + 
  ggtitle("TSO concatemers") + xlab(NULL) + ylab("% of total reads") + theme_args_violin_summary +
  scale_colour_manual(name = "Excluded", values = c("grey", "red"), aesthetics = c("colour", "fill"))
saveRDS(violin_cell_QC_TSO, file = str_c(path_for_figures, "violin_cell_QC_TSO.rds"))

violin_cell_QC_top100genes <- plotColData(PAG_sceset, x = "cell.type", y = "percent_top_100",
                                          colour_by = "filter_by_percent_top_100_manual", theme_size = 11, point_alpha = 0.8) + 
  ggtitle("Top 100 genes") + xlab(NULL) + ylab("% of total reads") + theme_args_violin_summary +
  scale_colour_manual(name = "Excluded", values = c("grey", "red"), aesthetics = c("colour", "fill"))
saveRDS(violin_cell_QC_top100genes, file = str_c(path_for_figures, "violin_cell_QC_top100genes.rds"))

violin_cell_QC_EYFP <- plotColData(PAG_sceset, x = "cell.fluorophore", y = "log2_total_counts_EYFP",
                                   colour_by = "filter_by_wrong_fluorophore_tdTomato_cells", theme_size = 11, point_alpha = 0.8) + 
  ggtitle("EYFP expression") + xlab(NULL) + ylab("log2 raw counts") + theme_args_violin_summary +
  scale_colour_manual(name = "Excluded", values = c("grey", "red"), aesthetics = c("colour", "fill"))
saveRDS(violin_cell_QC_EYFP, file = str_c(path_for_figures, "violin_cell_QC_EYFP.rds"))

violin_cell_QC_tdTomato <- plotColData(PAG_sceset, x = "cell.fluorophore", y = "log2_total_counts_tdTomato",
                                       colour_by = "filter_by_wrong_fluorophore_EYFP_cells", theme_size = 11, point_alpha = 0.8) + 
  ggtitle("tdTomato expression") + xlab(NULL) + ylab("log2 raw counts") + theme_args_violin_summary +
  scale_colour_manual(name = "Excluded", values = c("grey", "red"), aesthetics = c("colour", "fill"))
saveRDS(violin_cell_QC_tdTomato, file = str_c(path_for_figures, "violin_cell_QC_tdTomato.rds"))

# Compose figure using `patchwork`
cell_QC_summary_composed <- (violin_cell_QC_library + violin_cell_QC_detected) / 
  (violin_cell_QC_mitochondrial + violin_cell_QC_ribosomal) /
  (violin_cell_QC_ERCC + violin_cell_QC_TSO) /
  (violin_cell_QC_EYFP + violin_cell_QC_tdTomato) / 
  (violin_cell_QC_top100genes + plot_spacer()) +
  plot_annotation(title = "Summary of cell QC filters", theme = theme(plot.title = element_text(size = 12, face = "bold")), tag_levels = "A") + 
  plot_layout(guides = "collect") + theme(plot.tag = element_text(size = 11))

cell_QC_summary_composed

# Save the plot
ggsave(filename = str_c(date, "_violin_cell_QC_summary_composed.pdf"),
...
# Below are a couple of alternative (old) ways of generating this plot:
# ## Plot a summary of the cell QC filters, highlighting only one filter at time (default colours c("#729ECE", "#FF9E4A"):
# cell_QC_summary <- gridExtra::grid.arrange(plotColData(PAG_sceset, x = "cell.type", y = "total_million", 
#                                                        colour_by = "filter_by_total_counts_manual", theme_size = 11, point_alpha = 0.8) + 
#                                              ggtitle("Library size") + xlab(NULL) + ylab("Total reads (millions)") +
#                                              scale_colour_manual(name = "Excluded", values = c("grey", "red"), aesthetics = c("colour", "fill")),
#                                            plotColData(PAG_sceset, x = "cell.type", y = "detected",
#                                                        colour_by = "filter_by_expr_features_manual", theme_size = 11, point_alpha = 0.8) + 
#                                              ggtitle("Detected genes") + xlab(NULL) + ylab("Number of genes detected") + 
#                                              scale_colour_manual(name = "Excluded", values = c("grey", "red"), aesthetics = c("colour", "fill")),
#                                            plotColData(PAG_sceset, x = "cell.type", y = "subsets_Mitochondrial_percent",
#                                                        colour_by = "filter_by_pct_Mitochondrial_manual", theme_size = 11, point_alpha = 0.8) + 
#                                              ggtitle("Percent reads in Mitochondrial genes") + xlab(NULL) + ylab("% reads in Mitochondrial genes") + 
#                                              scale_colour_manual(name = "Excluded", values = c("grey", "red"), aesthetics = c("colour", "fill")),
#                                            plotColData(PAG_sceset, x = "cell.type", y = "subsets_Ribosomal_percent",
#                                                        colour_by = "filter_by_pct_Ribosomal_manual", theme_size = 11, point_alpha = 0.8) +
#                                              ggtitle("Percent reads in Ribosomal genes") + xlab(NULL) + ylab("% reads in Ribosomal genes") +
#                                              scale_colour_manual(name = "Excluded", values = c("grey", "red"), aesthetics = c("colour", "fill")),
#                                            plotColData(PAG_sceset, x = "cell.type", y = "altexps_ERCC_percent",
#                                                        colour_by = "filter_by_pct_ERCC_manual", theme_size = 11, point_alpha = 0.8) + 
#                                              ggtitle("Percent reads in ERCC spike-ins") + xlab(NULL) + ylab("% reads in ERCC spike-ins") + 
#                                              scale_colour_manual(name = "Excluded", values = c("grey", "red"), aesthetics = c("colour", "fill")),
#                                            plotColData(PAG_sceset, x = "cell.type", y = "subsets_TSO_concatemers_percent",
#                                                        colour_by = "filter_by_pct_TSO_concatemer_manual", theme_size = 11, point_alpha = 0.8) + 
#                                              ggtitle("Percent reads in TSO concatemers") + xlab(NULL) + ylab("% reads in TSO concatemers") + 
#                                              scale_colour_manual(name = "Excluded", values = c("grey", "red"), aesthetics = c("colour", "fill")),
#                                            plotColData(PAG_sceset, x = "cell.type", y = "percent_top_100",
#                                                        colour_by = "filter_by_percent_top_100_manual", theme_size = 11, point_alpha = 0.8) + 
#                                              ggtitle("Percent reads in top 100 genes") + xlab(NULL) + ylab("% reads in top 100 genes") + 
#                                              scale_colour_manual(name = "Excluded", values = c("grey", "red"), aesthetics = c("colour", "fill")),
#                                            plotColData(PAG_sceset, x = "cell.fluorophore", y = "log2_total_counts_EYFP",
#                                                        colour_by = "filter_by_wrong_fluorophore_tdTomato_cells", theme_size = 11, point_alpha = 0.8) + 
#                                              ggtitle("EYFP expression") + xlab(NULL) + ylab("Expression (log2 raw counts)") + 
#                                              scale_colour_manual(name = "Excluded", values = c("grey", "red"), aesthetics = c("colour", "fill")),
#                                            plotColData(PAG_sceset, x = "cell.fluorophore", y = "log2_total_counts_tdTomato",
#                                                        colour_by = "filter_by_wrong_fluorophore_EYFP_cells", theme_size = 11, point_alpha = 0.8) + 
#                                              ggtitle("tdTomato expression") + xlab(NULL) + ylab("Expression (log2 raw counts)") + 
#                                              scale_colour_manual(name = "Excluded", values = c("grey", "red"), aesthetics = c("colour", "fill"))
#                                            )
# # Save the plot
# ggsave(filename = str_c(date, "_violin_cell_QC_summary_specific.pdf"),
#        plot = cell_QC_summary,
#        device = "pdf", # or one of "eps", "ps", "tex" (pictex), "pdf", "jpeg", "tiff", "png", "bmp", "svg" or "wmf" (windows only).
#        path = path_for_figures,
#        width = 7, height = 10, units = "in", dpi = 300,
#        family = "Arial", bg = "transparent"
...
# 
# 
# ## Plot a summary of the cell QC filters, highlighting all excluded cells in each subplot (default colours c("#729ECE", "#FF9E4A"):
# cell_QC_summary_all <- gridExtra::grid.arrange(plotColData(PAG_sceset, x = "cell.type", y = "total_million",
#                                                            colour_by = "use", theme_size = 11, point_alpha = 0.8) + 
#                                                  ggtitle("Library size") + xlab(NULL) + ylab("Total reads (millions)") +
#                                                  scale_colour_manual(name = "Keep cell", values = c("red","grey"), aesthetics = c("colour", "fill")),
#                                                plotColData(PAG_sceset, x = "cell.type", y = "detected",
#                                                            colour_by = "use", theme_size = 11, point_alpha = 0.8) + 
#                                                  ggtitle("Detected genes") + xlab(NULL) + ylab("Number of genes detected") + 
#                                                  scale_colour_manual(name = "Keep cell", values = c("red","grey"), aesthetics = c("colour", "fill")),
...
#                                                )
# # Save the plot
# ggsave(filename = str_c(date, "_violin_cell_QC_summary_all.pdf"),
#        plot = cell_QC_summary_all,
#        device = "pdf", # or one of "eps", "ps", "tex" (pictex), "pdf", "jpeg", "tiff", "png", "bmp", "svg" or "wmf" (windows only).
#        path = path_for_figures,
#        width = 7, height = 10, units = "in", dpi = 300,
#        family = "Arial", bg = "transparent"
...
We have now QCed our dataset. Before we proceed to normalisation and downstream analyis, we save our progress.
```{r}
...
Before saving and finishing this section, we can take a look at the total number of cells before and after QC:
```{r}
# Relation of aspirated and sequenced cells before QC:
table(PAG_sceset$cell.type)
table(PAG_sceset$PAG.area)
table(PAG_sceset$celltype_PAGarea)
table(PAG_sceset$PAG.hemisphere)
table(PAG_sceset$mouse.sex)

# Relation of aspirated and sequenced cells after QC:
table(PAG_sceset_qc$cell.type)
table(PAG_sceset_qc$PAG.area)
table(PAG_sceset_qc$celltype_PAGarea)
table(PAG_sceset_qc$PAG.hemisphere)
table(PAG_sceset_qc$mouse.sex)
```
