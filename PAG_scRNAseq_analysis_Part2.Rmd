...
spikeNames(PAG_sceset) # Only ERCCs should appear as Spikes in the SCE object - Deprecated
PAG_sceset
...
Additionally, the number of detected features in each cell is defined as the number of features with non-zero counts for that cell. Any cell with very few detected genes is likely to be of poor quality as the diverse transcript population has not been successfully captured.
```{r}
...
# ERCCs
hist(PAG_sceset$subsets_ERCC_percent,
     breaks = 50,
...
summary(PAG_sceset$subsets_ERCC_percent)
...
PAG_sceset$filter_by_pct_ERCC_manual <- (PAG_sceset$subsets_ERCC_percent > 1)
...
# Custom filters: identify the cells from male mice that express the XIST
PAG_sceset$filter_by_wrong_Xist_in_males <- (PAG_sceset$mouse.sex=="M" & PAG_sceset$subsets_XIST_sum > 100)
...
# Finally, add a label to the metadata indicating whether a cell expresses only VGAT, only VGluT2, or both:
PAG_sceset$VGAT_VGluT2_expression[(which(PAG_sceset$cell.type=="VGAT" & PAG_sceset$log2_total_counts_VGluT2 == 0))] <- "VGAT_only"
PAG_sceset$VGAT_VGluT2_expression[(which(PAG_sceset$cell.type=="VGAT" & PAG_sceset$log2_total_counts_VGluT2 > 0))] <- "VGAT_VGluT2"
PAG_sceset$VGAT_VGluT2_expression[(which(PAG_sceset$cell.type=="VGluT2" & PAG_sceset$log2_total_counts_VGAT == 0))] <- "VGluT2_only"
PAG_sceset$VGAT_VGluT2_expression[(which(PAG_sceset$cell.type=="VGluT2" & PAG_sceset$log2_total_counts_VGAT > 0))] <- "VGluT2_VGAT"
PAG_sceset$VGAT_VGluT2_expression <- factor(PAG_sceset$VGAT_VGluT2_expression,
                                            levels = c("VGAT_only", "VGAT_VGluT2", "VGluT2_only", "VGluT2_VGAT"))
...
table(PAG_sceset$manual_filters, PAG_sceset$VGAT_VGluT2_expression)
```
...
```{r}
# ERCCs
PAG_sceset$filter_by_pct_ERCC_MADs <- isOutlier(PAG_sceset$subsets_ERCC_percent, nmads = 5, type = "higher", batch = NULL)
...
PAG_sceset <- runColDataPCA(PAG_sceset, outliers = TRUE,
                            variables = list("sum", "detected", "subsets_ERCC_percent", "subsets_Mitochondrial_percent", "subsets_Ribosomal_percent", "subsets_TSO_concatamers_percent"))
...
After comparing the different filtering strategies, we can see that the Manual filter is the most flexible and allows us to add custom filters based on our experimental design. We will prime knowing why we remove each cell and will thus apply the manual filter over the MADs and the PCA based filters.
```{r}
...
```{r}
gridExtra::grid.arrange(plotColData(PAG_sceset, x = "cell.type", y = "total", colour_by = "use") + ggtitle("Library size"),
                        plotColData(PAG_sceset, x = "cell.type", y = "detected", colour_by = "use") + ggtitle("Detected genes"),
                        plotColData(PAG_sceset, x = "cell.type", y = "percent_top_100", colour_by = "use") + ggtitle("Percent reads consumed by 100 most expressed genes"),
                        plotColData(PAG_sceset, x = "cell.type", y = "subsets_ERCC_percent", colour_by = "use") + ggtitle("Percent ERCC"),
                        plotColData(PAG_sceset, x = "cell.type", y = "subsets_Mitochondrial_percent", colour_by = "use") + ggtitle("Percent Mitochondrial genes"),
                        plotColData(PAG_sceset, x = "cell.type", y = "subsets_Ribosomal_percent", colour_by = "use") + ggtitle("Percent Ribosomal genes"),
                        plotColData(PAG_sceset, x = "cell.type", y = "subsets_TSO_concatamers_percent", colour_by = "use") + ggtitle("Percent TSO concatamers"),
                        plotColData(PAG_sceset, x = "cell.fluorophor", y = "subsets_EYFP_sum", colour_by = "use") + ggtitle("EYFP expression"),
                        plotColData(PAG_sceset, x = "cell.fluorophor", y = "subsets_tdTomato_sum", colour_by = "use") + ggtitle("tdTomato expression"),
                        plotColData(PAG_sceset, x = "cell.type", y = "subsets_VGAT_sum", colour_by = "use") + ggtitle("VGAT expression"),
                        plotColData(PAG_sceset, x = "cell.type", y = "subsets_VGluT2_sum", colour_by = "use") + ggtitle("VGluT2 expression")
                        )
```
...
rowData(PAG_sceset)$genes_expressed <- use_numcells > 0 # This will leave genes expressed in less than X number of cells as FALSE
