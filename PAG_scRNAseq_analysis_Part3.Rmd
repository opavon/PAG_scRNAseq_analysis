...
* _SCnorm (scRNA-seq specific)_: Another method developed to normalize scRNA-seq data (Bacher et al. Nature Methods 2017).
...
We will use `scran` to calculate size factors for our samples and then use them for normalization. Importantly, see Vallejos et al., Nature Methods 2017 for an explanation of why it is not a good idea to use CPM normalization on scRNA-seq data.
```{r}
...
p1 <- plotRLE(PAG_sceset_qc, 
              exprs_values = "counts",
              exprs_logged = TRUE,
              style = "minimal", # alternatively try "full"
              legend = TRUE,
              colour_by = "mouse.id"
              ) + ggtitle("Raw counts")
...
p3 <- plotRLE(PAG_sceset_qc, 
              ...
              legend = TRUE,
              colour_by = "mouse.id"
              ) + ggtitle("Logcounts Scran")

# If you use `style = "full"`, the resulting plots will be too heavy to plot at once (around 140MB each), so plot each graph one at a time if that option is selecte instead of "minimal":
plot(p1)
plot(p2)
plot(p3)
```
