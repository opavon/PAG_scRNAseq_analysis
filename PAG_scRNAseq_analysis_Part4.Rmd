...
subtitle: "Parts IV: modelling the technical noise and feature selection"
author:
...
Taking all this into consideration, we will use the HVGs obtained after modelling the *mean-variance after blocking* and the *mean-CV2 relation without blocking*.
...
plotExpression(PAG_sceset_qc_norm_filt, features = rownames(hvg_var_out_no_spikes_filt)[1:50]) + fontsize + ggtitle("Top 50 HVGs from mean-var modelling")
plotExpression(PAG_sceset_qc_norm_filt, features = rownames(hvg_var_out_no_spikes_block_filt)[1:50]) + fontsize + ggtitle("Top 50 HVGs from mean-var modelling after blocking")
plotExpression(PAG_sceset_qc_norm_filt, features = rownames(hvg_cv2_out_no_spikes_filt)[1:50]) + fontsize + ggtitle("Top 50 HVGs from mean-cv2 modelling")
plotExpression(PAG_sceset_qc_norm_filt, features = rownames(hvg_cv2_out_no_spikes_block_filt)[1:50]) + fontsize + ggtitle("Top 50 HVGs from mean-cv2 modelling after blocking")
```
