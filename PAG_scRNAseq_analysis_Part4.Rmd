...
# Fit variance-mean trend blocking on mouse.id
start_time <- Sys.time() # Takes around 32min
...
PAG_sceset_qc_norm_filt <- denoisePCA(PAG_sceset_qc_norm_filt, 
                                      ...
                                      assay.type = "corrected") # try "logcounts" as well
...
## Step 5.5 | Save the filtered, corrected, and denoised SCE object
```{r}
...
Log-transformation reduces the variance on the first principal component, separates some biological effects, and makes the distribution of the expression values more normal. BUT - it is not a proper normalisation step. Log-transformation is not enough to account for different technical factors between the cells (e.g. sequencing depth). Do not use `logcounts_raw` for your downstream analysis, instead use the __logcounts__ slot of the `SingleCellExperiment` object as a minimum suitable data, which is not just log-transformed, but also normalised by library size. By default only the top 500 most variable genes are used by `scater` to calculate the PCA. This can be adjusted by changing the __ntop__ argument.
...
By comparison, we should not observe a clear separation of cells by batch (`batch.processing` or `mouse.id`), indicating that our batch correction step using `removeBatchEffect()` was successful.
```{r}
plotReducedDim(PAG_sceset_qc_norm_filt,
               use_dimred = "PCA",
               ncomponents = 4,
...
pca1 <- plotPCA(PAG_sceset_qc_norm_filt,
                rerun = TRUE,
...
pca2 <- plotPCA(PAG_sceset_qc_norm_filt, 
                rerun = TRUE,
...
pca3 <- plotPCA(PAG_sceset_qc_norm_filt,    
                rerun = TRUE,
                run_args = list(exprs_values="logcounts"), # list(ntop = 200, exprs_values = "logcounts", feature_set = chosen),
                colour_by = "cell.type",
                size_by = "total_features_by_counts"
                ) + ggtitle("Scran")

# After removeBatchEffect correction
pca4 <- plotPCA(PAG_sceset_qc_norm_filt, 
                rerun = TRUE, 
...
```{r}
set.seed(1991)
chosen <- rownames(PAG_sceset_qc_norm_filt) %in% metadata(PAG_sceset_qc_norm_filt)$hvg_out_no_spikes_filt 
...
tsne_p50 <- plotTSNE(PAG_sceset_qc_norm_filt,
                     rerun = TRUE,
                     run_args = list(ncomponents = 2, ntop = 1000, feature_set = chosen, exprs_values = "corrected", 
...
plotTSNE(PAG_sceset_qc_norm_filt, colour_by="Gad1", shape_by="cell.type")
reducedDimNames(PAG_sceset_qc_norm_filt)
```
