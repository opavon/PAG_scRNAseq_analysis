...
hvg_out_spikes <- var_out_spikes[which(var_out_spikes$FDR <= 0.05 & var_out_spikes$bio > 0), ]
nrow(hvg_out_spikes)

hvg_out_spikes_mouseid <- var_out_spikes_mouseid[which(var_out_spikes_mouseid$FDR <= 0.05 & var_out_spikes_mouseid$bio > 0), ]
nrow(hvg_out_spikes_mouseid)

hvg_out_spikes_batch <- var_out_spikes_batch[which(var_out_spikes_batch$FDR <= 0.05 & var_out_spikes_batch$bio > 0), ]
nrow(hvg_out_spikes_batch)


# from trendVar without spikes
hvg_out_no_spikes <- var_out_no_spikes[which(var_out_no_spikes$FDR <= 0.05 & var_out_no_spikes$bio > 0), ]
nrow(hvg_out_no_spikes)

hvg_out_no_spikes_mouseid <- var_out_no_spikes_mouseid[which(var_out_no_spikes_mouseid$FDR <= 0.05 & var_out_no_spikes_mouseid$bio > 0), ]
nrow(hvg_out_no_spikes_mouseid)

hvg_out_no_spikes_batch <- var_out_no_spikes_batch[which(var_out_no_spikes_batch$FDR <= 0.05 & var_out_no_spikes_batch$bio > 0), ]
...
for(n in assayNames(PAG_sceset_qc_norm_filt)) {
    print(
        plotPCA(
            ...
            shape_by = "cell.type",
            size_by = "total_features_by_counts",
            run_args=list(exprs_values = n, feature_set = metadata(PAG_sceset_qc_norm_filt)$hvg_out_no_spikes_filt)
        ) +
        ggtitle(n)
    )
}
...
## Step 5.3 | Denoising expression values using PCA
...
```{r}
PAG_sceset_qc_norm_filt <- denoisePCA(PAG_sceset_qc_norm_filt, 
                                      technical = var_out_no_spikes, 
...
### 5.3.1 | Correlations with Principal Components - Choosing the number of PCs
...
```{r}
principal_components <- reducedDim(PAG_sceset_qc_norm_filt, "PCA")
proportion_var <- attr(principal_components, "percentVar")
plot(proportion_var, xlab="PC", ylab="Proportion of variance explained")
```

## Step 5.4 | Save the filtered, corrected, and denoised SCE object
```{r}
...
Log-transformation reduces the variance on the first principal component, separates some biological effects, and makes the distribution of the expression values more normal. BUT - it is not a proper normalisation step. Log-transformation is not enough to account for different technical factors between the cells (e.g. sequencing depth). Do not use logcounts_raw for your downstream analysis, instead as a minimum suitable data use the __logcounts__ slot of the SingleCellExperiment object, which not just log-transformed, but also normalised by library size (e.g. CPM normalisation).

By default only the top 500 most variable genes are used by scater to calculate the PCA. This can be adjusted by changing the __ntop__ argument. Ideally use it with logcounts normalised.
```{r}
# If you plot the PCAs using the size_by = total_features_by_counts you can assess whether the total features correlates with the PCs: if there is a correlation with the total features, you will see that the cells are distributed in a particular way (large bubbles on the left, and small bubbles on the right).
set.seed(1991)
...
pca4 <- plotPCA(
    PAG_sceset_qc[endogenous_genes, ], 
    rerun = TRUE, 
    run_args = list(ntop = 200, exprs_values = "logcounts"),
    colour_by = "Gad1",
    shape_by = "cell.type"#,
    #size_by = "total_features_by_counts"
) + ggtitle("Scran")
...
```
After QC the data should look more homogeneous, because we removed the bad cells.

### runPCA
scater allows you to store the results of PCA in the reducedDim slot
```{r}
PAG_sceset_qc <- runPCA(PAG_sceset_qc,
                        run_args = list(exprs_values="logcounts"),
                        rerun = TRUE,
                        ncomponents = 10,
                        #ntop = 20835,
...
plotReducedDim(PAG_sceset_qc, use_dimred="PCA", colour_by = "Slc32a1", shape_by = "cell.type", size_by = "total_features_by_counts")
...
plotReducedDim(PAG_sceset_qc_norm_filt,
               use_dimred = "PCA",
               ncomponents = 4,
               colour_by="cell.type") + fontsize
```

By comparison, we should not observe a clear separation of cells by batch (`batch.processing` or `mouse.id`), which would indicate that our batch correction step using `removeBatchEffect()` was successful.
```{r}
plotReducedDim(PAG_sceset_qc_norm_filt,
               use_dimred = "PCA",
               ncomponents = 4,
...
```{r}
set.seed(1991)
tsne_p5 <- plotTSNE(PAG_sceset_qc_norm_filt, 
                    rerun = TRUE,
                    run_args=list(feature_set = "selected genes", exprs_values = "corrected", use_dimred = "PCA", perplexity = 5),
                    colour_by="batch.processing") + fontsize + ggtitle("Perplexity = 5")

tsne_p10 <- plotTSNE(PAG_sceset_qc_norm_filt, 
                     rerun = TRUE,
                     run_args=list(use_dimred="PCA", perplexity=10),
                     colour_by="cell.type") + fontsize + ggtitle("Perplexity = 10")

tsne_p20 <- plotTSNE(PAG_sceset_qc_norm_filt,
                     rerun = TRUE,
                     run_args=list(use_dimred="PCA", perplexity=20),
                     colour_by="PAG.areacollection") + fontsize + ggtitle("Perplexity = 20")

multiplot(tsne_p5, tsne_p10, tsne_p20, cols=3)
...
```{r}
set.seed(1991)
PAG_sceset_qc_norm_filt <- runTSNE(PAG_sceset_qc_norm_filt, use_dimred="PCA", perplexity=20)
reducedDimNames(PAG_sceset_qc_norm_filt)
```

An alternative to PCA for visualizing scRNASeq data is a tSNE plot. tSNE (t-Distributed Stochastic Neighbor Embedding) combines dimensionality reduction (e.g. PCA) with random walks on the nearest-neighbour network to map high dimensional data to a 2-dimensional space while preserving local distances between cells. In contrast with PCA, tSNE is a stochastic algorithm which means running the method multiple times on the same dataset will result in different plots. Due to the non-linear and stochastic nature of the algorithm, tSNE is more difficult to intuitively interpret. To ensure reproducibility, we fix the seed of the random-number generator in the code below so that we always get the same plot.

tSNE plots are good for visualising your data, but should NOT be used for clustering. Furthermore tSNE requires you to provide a value of perplexity which reflects the number of neighbours used to build the nearest-neighbour network; a high value creates a dense network which clumps cells together while a low value makes the network more sparse allowing groups of cells to separate from each other. scater uses a default perplexity of the total number of cells divided by five (rounded down). You should try different perplexities and run it several times to make sure you are not getting a weird result. If your data are very homogeneous the tSNE plots are going to be hard to interpret, whereas if they contain very well defined clusters it will visualize them very nicely.
```{r}
set.seed(1991)
...
tsne1 <- plotTSNE(
  PAG_sceset_qc[endogenous_genes, ],
  rerun = TRUE, 
  run_args = list(ntop = 200, exprs_values = "logcounts", perplexity=20, pca = TRUE, initial_dims = 2),
  colour_by="Accn4",
  shape_by="cell.type"
) +  ggtitle("Scran (p=20, dims=2)")

tsne2 <- plotTSNE(
  PAG_sceset_qc[endogenous_genes, ],
  rerun = TRUE, 
  run_args = list(ntop = 200, exprs_values = "logcounts", perplexity=20, pca = TRUE, initial_dims = 10),
  colour_by="Accn4",
  shape_by="cell.type"
) +  ggtitle("Scran (p=20, dims=10)")

tsne3 <- plotTSNE(
  PAG_sceset_qc[endogenous_genes, ],
  rerun = TRUE, 
  run_args = list(ntop = 200, exprs_values = "logcounts", perplexity=50, pca = TRUE, initial_dims = 2),
  colour_by="Slc30a3",
  shape_by="cell.type"
) +  ggtitle("Scran (p=50, dims=2)")

tsne4 <- plotTSNE(
  PAG_sceset_qc[endogenous_genes, ],
  rerun = TRUE, 
  run_args = list(ntop = 200, exprs_values = "logcounts", perplexity=50, pca = TRUE, initial_dims = 10),
  colour_by="Gad1",
  shape_by="cell.type"
) +  ggtitle("Scran (p=50, dims=10)")

multiplot(tsne1, tsne2, tsne3, tsne4, cols=2)
```

#### runTSNE
```{r}
set.seed(1991)
PAG_sceset_qc <- runTSNE(PAG_sceset_qc,
                      rerun = TRUE,
                      run_args = list(exprs_values = "logcounts", perplexity=20)
                      )
plotTSNE(PAG_sceset_qc, colour_by="Gad1", shape_by="cell.type")
```


## Step 6.3 | Using HVGs for further data exploration
### 4a.3.1 | Heatmaps
...
Paper: McInnes L, Healy J (2018). UMAP: Uniform Manifold Approximation and Projection for Dimension Reduction. arXiv (https://arxiv.org/abs/1802.03426)
```{r}
library(umap)

PAG_sceset_qc <- runUMAP(
  PAG_sceset_qc[endogenous_genes, ],
  rerun = TRUE,
  run_args = list(exprs_values = "logcounts"),
  colour_by="Gad1",
...
plotUMAP(PAG_sceset_qc, colour_by="Gad1", shape_by="cell.type")
```
...
## STEP 4.4: iSEE - interactive SummarizedExperiment Explorer
