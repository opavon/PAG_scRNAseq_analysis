...
__Based on the [simpleSingleCell workflow in Bioconductor](https://bioconductor.org/packages/3.9/workflows/html/simpleSingleCell.html) and [Description of the HVG machinery in scran](https://github.com/LTLA/HVGDetection2018) maintained by Aaron Lun__.
...
hvg_out_spikes_ <- var_out_spikes[which(var_out_spikes$FDR <= 0.01 & var_out_spikes$bio >= 0.5),]
nrow(hvg_out_spikes)

hvg_out_spikes_mouseid <- var_out_spikes_mouseid[which(var_out_spikes_mouseid$FDR <= 0.01 & var_out_spikes_mouseid$bio >= 0.5),]
nrow(hvg_out_spikes_mouseid)

hvg_out_spikes_batch <- var_out_spikes_batch[which(var_out_spikes_batch$FDR <= 0.01 & var_out_spikes_batch$bio >= 0.5),]
nrow(hvg_out_spikes_batch)


# from trendVar without spikes
hvg_out_no_spikes <- var_out_no_spikes[which(var_out_no_spikes$FDR <= 0.01 & var_out_no_spikes$bio >= 0.5),]
nrow(hvg_out_no_spikes)

hvg_out_no_spikes_mouseid <- var_out_no_spikes_mouseid[which(var_out_no_spikes_mouseid$FDR <= 0.01 & var_out_no_spikes_mouseid$bio >= 0.5),]
nrow(hvg_out_no_spikes_mouseid)

hvg_out_no_spikes_batch <- var_out_no_spikes_batch[which(var_out_no_spikes_batch$FDR <= 0.01 & var_out_no_spikes_batch$bio >= 0.5),]
...
Before we store them in our `SingleCellExperiment` object, we check the distribution of expression values for the genes with the largest biological components to ensure that the variance estimate is not being dominated by one or two outlier cells.
```{r}
fontsize <- theme(axis.text=element_text(size=12), axis.title=element_text(size=16))

plotExpression(PAG_sceset_qc_norm, features=rownames(hvg_out_spikes_filt)[1:20]) + fontsize
plotExpression(PAG_sceset_qc_norm, features=rownames(hvg_out_spikes_mouseid_filt)[1:20]) + fontsize
plotExpression(PAG_sceset_qc_norm, features=rownames(hvg_out_spikes_batch_filt)[1:20]) + fontsize

plotExpression(PAG_sceset_qc_norm, features=rownames(hvg_out_no_spikes_filt)[1:20]) + fontsize
plotExpression(PAG_sceset_qc_norm, features=rownames(hvg_out_no_spikes_mouseid_filt)[1:20]) + fontsize
plotExpression(PAG_sceset_qc_norm, features=rownames(hvg_out_no_spikes_batch_filt)[1:20]) + fontsize
```

#### 4a.2.2 | Store and export the results
Once we have ordered and filtered the identified HVGs we can add the results into the `metadata` component of the `PAG_sceset_qc_norm` object. The metadata component can hold any object, as it is a list container. Any results that you’d like to keep are safe to store here, and a great way to save or share intermediate results that would otherwise be kept in separate objects.
```{r}
metadata(PAG_sceset_qc_norm)$hvg_out_spikes <- rownames(hvg_out_spikes)
metadata(PAG_sceset_qc_norm)$hvg_out_spikes_filt <- rownames(hvg_out_spikes_filt)

length(metadata(PAG_sceset_qc_norm)$hvg_out_spikes)
length(metadata(PAG_sceset_qc_norm)$hvg_out_spikes_filt)
...
metadata(PAG_sceset_qc_norm)$hvg_out_spikes <- rownames(hvg_out_spikes)
metadata(PAG_sceset_qc_norm)$hvg_out_spikes[1:20]

metadata(PAG_sceset_qc_norm)$hvg_out_spikes_mouseid <- rownames(hvg_out_spikes_mouseid)
metadata(PAG_sceset_qc_norm)$hvg_out_spikes_mouseid[1:20]

metadata(PAG_sceset_qc_norm)$hvg_out_spikes_batch <- rownames(hvg_out_spikes_batch)
metadata(PAG_sceset_qc_norm)$hvg_out_spikes_batch[1:20]

metadata(PAG_sceset_qc_norm)$hvg_out_no_spikes <- rownames(hvg_out_no_spikes)
metadata(PAG_sceset_qc_norm)$hvg_out_no_spikes[1:20]

metadata(PAG_sceset_qc_norm)$hvg_out_no_spikes_mouseid <- rownames(hvg_out_no_spikes_mouseid)
metadata(PAG_sceset_qc_norm)$hvg_out_no_spikes_mouseid[1:20]

metadata(PAG_sceset_qc_norm)$hvg_out_no_spikes_batch <- rownames(hvg_out_no_spikes_batch)
metadata(PAG_sceset_qc_norm)$hvg_out_no_spikes_batch[1:20]

# Compare how many genes you get in each condition with or without spike-ins
sum(rownames(hvg_out_spikes) %in% rownames(hvg_out_no_spikes))
sum(rownames(hvg_out_no_spikes_mouseid) %in% rownames(hvg_out_spikes_mouseid))
sum(rownames(hvg_out_no_spikes_batch) %in% rownames(hvg_out_spikes_batch))

sum(rownames(hvg_out_spikes) %in% rownames(hvg_out_spikes_mouseid))
sum(rownames(hvg_out_spikes_batch) %in% rownames(hvg_out_spikes_mouseid))
sum(rownames(hvg_out_spikes) %in% rownames(hvg_out_spikes_batch))

sum(rownames(hvg_out_no_spikes_mouseid) %in% rownames(hvg_out_no_spikes))
sum(rownames(hvg_out_no_spikes_batch) %in% rownames(hvg_out_no_spikes))
sum(rownames(hvg_out_no_spikes_batch) %in% rownames(hvg_out_no_spikes_mouseid))
```
...
var.cor <- correlatePairs(PAG_sceset_qc_norm, subset.row=rownames(hvg_out_no_spikes)) 
...
### Step 4a.4 | Using correlated HVGs for further data exploration
#### Heatmaps
...
norm.exprs <- exprs(PAG_sceset_qc_norm)[chosen,,drop=FALSE] 
...
```{r}
out_pca <- plotPCA(PAG_sceset_qc_norm[endogenous_genes, ],
                   rerun = TRUE,
...
```{r}
set.seed(1991) 
out5 <- plotTSNE(PAG_sceset_qc_norm[endogenous_genes, ],
                 rerun = TRUE, 
                 run_args = list(perplexity=5, feature_set=chosen),
                 colour_by="Slc17a6"
                 ) + ggtitle("Perplexity = 5")

out10 <- plotTSNE(PAG_sceset_qc_norm[endogenous_genes, ],
                  rerun = TRUE, 
                  run_args = list(perplexity=10, feature_set=chosen),
                  colour_by="Slc17a6"
                  ) + ggtitle("Perplexity = 10")

out20 <- plotTSNE(PAG_sceset_qc_norm[endogenous_genes, ],
                  rerun = TRUE, 
                  run_args = list(perplexity=20, feature_set=chosen),
                  colour_by="Slc17a6"
                  ) + ggtitle("Perplexity = 20")

out50 <- plotTSNE(PAG_sceset_qc_norm[endogenous_genes, ],
                  rerun = TRUE, 
...
```{r}
#expr_matrix <- M3Drop::M3DropConvertData(PAG_sceset_qc_norm) # For some reason it does not work
...
```{r}
plotExplanatoryPCs(PAG_sceset_qc_norm,
                   nvars_to_plot = 10,
                   npcs_to_plot = 10,
                   exprs_values = "logcounts", 
                   variables = c(
...
See: https://scrnaseq-course.cog.sanger.ac.uk/website/cleaning-the-expression-matrix.html#dealing-with-confounders
...
table(PAG_sceset_qc_norm$cell.type, PAG_sceset_qc_norm$PAG.hemisphere)
table(PAG_sceset_qc_norm$cell.type, PAG_sceset_qc_norm$mouse.sex)
#table(PAG_sceset_qc_norm$cell.type, PAG_sceset_qc_norm$reads.mitochondrial)
```

```{r}
# You can also check the distribution of each of these metadata classifications:
summary(factor(PAG_sceset_qc_norm$cell.type))
summary(factor(PAG_sceset_qc_norm$PAG.hemisphere))
summary(factor(PAG_sceset_qc_norm$PAG.areacollection))
summary(factor(PAG_sceset_qc_norm$mouse.sex))
```
...
```{r}
library(limma)
assay(PAG_sceset_qc_norm, "corrected") <- removeBatchEffect(logcounts(PAG_sceset_qc_norm),
                                                       design=model.matrix(~PAG_sceset_qc_norm$cell.type), 
                                                       batch=PAG_sceset_qc_norm$mouse.id,
                                                       batch2=PAG_sceset_qc_norm$batch.processing)
assayNames(PAG_sceset_qc_norm)
```
...
```{r}
for(n in assayNames(PAG_sceset_qc_norm)) {
    print(
        plotPCA(
            PAG_sceset_qc_norm[endogenous_genes, ],
...
res <- list()
for(n in assayNames(PAG_sceset_qc_norm)) {
    res[[n]] <- suppressWarnings(calc_cell_RLE(assay(PAG_sceset_qc_norm, n), erccs))
}
par(mar=c(6,4,1,1))
boxplot(res, las=2)
```
