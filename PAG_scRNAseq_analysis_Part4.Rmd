...
var.cor <- correlatePairs(PAG_sceset_qc, subset.row=rownames(hvg_out_no_spikes)) 
...
```{r}
out_pca <- plotPCA(PAG_sceset_qc[endogenous_genes, ],
                   rerun = TRUE,
...
```{r}
table(PAG_clusters, PAG_sceset_qc$cell.type)
table(PAG_clusters, PAG_sceset_qc$PAG.area)
```
...
```{r}
PAG_sceset_qc$Cluster <- factor(PAG_clusters)
plotTSNE(PAG_sceset_qc, colour_by="Cluster") + fontsize
```
...
```{r}
marker_genes <- findMarkers(PAG_sceset_qc, clusters=PAG_clusters, block=PAG_sceset_qc$mouse.id)
```
...
plotHeatmap(PAG_sceset_qc, 
            ...
            columns=order(PAG_sceset_qc$Cluster),
