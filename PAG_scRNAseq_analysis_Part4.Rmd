...
subtitle: "Part IV: modelling technical noise and feature selection"
author:
...
We continue using the `PAG_sceset_qc_norm` after normalisation. We should thus have a `logcounts` slot in `assays`:
```{r}
...
# If this is not 0, it means some dropouts were introduced by the normalisation and log-transformation steps as we had excluded genes with zero expression in all cells before normalisation
...
# If this is not 0, it means some dropouts were introduced by the normalisation step as we had excluded genes with zero expression in all cells before normalisation
...
plot(CV2_fit_no_spikes$mean,
     ...
     xlab = "Mean of normalised expression",
     ylab = "CV2 of normalised expression",
     xlim = c(1e-05, 1e+06))
...
Now that we have modelled the gene expression variance and CV2, and before moving on to downstream analysis, we consider dropping any Ribosomal, Mitochondrial, ERCC, sex-specific genes (e.g. XIST), transgenes (EYFP, tdTomato, Cre, TSO concatemers), and genes used for transgenic labelling of cells (VGAT and VGluT2) from the dataset, as some of them are not be biologically informative and others have been used to select the cells. We do so for both our `SingleCellExperiment` object and for the different sets of HVGs:
```{r}
##### 
...
rowData(PAG_sceset_qc_norm)$is_SmartSeqTSO <- (rowData(PAG_sceset_qc_norm)$Chromosome == "D") # TSO concatemers
# VGAT and VGluT2
...
# HVG from mean-var modelling
violin_HVG_var <- plotExpression(PAG_sceset_qc_norm_filt, features = rownames(hvg_var_out_no_spikes_filt)[1:25],
                                 exprs_values = "logcounts", theme_size = 11, point_alpha = 0.8
                                 ) + ggtitle("Top 25 HVGs from mean-var modelling") + theme_args
violin_HVG_var
saveRDS(violin_HVG_var, file = str_c(path_for_figures, "violin_HVG_var.rds"))
...
# HVG from mean-var modelling blocking by batch
violin_HVG_var_block <- plotExpression(PAG_sceset_qc_norm_filt, features = rownames(hvg_var_out_no_spikes_block_filt)[1:25],
                                       exprs_values = "logcounts", theme_size = 11, point_alpha = 0.8
                                       ) + ggtitle("Top 25 HVGs from mean-var modelling after blocking") + theme_args
violin_HVG_var_block
saveRDS(violin_HVG_var_block, file = str_c(path_for_figures, "violin_HVG_var_block.rds"))
...
# HVG from mean-cv2 modelling
violin_HVG_cv2 <- plotExpression(PAG_sceset_qc_norm_filt, features = rownames(hvg_cv2_out_no_spikes_filt)[1:25],
                                       exprs_values = "logcounts", theme_size = 11, point_alpha = 0.8
                                 ) + ggtitle("Top 25 HVGs from mean-cv2 modelling") + theme_args
violin_HVG_cv2
saveRDS(violin_HVG_cv2, file = str_c(path_for_figures, "violin_HVG_cv2.rds"))
...
# HVG from mean-cv2 modelling blocking by batch
violin_HVG_cv2_block <- plotExpression(PAG_sceset_qc_norm_filt, features = rownames(hvg_cv2_out_no_spikes_block_filt)[1:25],
                                       exprs_values = "logcounts", theme_size = 11, point_alpha = 0.8
                                       ) + ggtitle("Top 25 HVGs from mean-cv2 modelling after blocking") + theme_args
violin_HVG_cv2_block
saveRDS(violin_HVG_cv2_block, file = str_c(path_for_figures, "violin_HVG_cv2_block.rds"))
...
Just to ensure we keep everything, we also export the full table (not just the gene names) into a `.tsv`, `.csv`, or `.xlsx` file:
```{r}
## Export into .tsv
# Before filtering unwanted genes:
# write.table(hvg_var_out_no_spikes, file = str_c(path_for_figures, "PAG_hvg_var_out_no_spikes.tsv"),
#             sep = "\t", quote = FALSE, row.names = TRUE, col.names = NA)
# write.table(hvg_var_out_no_spikes_block, file = str_c(path_for_figures, "PAG_hvg_var_out_no_spikes_block.tsv"),
#             sep = "\t", quote = FALSE, row.names = TRUE, col.names = NA)
# write.table(hvg_cv2_out_no_spikes, file = str_c(path_for_figures, "PAG_hvg_cv2_out_no_spikes.tsv"),
#             sep = "\t", quote = FALSE, row.names = TRUE, col.names = NA)
# write.table(hvg_cv2_out_no_spikes_block, file = str_c(path_for_figures, "PAG_hvg_cv2_out_no_spikes_block.tsv"),
#             sep = "\t", quote = FALSE, row.names = TRUE, col.names = NA)
# 
# # After filtering unwanted genes:
# write.table(hvg_var_out_no_spikes_filt, file = str_c(path_for_figures, "PAG_hvg_var_out_no_spikes_filt.tsv"),
#             sep = "\t", quote = FALSE, row.names = TRUE, col.names = NA)
# write.table(hvg_var_out_no_spikes_block_filt, file = str_c(path_for_figures, "PAG_hvg_var_out_no_spikes_block_filt.tsv"),
#             sep = "\t", quote = FALSE, row.names = TRUE, col.names = NA)
# write.table(hvg_cv2_out_no_spikes_filt, file = str_c(path_for_figures, "PAG_hvg_cv2_out_no_spikes_filt.tsv"),
#             sep = "\t", quote = FALSE, row.names = TRUE, col.names = NA)
# write.table(hvg_cv2_out_no_spikes_block_filt, file = str_c(path_for_figures, "PAG_hvg_cv2_out_no_spikes_block_filt.tsv"),
#             sep = "\t", quote = FALSE, row.names = TRUE, col.names = NA)

## Export into .csv
# Before filtering unwanted genes:
write.csv(hvg_var_out_no_spikes, file = str_c(path_for_figures, "PAG_hvg_var_out_no_spikes.csv"), quote = FALSE, row.names = TRUE)
write.csv(hvg_var_out_no_spikes_block, file = str_c(path_for_figures, "PAG_hvg_var_out_no_spikes_block.csv"), quote = FALSE, row.names = TRUE)
write.csv(hvg_cv2_out_no_spikes, file = str_c(path_for_figures, "PAG_hvg_cv2_out_no_spikes.csv"), quote = FALSE, row.names = TRUE)
write.csv(hvg_cv2_out_no_spikes_block, file = str_c(path_for_figures, "PAG_hvg_cv2_out_no_spikes_block.csv"), quote = FALSE, row.names = TRUE)

# After filtering unwanted genes:
write.csv(hvg_var_out_no_spikes_filt, file = str_c(path_for_figures, "PAG_hvg_var_out_no_spikes_filt.csv"), quote = FALSE, row.names = TRUE)
write.csv(hvg_var_out_no_spikes_block_filt, file = str_c(path_for_figures, "PAG_hvg_var_out_no_spikes_block_filt.csv"), quote = FALSE, row.names = TRUE)
write.csv(hvg_cv2_out_no_spikes_filt, file = str_c(path_for_figures, "PAG_hvg_cv2_out_no_spikes_filt.csv"), quote = FALSE, row.names = TRUE)
write.csv(hvg_cv2_out_no_spikes_block_filt, file = str_c(path_for_figures, "PAG_hvg_cv2_out_no_spikes_block_filt.csv"), quote = FALSE, row.names = TRUE)

## Export everything into .xlsx files
# library(xlsx)
# write.xlsx(hvg_var_out_no_spikes, file = str_c(path_for_figures, "PAG_hvg_all.xlsx"), sheetName = "hvg_var")
# write.xlsx(hvg_var_out_no_spikes_block, file = str_c(path_for_figures, "PAG_hvg_all_block.xlsx"), sheetName = "hvg_var_block")
# write.xlsx(hvg_cv2_out_no_spikes, file = str_c(path_for_figures, "PAG_hvg_all.xlsx"), sheetName = "hvg_cv2", append = TRUE)
# write.xlsx(hvg_cv2_out_no_spikes_block, file = str_c(path_for_figures, "PAG_hvg_all_block.xlsx"), sheetName = "hvg_cv2_block", append = TRUE)
# write.xlsx(hvg_var_out_no_spikes_filt, file = str_c(path_for_figures, "PAG_hvg_all_filt.xlsx"), sheetName = "hvg_var_filt")
# write.xlsx(hvg_var_out_no_spikes_block_filt, file = str_c(path_for_figures, "PAG_hvg_all_block_filt.xlsx"), sheetName = "hvg_var_block_filt")
# write.xlsx(hvg_cv2_out_no_spikes_filt, file = str_c(path_for_figures, "PAG_hvg_all_filt.xlsx"), sheetName = "hvg_cv2_filt", append = TRUE)
# write.xlsx(hvg_cv2_out_no_spikes_block_filt, file = str_c(path_for_figures, "PAG_hvg_all_block_filt.xlsx"), sheetName = "hvg_cv2_block_filt", append = TRUE)
```

And we finally plot the modelled trend with the selected HVGs colored:
```{r}
options(scipen = 10) # default is 0. Doing this prevents having scientific notation in these plots
